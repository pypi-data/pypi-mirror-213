...
class RegisterUser(QDialog):
    ...
    def save_data(self):
        ...
        if not self.client_name.text():
            ...
            password_encoded = self.client_passwd.text().encode('utf-8')
            password_salt = self.client_name.text().lower().encode('utf-8')
            password_hash = hashlib.pbkdf2_hmac('sha256', password_encoded, password_salt, 100000)
            password_hex = binascii.hexlify(password_hash)

            self.database.add_user(self.client_name.text(), password_hex)
