...
def zip_and_delete_folder(src_folder, target_folder, zip_name):
    logger('INFO', f'Zippage du dossier {src_folder}')
    # Construire le chemin complet du fichier zip
    zip_path = os.path.join(target_folder, zip_name)

    # Faire une copie du chemin du dossier source pour supprimer plus tard
    original_path = src_folder

    # Vérifier si le fichier zip existe déjà, le supprimer le cas échéant
    if os.path.exists(zip_path):
        os.remove(zip_path)

    # Créer un fichier zip à partir du dossier source
    shutil.make_archive(zip_path, 'zip', src_folder)

    logger('INFO', f'Le Zippage a été créé avec succès dans {target_folder}')
    logger('INFO', f'Suppression du dossier d origine')
