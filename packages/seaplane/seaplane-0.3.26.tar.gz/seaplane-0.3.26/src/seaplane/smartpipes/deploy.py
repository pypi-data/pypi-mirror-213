...
CMD gunicorn --bind 0.0.0.0:${PORT} --workers 5 --timeout 300 demo:app
    """
...
def create_carrier_workload_file(
    ...
    workload = {
        "tenant": "tnt-0oqdcinn0pubb000tq8ng6qoos", # reject if you see this
        "id": coprocessor.id,
        "input": {
            "label": "carrier_in",
            "carrier": {
...
def deploy_coprocessor(smart_pipe: SmartPipe, coprocessor: Coprocessor, schema: Dict[str, Any]) -> None:
    ...
    workload = create_carrier_workload_file(smart_pipe.id, coprocessor, next_coprocessors)
        ...
        for sm in context.smart_pipes:                    
            for c in sm.coprocessors: 
                if c.id == coprocessor_id:
                    deploy_coprocessor(sm, c, schema)
