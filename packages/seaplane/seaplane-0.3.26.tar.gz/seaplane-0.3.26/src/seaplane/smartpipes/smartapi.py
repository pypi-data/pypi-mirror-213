...
async def publish_message(message: Any, streams: List[str]) -> None:    
    nc = await nats.connect("nats://carrier.staging.cplane.dev:2003", user_credentials=os.path.abspath("./carrier.creds"))
    js = nc.jetstream()

    nats_message = {"input": message}

    for topic in streams:
        log.debug(f"Sending to f{topic} coprocessor,  message: {nats_message}")
        ack = await js.publish(topic, str(json.dumps(nats_message)).encode())
        log.debug(f"ACK: {ack}")
...
def prod_https_api_start() -> Flask:
    ...
    for smart_pipe in smart_pipes:
        ...
        def endpoint_func(pipe: SmartPipe = smart_pipe) -> Any:
            ...
            loop.run_until_complete(publish_message(data, smart_pipe_first_coprocessors))
            return "Ok"
...
def start_coprocessor(coprocessor_id: str) -> None:
    ...
    while True:
        try:
            ...
            log.error(f"Error running Coprocessor: \n {traceback.format_exception(type(e), e, e.__traceback__)}")
...
def start() -> Optional[Flask]:
    if len(sys.argv) > 1:
        ...
        if command == "build":
            ...
            if len(sys.argv) == 3:
                coprocessor_id = sys.argv[2]
                deploy(coprocessor_id)
            else:
                deploy()
            return None
    ...
    if not coprocessor_id:
        ...
        #loop.run_until_complete(publish_message("HOLA", ["pre-processing"]))        
        #loop.close()
