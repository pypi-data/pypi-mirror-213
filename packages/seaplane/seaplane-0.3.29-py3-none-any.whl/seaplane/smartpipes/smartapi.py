...
async def publish_message(stream: str, message: Any, coprocessors: List[str]) -> None:    
    ...
    publish_coprocessors = []

    for coprocessor in coprocessors:
        log.debug(f"Sending to {stream}.{coprocessor} coprocessor,  message: {nats_message}")
        publish_coprocessors.append(js.publish(f"{stream}.{coprocessor}", str(json.dumps(nats_message)).encode()))        

    await asyncio.gather(publish_coprocessors)     
    ...
    for smart_pipe in smart_pipes:
        ...
        def endpoint_func(pipe: SmartPipe = smart_pipe) -> Any:
            if request.method == "POST" or request.method == "PUT":
                ...
                loop.run_until_complete(publish_message(pipe.id, data, smart_pipe_first_coprocessors))
                return "Ok"
            elif request.method == "GET":                
                return pipe.func("nothing")
