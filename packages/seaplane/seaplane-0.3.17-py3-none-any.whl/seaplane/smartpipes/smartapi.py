    ...
    while True:
        try: 
            ...
            if message["log"] is None:
                message["log"] = { coprocessor.id: message}
            else:
                message["log"][coprocessor.id] = message

            next_message = {"input": message["output"], "log": message["log"] }
            processor.write(str(next_message).encode())
        except Exception as ex:
            log.error(f"Error running Coprocessor: {str(ex)}")
