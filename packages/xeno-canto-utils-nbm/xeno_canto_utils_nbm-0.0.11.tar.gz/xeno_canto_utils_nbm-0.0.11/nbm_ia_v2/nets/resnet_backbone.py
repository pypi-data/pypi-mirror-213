    ...
    def __init__(
        ...
        if self.fpn:
            ...
            self.p_convolutions = nn.Sequential(*[
                nn.Conv2d(in_channels=channels + int(self.encode_frequency), out_channels=fpn_p_channels, kernel_size=1) for channels in [256, 512, 1024, 2048]
            ])
        ...
        if self.self_attention:
            ...
            self.att_weight = nn.Parameter(torch.ones(1).mul_(0.1), requires_grad=True)
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        ...
        for block in self.layers:
            ...
            if self.encode_frequency:
                freq_encoding = torch.sin(torch.Tensor(np.arange(x.size(2)) * np.pi / (12 * x.size(2)))).view(1, 1, x.size(2), 1)\
                    .repeat(x.size(0), 1, 1, x.size(3)).to(device)
...
def resnet50(pretrained: bool = False, pretrained_path: str = None, progress: bool = True, **kwargs: Any) -> ResNet:
    ...
    Args:
        ...
        progress (bool): If True, displays a progress bar of the download to stderr
    """
    return _resnet('resnet50', pretrained_path, Bottleneck, [3, 4, 6, 3], pretrained, progress,
                   **kwargs)