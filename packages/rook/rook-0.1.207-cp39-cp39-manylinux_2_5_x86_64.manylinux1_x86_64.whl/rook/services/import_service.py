...
class ImportService(object):
        ...
        with CountingImportLock():
            ...
            for module_key, module_object in self._modules.items():
                ...
                if filename == os.path.basename(module_filename):
                    file_hash = self._get_file_hash(module_object, module_filename)
                    if file_hash:
    ...
    def _get_file_hash(self, module, module_filename):
        result = self._file_hash_cache.get(module_filename, None)
        if result is not None:
            return result
        ...
        if file_content:
            file_hash = hashlib.sha256(file_content).hexdigest()
            self._file_hash_cache[module_filename] = file_hash
            return file_hash
