...
def protes_dev(f, n, m, k=50, k_top=5, k_gd=100, lr=1.E-4, r=5, seed=42, is_max=False, log=False, log_ind=False, info={}, P=None, with_info_i_opt_list=False, with_info_full=False):
    time = tpc()
    ...
    @jax.jit
    def loss(P_cur, I_cur, I2):
        l = likelihood(P_cur, I_cur)
        l2 = likelihood(P_cur, I2)
        return jnp.mean(-l) - jnp.mean(0.5 * l2)

    loss_grad = jax.grad(loss)

    @jax.jit
    def optimize(state, P_cur, I_cur, I2):
        grads = loss_grad(P_cur, I_cur, I2)
    ...
    while True:
        ...
        ind2 = jnp.argsort(y, kind='stable')
        ind2 = (ind2[::-1] if is_max else ind2)[k_top:]

        for _ in range(k_gd):
            state, P = optimize(state, P, I[ind, :], I[ind2, :])
