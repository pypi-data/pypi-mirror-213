...
class _SubProcessContainer(object):
    ...
    def remove(self, pid):
        with _thread_lock, suppress(KeyError):
...
def execute_anybodycon(
    ...
    try:
        macrofile_path = folder / (Path(logfile.name).stem + ".anymcr")
...
class AnyPyProcess(object):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not (ignore_errors is None or isinstance(ignore_errors, list)):
            raise ValueError("ignore_errors must be a list of strings")

        if not (warnings_to_include is None or isinstance(warnings_to_include, list)):
    ...
    def _worker(self, task, task_queue):
        ...
        try:
            ...
            if not self.keep_logfiles and not task.has_error():
                Path(logfile.name).unlink(missing_ok=True)
                task.logfile = ""
            task_queue.put(task)
        ...
        for task in tasklist:
            try:
                if not self.keep_logfiles:
                    if not task.has_error() or task.retcode == _KILLED_BY_ANYPYTOOLS:
                        Path(task.logfile).unlink(missing_ok=True)
                        task.logfile = ""
            except OSError as e:
            ...
            if not self.keep_logfiles and task.logfile:
                try:
                    ...
                    macrofile.unlink(missing_ok=True)
                except OSError as e:
