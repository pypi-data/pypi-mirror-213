...
class OsRepository:
    ...
    def __init__(self):
        try:
            ...
            raise RuntimeError("Please use SSH connection for deployment")
    ...
    def _run_nexus(self):
        ...
        run_cmd = (
            f"docker run -d --name nexus -p {self.nexus_run_ip}:{self.NEXUS_RUN_PORT}:8081 "
...
class YumRepository(OsRepository):
    ...
    def upload_rpm(self):
        ...
        for i in download_os_list:
            if os.path.basename(i) not in self.config_content["rpm_os"]:
                continue
