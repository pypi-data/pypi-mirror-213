...
class InlineHtmlEditNode(template.Node):
    ...
    def _render_value(self, context, obj_lookup, value):
        ...
        if (not self.read_only) and context.get('inline_html_edit'):
...
class InlineHtmlMultipleEditNode(InlineHtmlEditNode):
    """base class when working with multiple objects to edit"""

    def __init__(self, *args, **kwargs):
        super(InlineHtmlMultipleEditNode, self).__init__(*args, **kwargs)
    ...
    for lookup_item in lookups:
        try:
            ...
            raise template.TemplateSyntaxError(
                u"inline_html_edit {0} is an invalid lookup. It should be key=value".format(lookup_item)
            )
        lookup[key] = value
    
    if not lookup:
        raise template.TemplateSyntaxError(u"The inline_html_edit templatetag requires at least one lookup")
