...
class FermiHandler:
    ...
    def plot_fermi_surface(self, 
        ...
        if options_dict['scalars'] ==  "spin_magnitude" or options_dict['scalars'] ==  "Fermi Velocity Vector_magnitude":
            ...
            if arrow_color is None:
                plotter.add_mesh(arrows,scalars=options_dict['scalars'], cmap=cmap, show_scalar_bar=False)
            else:
    ...
    def create_isovalue_gif(self, 
        ...
        # print(arrows.active_scalars_info)
        # print(arrows.active_vectors_info)
        ...
        for e_surface,evalues in zip(e_surfaces,energy_values):
            ...
            surface.set_active_scalars(name=options_dict['scalars'])
            ...
            if options_dict['scalars'] ==  "spin_magnitude" or options_dict['scalars'] ==  "Fermi Velocity Vector_magnitude":
                ...
                arrows.set_active_scalars(name=options_dict['scalars'])
                # arrows.set_active_vectors(name=options_dict['vector_name'])

                # print(arrows.point_data.keys())
                # print(arrows.active_scalars_info)
                # print(arrows.active_vectors_info)
        ...
        for e_surface, evalues in zip(e_surfaces[::-1],energy_values[::-1]):
            ...
            if options_dict['scalars'] ==  "spin_magnitude" or options_dict['scalars'] ==  "Fermi Velocity Vector_magnitude":
                ...
                arrows.set_active_scalars(name=options_dict['scalars'])
                # arrows.set_active_vectors(name=options_dict['vectors'])
    ...
    def plot_fermi_cross_section(self,
        ...
        """A method to plot the fermi sruafce with a cross section slicing widget
        ...
        tot_surface= fermi_surface
        if transparent_mesh:
            plotter.add_mesh(tot_surface,color='white',opacity=0.1)
    ...
    def plot_fermi_cross_section_box_widget(self,
        ...
        """A method to plot the fermi sruafce with a cross section and box slicing widget
...
def add_custom_mesh_slice(
        ...
        plotter.add_plane_widget(callback=callback_plane, bounds=mesh.bounds,
        ...
        plane_origin = plotter.plane_widgets[0].GetOrigin()
        plane_normal = plotter.plane_widgets[0].GetNormal()
        callback_plane(normal=plane_normal, origin=plane_origin)
        return actor

def add_custom_box_slice_widget(
                    plotter,
                    mesh, 
                    options_dict:dict,
                    show_cross_section_area:bool=False,
        ...
        plotter.add_mesh(mesh.outline(), name=f"{name}-outline", opacity=0.0)
        ...
        plotter.box_clipped_meshes.append(box_clipped_mesh)
        ...
        clipped_box_mesh =  plotter.box_clipped_meshes[0]
        ...
        plotter.add_mesh(clipped_box_mesh.outline(), name=name+"outline", opacity=0.0, line_width=line_width,show_scalar_bar=False, rgba=options_dict['use_rgba'])
