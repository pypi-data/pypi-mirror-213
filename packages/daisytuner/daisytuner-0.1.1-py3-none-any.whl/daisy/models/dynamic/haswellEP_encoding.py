...
class HaswellEPEncoding(DynamicEncoding):
    def __init__(self, map_nest: MapNest, hostname: str) -> None:
        super().__init__(map_nest=map_nest, hostname=hostname, arch="haswellEP")
    ...
    def _vectorize(self, data: pd.DataFrame) -> np.ndarray:
        ...
        ] = DynamicEncoding._normalize(data, "MEM_UOPS_RETIRED_LOADS")
        encoding[
            15 * num_statistics : 16 * num_statistics
        ] = DynamicEncoding._normalize(data, "MEM_UOPS_RETIRED_STORES")
