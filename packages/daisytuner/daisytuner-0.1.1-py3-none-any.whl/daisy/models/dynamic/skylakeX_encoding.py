...
class SkylakeXEncoding(DynamicEncoding):
    def __init__(self, map_nest: MapNest, hostname: str) -> None:
        super().__init__(map_nest=map_nest, hostname=hostname, arch="skylakeX")
    ...
    def _vectorize(self, data: pd.DataFrame) -> np.ndarray:
        ...
        encoding[1 * num_statistics : 2 * num_statistics] = DynamicEncoding._normalize(
            data, "FP_ARITH_INST_RETIRED_SCALAR_SINGLE"
        )
        ...
        encoding[4 * num_statistics : 5 * num_statistics] = DynamicEncoding._normalize(
            data,
        ...
        encoding[5 * num_statistics : 6 * num_statistics] = DynamicEncoding._normalize(
            data, "FP_ARITH_INST_RETIRED_SCALAR_DOUBLE"
        )
        ...
        encoding[8 * num_statistics : 9 * num_statistics] = DynamicEncoding._normalize(
            data,
        ...
        ] = DynamicEncoding._normalize(data, "MEM_INST_RETIRED_ALL_LOADS")
        encoding[
            15 * num_statistics : 16 * num_statistics
        ] = DynamicEncoding._normalize(data, "MEM_INST_RETIRED_ALL_STORES")
