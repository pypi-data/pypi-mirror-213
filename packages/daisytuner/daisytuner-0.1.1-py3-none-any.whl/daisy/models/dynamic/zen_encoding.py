...
class ZenEncoding(DynamicEncoding):
    def __init__(self, map_nest: MapNest, hostname: str) -> None:
        super().__init__(map_nest=map_nest, hostname=hostname, arch="zen")
    ...
    def _vectorize(self, data: pd.DataFrame) -> np.ndarray:
        ...
        ] = DynamicEncoding._normalize(data, "L3_ACCESS")

        # L2 Volume
        encoding[
            13 * num_statistics : 14 * num_statistics
        ] = DynamicEncoding._normalize(data, "REQUESTS_TO_L2_GRP1_ALL_NO_PF")

        # DRAM Controller
        encoding[
            14 * num_statistics : 15 * num_statistics
        ] = DynamicEncoding._normalize(data, "LS_DISPATCH_LOADS")
        encoding[
            15 * num_statistics : 16 * num_statistics
        ] = DynamicEncoding._normalize(data, "LS_DISPATCH_STORES")
