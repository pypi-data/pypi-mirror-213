...
def watch(
    ...
    if isinstance(setfit, SetFitTrainer):
        if validate_before_training:
            ...
            dq_helper_data = copy.deepcopy(logger_config.helper_data)
        setfitmanager = PatchManager()
        ...
        if validate_before_training:
            ...
            with contextlib.redirect_stderr(f_err), contextlib.redirect_stdout(f_out):
                ...
                setfit.train_dataset = sample_dataset(
                    setfit.train_dataset, num_samples=2
                )
                ...
                dq.get_data_logger().logger_config.helper_data = dq_helper_data
                patched_trainer = _PatchSetFitTrainer(
                    setfit,
                    labels=labels,
                    finish=finish,
