...
class NovumApiError(Exception):
    def __init__(self, message: str, status_code: int):
        ...
        super().__init__(f"NovumApiError: {message} (status: {status_code})")
...
def full_name(user: dict) -> str:
    if len(user.keys()) > 0:
        ...
        raise NovumApiError("User not found!", 400)
...
class BaseAPIClient:
    ...
    def _get_json(self, url: str, filter_json=None, option=None, timeout: float = 4.0):
        ...
        if response.status_code == requests.codes.get("ok"):
            ...
            raise NovumApiError("Unable to get data.", response.status_code)
    ...
    def _post_json(self, url: str, data=None, filter_json=None, option=None, timeout: float = 4):
        ...
        if response.status_code == requests.codes.get("ok"):
            ...
            raise NovumApiError("The data could not be posted", response.status_code)
    ...
    def _put_json(self, url: str, data=None, filter_json=None, option=None, timeout: float = 4):
        ...
        if response.status_code == requests.codes.get("ok"):
            ...
            raise NovumApiError("The data was not updated", response.status_code)
    ...
    def _delete_json(self, url: str, filter_json=None, option=None, timeout: float = 4.0):
        ...
        if response.status_code <= 204:
            ...
            raise NovumApiError("The data was not removed", response.status_code)
