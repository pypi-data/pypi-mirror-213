...
@pytest.fixture
def basic_dev_output_task(
    recipe_run_id, recipe_run_configuration, dev_output_task, tmp_path, mocker
):
    mocker.patch(
...
def test_format_object_key(basic_dev_output_task, custom_dir_name):
    """
    ...
    """
    task, proposal_id = basic_dev_output_task
...
def test_construct_intermediate_name_from_tags(basic_dev_output_task):
    """
    Given: A base task from TransferDevDataBase
    When: Formatting a sensible name for an INTERMEDIATE tag
    Then: The correct name is returned
    """
    task, _ = basic_dev_output_task
...
@pytest.mark.parametrize("custom_dir_name", [pytest.param(None, id="Default dev dir name")])
def test_construct_generic_name_from_tags(basic_dev_output_task):
    """
    Given: A base task from TransferDevDataBase
...
def test_build_transfer_list(complete_dev_output_task, dummy_globus_transfer_item):
    ...
    """
    task, *_ = complete_dev_output_task
...
@pytest.mark.parametrize("custom_dir_name", [pytest.param("foo", id="Default dev dir name")])
def test_build_debug_transfer_list(complete_dev_output_task, destination_bucket, custom_dir_name):
    """
...
def test_build_intermediate_transfer_list(complete_dev_output_task, destination_bucket):
    ...
    Then: The resulting transfer list has the correct source and destination paths and references the correct frames
    """
    task, proposal_id, _, intermediate_file_obj, _ = complete_dev_output_task
...
@pytest.mark.parametrize("custom_dir_name", [pytest.param(None, id="Default dev dir name")])
def test_build_transfer_list_from_tag_list(complete_dev_output_task, destination_bucket):
    """
    Given: A Task based on TransferDevDataBase with tagged frames
...
def test_build_transfer_list_from_tag_list_single_list(
    complete_dev_output_task, destination_bucket
):
    """
    Given: A Task based on TransferDevDataBase with tagged frames
    ...
    Then: The transfer list is built with correct source and destination paths
    """
    task, _, debug_file_obj, *_ = complete_dev_output_task
