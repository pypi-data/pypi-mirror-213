...
class CNN(nn.Module):
    ...
    def __init__(self, input_dim, output_dim, fc_layers = [], conv_layers = [], stride = 1, device="cpu", dropout=0.1, inchannels = 1):
        ...
        self.conv_network, self.lin_network = self.create_network("o", self.input_dim, self.output_dim, fc_layers = self.fc_layers, conv_layers = self.conv_layers)


    def create_network(self, name, input_dim, output_dim, append_dim=0, fc_layers=[], conv_layers=[]):
