...
def my_Func2(params,input_data):
    a = params[0,0]
    b = params[1,0]
    c= params[2,0]
    y=[]
    for each in input_data:
        y.append((a+b*each)/(c-each))
    return np.array(y)
...
#     mid,sigma = 0,5
#     y = my_Func2(params,x) + np.random.normal(mid, sigma, num_data).reshape(num_data,1)
#     return x,y

def generate_data2(params,num_data):
    x=np.array([0.0395,
    0.0439,
    0.0474,
    0.0523,
...
def cal_deriv(params,input_data,param_index):
    ...
    data_est_output1 = my_Func2(params1,input_data)
    data_est_output2 = my_Func2(params2,input_data)
...
def cal_residual(params,input_data,output_data):
    data_est_output = my_Func2(params,input_data)
...
def main():
    ...
    data_input,data_output = generate_data2(params,num_data)#generate data as requested
    ...
    x = data_input #生成0-10的共100个数据，然后设置间距为0.1
    plt.plot(x,(a_est+b_est*x)/(c_est-x),'r',lw=1.0)
