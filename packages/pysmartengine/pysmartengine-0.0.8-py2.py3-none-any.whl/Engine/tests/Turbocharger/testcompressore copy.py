...
T=Turbocharger.gasturb2commap("LPC01.MAP")

T.table[0]*=90000
# T.show()

map=Turbocharger.Compressor.CompressorMap("compre").init(T,1.e5,300,0,1)
...
for i in np.linspace(min(map.keys),max(map.keys)):
    t=map.getline(i)
    xx=np.linspace(min(t.table[2].data),max(t.table[2].data)*1.2)
    yy=[]
    for each in xx:
...
for i in np.linspace(min(map.keys),max(map.keys)):
    t=map.getline(i)
    xx=np.linspace(min(t.table[2].data),max(t.table[2].data)*0.999)
    yy=[];zz=[]
    for each in xx:
