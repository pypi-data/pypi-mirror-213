        ...
        if message["object"] == "page":
            for k, value in message["properties"].items():
                ...
                _message = dict(
                    ...
                    type=value["type"],
                    content=handlers[value["type"]](value),
                )
            ...
            if message["type"] in self.accepted_blocks:
                _message = dict(
                    ...
                    parent=parent
                    or {
                        "id": message["parent"][message["parent"]["type"]],
                        "type": message["parent"]["type"],
                    },
                    type=message["type"],
                    content=handlers[message["type"]](message),
...
def file_handler(data):
    ...
    if not s3.exists(key):
        s3.data = SimpleS3Data(_object.content, file_key=key, mime=_object.mime)
        s3.put()
...
def files_handler(data):
    ...
    for ind, file in enumerate(data["files"]):
        ...
        if not s3.exists(key):
            s3.data = SimpleS3Data(_file.content, file_key=key, mime=_file.mime)
            s3.put()
        result.append(key)
...
def table_row_handler(data):
    ...
    for cell in data["table_row"]["cells"]:
        result.append(handlers["text"](cell))
    return result
...
handlers["text"] = lambda data: "".join([text["text"]["content"] for text in data])
