...
class Command(BaseCommand):
    ...
    def update_device_sni_status_data(self, device):
        ...
        if covered:
            ...
            api_status = "API user not associated with contract and device (No authorization to most of the API information)"
    ...
    def update_device_sni_summary_data(self, device, pid, eox_data):
        ...
        # Update the device pid as model
        ds.model = pid
        self.stdout.write(f"{device['sr_no']} - model: {pid}")

        print(json.dumps(eox_data, indent=4))
        try:
        ...
        if eox_data["eox_announcement_date"]:
        ...
        if eox_data["end_of_security_vul_support_date"]:
        ...
        if eox_data["end_of_routine_failure_analysis_date"]:
        ...
        if eox_data["end_of_service_contract_renewal"]:
        ...
        if eox_data["last_date_of_support"]:
        ...
        if eox_data["end_of_svc_attach_date"]:
