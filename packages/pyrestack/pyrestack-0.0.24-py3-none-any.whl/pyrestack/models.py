...
@dataclass
class ReStackApiResponse(ReStackBaseModel):
    """API response model for ReStack."""

    _method: str | None = None
    _api_path: str | None = None
    data: list[ReStackStack] | None = None

    @staticmethod
    def from_prometheus(data: dict[str, Any]) -> ReStackApiResponse:
        """Generate object from json."""
        obj: dict[str, Any] = {}
        stacks = []

        for key, value in data.items():
            if hasattr(ReStackApiResponse, key):
                obj[key] = value
