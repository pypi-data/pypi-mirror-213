...
def create_merge_request(target: Path,
    ...
    if hasattr(project, 'merge_requests_template'):
        ...
        if mr_template:
            ...
            try:
                spinner.stop()
                description = Utils.edit_mr_description(description, confirm, preview)
                click.secho("Done editing MR", fg='green')
                spinner.start()
            except IOError:
                click.secho(f"Failed to edit MR description, using defaults instead", fg='red')

    merge_requests_for_issue: [ProjectMergeRequest] = project.mergerequests.list(search=issue_id)

    # filter opened and locked MRs
    merge_requests_for_issue = [mr for mr in merge_requests_for_issue if mr.state not in ['merged', 'locked', 'closed']]
