...
from lollms.console import MainMenu, BindingBuilder, ModelBuilder
...
class LoLLMsServer:
    ...
    def run(self, host="localhost", port="9600"):
        ...
        if cfg_path.exists():
            self.config = LOLLMSConfig(cfg_path,self.lollms_paths)
        else:
        ...
        for p in self.config.personalities:
            personality = AIPersonality(self.config.lollms_paths.personalities_zoo_path/p, self.current_model)
            self.personalities.append(personality)
