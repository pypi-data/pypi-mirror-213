...
class LoLLMsServer:
    ...
    def load_personality(self):
        if len(self.config["personalities"][self.config["default_personality_id"]].split("/"))==3:
            self.personality = AIPersonality(lollms_personalities_zoo_path / self.config["personalities"][self.config["default_personality_id"]], self.model)
        else:
    ...
    def run(self, host="localhost", port="9600"):
        ...
        self.active_personality = self.personalities[self.config.default_personality_id]
