        ...
        if prompt.strip().lower()=="send_file":
            ...
            for i,chunk in enumerate(self.text_store.paragraphs):
                if len(chunk.split())<50:
                    print(chunk)
                    continue
                docs = '### Instruction: Summarize the following paragraph in the form of bullet points. Be concise and use a single idea for each bullet point.\nParagraph:'+chunk+"\nBullet points:\n"
                ASCIIColors.error("-------------- Documentation -----------------------")
                ASCIIColors.error(docs)
                ASCIIColors.error("----------------------------------------------------")
                print("Thinking")
                output = self.generate(docs, self.config["max_answer_size"])
