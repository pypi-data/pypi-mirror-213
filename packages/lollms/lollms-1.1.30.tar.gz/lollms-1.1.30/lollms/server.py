...
from lollms.console import ASCIIColors, MainMenu
from lollms.paths import lollms_personal_configuration_path, lollms_bindings_zoo_path, lollms_personalities_zoo_path, lollms_default_cfg_path, lollms_personal_models_path
from lollms.console import ASCIIColors, MainMenu, BindingBuilder, ModelBuilder
...
class LoLLMsServer:
    ...
    def load_model(self):
        if not self.config.check_model_existance():
            ...
            try:
                ...
                print(ex)
                print(f"Couldn't load model. Please verify your configuration file or use the next menu to select a valid model")
                self.menu.select_model()
    ...
    def run(self, host="localhost", port="9600"):
        ...
        parser.add_argument('--bindings_path', '-bp', default=str(lollms_bindings_zoo_path),
                            help='The path to the Bindings folder')
        parser.add_argument('--personalities_path', '-pp',
                            default=str(lollms_personalities_zoo_path),
                            help='The path to the personalities folder')
        parser.add_argument('--models_path', '-mp', default=str(lollms_personal_models_path),
