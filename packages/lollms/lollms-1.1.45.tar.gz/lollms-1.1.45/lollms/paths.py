...
class LollmsPaths:
    ...
    @staticmethod
    def find_paths(force_local=False):
        lollms_path = Path(__file__).parent
        ...
        if global_paths_cfg.exists():
            try:
                ...
                return LollmsPaths(lollms_path, lollms_personal_path)
            ...
            if global_paths_cfg.exists():
                ...
                return LollmsPaths(cfg.lollms_path, cfg.lollms_personal_path)
    ...
    def reset_configs():
        ...
        global_paths_cfg = Path("./global_paths_cfg.yaml")
        if global_paths_cfg.exists():
            ASCIIColors.print(ASCIIColors.color_red,"Resetting local settings")
            global_paths_cfg.unlink()
            return
        global_paths_cfg = lollms_path/"global_paths_cfg.yaml"
        if global_paths_cfg.exists():
            ASCIIColors.print(ASCIIColors.color_red,"Resetting global settings")
            global_paths_cfg.unlink()
