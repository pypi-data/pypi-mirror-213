...
class LoLLMsServer:
    ...
    def load_personality(self):
        if len(self.config["personalities"][self.config["active_personality_id"]].split("/"))==3:
            self.personality = AIPersonality(self.lollms_paths, self.lollms_paths.personalities_zoo_path / self.config["personalities"][self.config["active_personality_id"]], self.model)
        else:
            self.personality = AIPersonality(self.lollms_paths, self.config["personalities"][self.config["active_personality_id"]], self.model, is_relative_path=False)
    ...
    def initialize_routes(self):
        ...
        def handle_connect():
            ...
            ASCIIColors.print(ASCIIColors.color_green, f'Client connected with session ID: {client_id}')
