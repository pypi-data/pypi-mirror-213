from lollms import AIPersonality, lollms_path, lollms_default_cfg_path, lollms_personalities_zoo_path, lollms_bindings_zoo_path, lollms_personal_models_path, lollms_personal_configuration_path,  MSG_TYPE
...
class MainMenu:
    ...
    def select_binding(self):
        ...
        for p in lollms_bindings_zoo_path.iterdir():
            if p.is_dir():
    ...
    def select_model(self):
        ...
        models_dir:Path = (lollms_personal_models_path/self.conversation.config['binding_name'])
    ...
    def select_personality(self):
        ...
        personality_languages = [p.stem for p in lollms_personalities_zoo_path.iterdir() if p.is_dir()] + ["Back"]
        ...
        if 1 <= choice <= len(personality_languages)-1:
            ...
            personality_categories = [p.stem for p in (lollms_personalities_zoo_path/language).iterdir() if p.is_dir()]+["Back"]
            ...
            if 1 <= choice <= len(personality_categories):
                ...
                personality_names = [p.stem for p in (lollms_personalities_zoo_path/language/category).iterdir() if p.is_dir()]+["Back"]
...
class Conversation:
    def __init__(
        ...
        original = lollms_default_cfg_path
        if configuration_path is None:
            local = lollms_personal_configuration_path / "local_config.yaml"        
        else:
...
"""
    ...
    def load_binding(self):
        if self.config.binding_name is None:
            ...
            try:
                self.binding_class = BindingBuilder().build_binding(lollms_bindings_zoo_path, self.config)
            except Exception as ex:
                print(ex)
    ...
    def load_personality(self):
        if len(self.config["personalities"][self.config["active_personality_id"]].split("/"))==3:
            self.personality = AIPersonality(lollms_personalities_zoo_path / self.config["personalities"][self.config["active_personality_id"]], self.model)
        else:
            self.personality = AIPersonality(self.config["personalities"][self.config["active_personality_id"]], self.model, is_relative_path=False)
