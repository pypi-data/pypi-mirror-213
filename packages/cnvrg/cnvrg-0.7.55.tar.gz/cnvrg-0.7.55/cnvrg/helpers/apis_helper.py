...
def __get_headers():
    return {"Auth-Token": credentials.token, "Authorization": credentials.get_auth_header()}
...
def send_file(url, data=None, files=None):
    ...
    verify_logged_in()
    get_url = url_join(base_url, url)
    resp = requests.post(get_url, files=files, data=data, headers={"AUTH-TOKEN": credentials.token, "Authorization": credentials.get_auth_header()}, verify=False)
