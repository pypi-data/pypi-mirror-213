"""Similiarity metrics for in-domain subcorpus selection
Code adapted from: 
github: https://github.com/georgian-io/Transformers-Domain-Adaptation.
paper: https://github.com/sebastianruder/learn-to-select-data.
"""

from typing import Callable, Dict, NewType, Sequence
...
def jensen_shannon_similarity(repr1: np.ndarray, repr2: np.ndarray) -> np.ndarray:
    ...
    # the similarity is -inf if no term is in the vocabulary
...
def renyi_similarity(
    ...
    ref: https://en.wikipedia.org/wiki/R%C3%A9nyi_entropy#R.C3.A9nyi_divergence
    """
...
def bhattacharyya_similarity(repr1: np.ndarray, repr2: np.ndarray) -> np.ndarray:
    ...
    ref: https://en.wikipedia.org/wiki/Bhattacharyya_distance
    """
    distance = -np.log(np.sqrt(repr1 * repr2).sum(axis=-1))
    assert not np.isnan(distance).any(), "Error: Similarity is nan."

    # the distance is -inf if no term is in the vocabulary
...
def similarity_metrics_factory(metric: SimilarityMetric) -> SimilarityFunc:
