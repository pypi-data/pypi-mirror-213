...
def monthly_returns(returns, eoy=True):
    ...
    # Extract Year and Month from the multi-index and create respective columns
    monthly_returns["Year"] = monthly_returns.index.get_level_values(0)

    # Convert integer month to its string abbreviation
    months = [
        "Jan",
        "Feb",
        "Mar",
    ...
    monthly_returns["Month"] = monthly_returns.index.get_level_values(1).map(
        lambda x: months[x - 1]
    )
    ...
    pivot_table = pivot_table.loc[
        :,
        [(asset, month) for asset in pivot_table.columns.levels[0] for month in months],
    ]
    ...
    if eoy:
        ...
        for asset in yearly_returns.columns:
            pivot_table[asset, "EOY"] = yearly_returns[asset].values
...
def best(returns, aggregate=None, compounded=True):
    """
    Returns the date and the highest return for each asset over a specified period.

    Args:
        returns (pd.DataFrame or pd.Series): Asset returns.
        aggregate (str, optional): The period over which to aggregate returns. Can be 'day', 'month', 'week', 'quarter', or 'year'.
        compounded (bool, optional): If True, calculates compounded returns. Defaults to True.

    Returns:
        pd.DataFrame: A DataFrame with asset names as columns, the highest return for each asset, and corresponding date as index.
    """
    # Aggregate returns over the specified period, then find the maximum value
    aggregated_returns = utils.aggregate_returns(returns, aggregate, compounded)
    max_returns = aggregated_returns.max()
    max_return_dates = aggregated_returns.idxmax()
    return pd.DataFrame({"Max Return": max_returns, "Date": max_return_dates})


def worst(returns, aggregate=None, compounded=True):
    """
    Returns the date and the lowest return for each asset over a specified period.
    ...
    Returns:
        pd.DataFrame: A DataFrame with asset names as columns, the lowest return for each asset, and corresponding date as index.
    """
    # Aggregate returns over the specified period, then find the minimum value
    aggregated_returns = utils.aggregate_returns(returns, aggregate, compounded)
    min_returns = aggregated_returns.min()
    min_return_dates = aggregated_returns.idxmin()
    return pd.DataFrame({"Min Return": min_returns, "Date": min_return_dates})
