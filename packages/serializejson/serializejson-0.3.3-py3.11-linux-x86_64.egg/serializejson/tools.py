...
# encoding -------------
# pickle plugins (used by serializejson if not serializejson plugin or methode)
dispatch_table
serializejson_ = {}  # serializejson plugins
# serializejson plugins actives even if strict_pickle is True
serializejson_builtins = {}
# encoder extra parameters for plugins, with their default value
encoder_parameters = {}
...
def getstate(
    ...
    *,
    ...
    Args:
        ...
        properties :
                Whether properties will be saved.
        ...
        getters :
                Whether values from getters will be saved.
    ...
    if (properties is True) or (getters is True):
        (
            ...
            class_getters,
            _,
        ) = slots_properties_getters_setters_from_class(type(self))
    ...
    if split_dict_slots and (has_slots or properties or getters):
        # ATTENTION PAS LE CAS SI PAS DE __SLOT__ ?
        state_dict = dict()
        state_slots = dict()
    ...
    if keep is not None:
        for key in keep:
            if key in getters:
                ...
                # on commenc par voir si existe sous forme de propriété
                ...
                if type(key) is tuple:
                    ...
                    for k, value in zip(key, result):
                        if (
                            k is not None
                            and k not in remove
                            and not (remove_types and (type(value) in remove_types))
                        ):
                            state_slots[k] = value
                ...
                for key in slots:
                    if (
                        ...
                        and not (
                            ...
                            and (
                                key in state_slots
                                or (key.startswith("_") and key[1:] in state_slots)
                            )
                        )
                    ):
                        value = _getattribute(key)
    ...
    if splited_dict_slots and state_slots:
        if sort_keys:
            state_dict = sorted_dict(state_dict)
        if state_dict:
            # peut planter si self n'a pas de __dict__ et  pas de __setstate__
            return state_dict, state_slots
        if state_slots:
...
def setstate(
    ...
    if not type(state) is dict:
        raise Exception(
            "try to restore object to a no dictionary state and without __setstate__ method"
        )

    if restore_default_values:
        # est-ce utile si pas d'update et qu'on vient de recrer l'objet  ?
        if default_values is None:
            default_values = default_state_from_class(type(self))
        for key, default_value in default_values.items():
            if key not in state:
    ...
    if order is None:
        ...
        for attribut in order:
            ...
            if attribut in setattr_:
                ...
                if attributs:
                    ...
                    for k in attributs:
                        if k == attribut:
                            # attribut as already be poped , state[k] doesn't exciste anymore
                            values.append(value)
                        else:
                            values.append(state[k])
                        attribut_to_multi_attributs[attribut] = False
        ...
        if state:
            raise Exception(
                f"{list(state.keys())} are not in __setstate__ order parameter"
            )
    # else :
    #    sentinel = []
    #    for key in slots :
...
@cached_one_arg_func
def import_str_from_class_str(class_str):
    if class_str in constructors and class_str.find(".") == -1:
        # permet de gerer le cas numpyB64
        class_ = constructors[class_str]
        if class_.__name__ == class_str:
            return f"from {class_.__module__} import {class_str}"
        else:
            return f"from {class_.__module__} import {class_.__name__} as {class_str}"
    else:
        module_str = class_str
        stop = module_str.rfind(".")
        while stop != -1:
            module_str = module_str[:stop]
            if module_str in sys.modules:
                true_module_str = sys.modules[module_str].__name__
                if module_str == true_module_str:
                    return f"import {module_str}"
                else:
                    parent_module, sub_module = true_module_str.rsplit(".", 1)
                    if module_str == sub_module:
                        return f"from {parent_module} import {module_str}"
                    else:
                        return (
                            f"from {parent_module} import {sub_module} as {module_str}"
                        )
            stop = module_str.rfind(".")
        # print(class_str)
...
def tuple_from_reduce(func, args, state=None, listitems=None, dictitems=None, obj=None):
    ...
    if func is __newobj__ or func is __newobj_ex__:
        ...
        if func is __newobj__:
            ...
            if len(args) > 1:
                # le reduce n'a pas été réimplémenté , on doit pas utiliser d'initArgs
                newArgs = args[1:]
            ...
            if serialize_parameters.strict_pickle:
                if not hasattr(class_, "__new__"):
                    raise PicklingError(
                        "args[0] from {} args has no __new__".format(
        ...
        if not serialize_parameters.strict_pickle:
            ...
            if type(state) is tuple:
                ...
                if not class_has_method(class_, "__getstate__") or not class_has_method(
                    ...
                    if __dict__:
                        # fusionne slots et __dict__ dans un seul dictionnaire
...
def reduce(obj, protocol):
    ...
    if reduce_func is not None:
        ...
        if reduce_func is not None:
            # try :
            # fait appel à __gestate__() si on n'a pas réimplementé __reduce__()
            reduced = reduce_func(protocol)
            # except :
            ...
            if reduce_func is not None:
                ...
                raise PicklingError(
                    "Can't pickle %r object: %r" % (type(obj).__name__, obj)
                )

    if isinstance(reduced, str):
        raise ValueError(
            "{} reduce's methode return a string. It is not yet suported by serializejson"
        )  # A REVOIR !!!
    ...
    if serialize_parameters.strict_pickle:
        ...
        if not (2 <= len(reduced) <= 5):
            raise PicklingError(
                "Tuple returned by %s must have " "two to five elements" % reduce_func
            )
...
def generic__reduce_ex__(self, protocol=4):
    if (
        hasattr(self, "__reduce_ex__")
        and self.__class__.__reduce_ex__ is not object.__reduce_ex__
    ):
        return self.__reduce_ex__(protocol)
    if (
        hasattr(self, "__reduce__")
        and self.__class__.__reduce__ is not object.__reduce__
    ):
    ...
    if hasattr(self, "__getnewargs_ex__"):
        ...
        return (
            __newobj_ex__,
            (self.__class__, new_args, kwargs),
            state,
            listitems,
            dictitems,
        )
    ...
    **argsSup,
    ...
    if type(__class__) is str:
        ...
        try:
            ...
            class_ = class_from_class_str_dict[__class__] = class_from_class_str(
                __class__
            )
    else:
    ...
    if inst is None:
        ...
        if __new__ is not None or __init__ is None:
            ...
            if __new__type in (list, tuple):
                ...
                # when braces have been removed during serialization
                inst = class_.__new__(class_, __new__)
            else:
    ...
    if accept_dict_as_object and type(obj) is dict :#and "__class__" in obj:
        ...
        try:
            ...
            for methode in [
                f"get{attribut}",
                f"get_{attribut}",
                f"get{attribut[0].upper()}{attribut[1:]}",
            ]:
...
def _get_getters(extra_getters):
    if isinstance(extra_getters, bool):
        ...
        if isinstance(extra_getters, (list, set, tuple)):
            ...
            raise TypeError(
                "Encoder getters argument must be None, bool, list, tuple, set or dict, not '%s'"
                % type(extra_getters)
            )
        return getters_
...
def _get_setters(extra_setters):
    if isinstance(extra_setters, bool):
        ...
        if isinstance(extra_setters, (list, set, tuple)):
            ...
            raise TypeError(
                "Decoder setters argument must be None, bool, list, tuple, set or dict, not '%s'"
                % type(extra_setters)
            )
        return setters_
...
# --- Import of plugins -------------------------------------------------------
from . import plugins  # le laisser à la fin ! needed for plugin regord
