...
class DjangoJobStore(DjangoResultStoreMixin, BaseJobStore):
    ...
    def get_next_run_time(self):
        try:
            job = DjangoJob.objects.filter(next_run_time__isnull=False, is_delete=False).earliest(
                "next_run_time"
            )
    ...
    def _get_jobs(self, **filters):
        ...
        job_states = DjangoJob.objects.filter(**filters, is_delete=False).values_list("id", "job_state")
