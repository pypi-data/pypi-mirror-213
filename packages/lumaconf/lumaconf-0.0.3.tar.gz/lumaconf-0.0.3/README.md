...
With github SSH private key set up, you can install as follows

```sh
pip install git+ssh://git@github.com/lumalabs/config.git
```
...
# Load configs/base.conf
conf = lumaconf.load()

# Load some other config
conf2 = lumaconf.load("configs/myconf.conf")
```

New features
- Cleaner loading interface
- Wrapper with config override feature (pass `override=<dict>` or `override="<json string>"` to load())
- Math support `${train.num_epochs+1}`, `${math.sin(math.pi)}`

## HOCON Specs

https://github.com/typesafehub/config/blob/master/HOCON.md

## Usage (original pyhocon)
...
```python
from lumaconf import ConfigFactory
