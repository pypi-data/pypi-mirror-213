...
class ConfigParser(object):
    ...
    @classmethod
    def _recursive_resolve_config_value(cls, token, config: ConfigTree, max_retries: int = 15):
        if isinstance(token, ConfigValues):
            for _ in range(max_retries):
                subs = token.get_substitutions()
                for sub in subs:
                    success, rv = cls._resolve_variable(config, sub)
                    if not success:
                        # Failed to resolve
                        print('Failed to resolve variable: ', sub)
                        return None
                    cls._do_substitute(sub, rv)
                token = token.transform()
                if not isinstance(token, ConfigValues):
                    break
        return token

    @classmethod
    def _safe_eval_python_expr(cls, expr: str, config: ConfigTree):
        """
        ...
        Returns none if not able to resolve all.
        """
        ...
        for i, c in enumerate(expr):
            if c in main_chars:
                ...
                if curr_token:
                    if has_char:
                        try:
                            get_token = config.get(curr_token)
                            if get_token is None:
                                return None
                            get_token = cls._recursive_resolve_config_value(get_token, config)
                            if isinstance(get_token, ConfigValues):
                                print('Warning: Unresolvable ConfigValues found in expression: ' + expr)
                                return None
                            out_expr += str(get_token)
        ...
        if out_expr.startswith('[') or out_expr.startswith('{'):
            return None
    ...
    def _eval_or_get(cls, expr: str, config: ConfigTree):
        expr = expr.strip()
        if (any(x in '+-*/()' for x in expr) or 'math.' in expr ) and not expr.startswith('[') and not expr.startswith('{'):
            # Math evaluation (special)
            val = cls._safe_eval_python_expr(expr, config)
        else:
            val = config.get(expr)
        if isinstance(val, ConfigValues):
            val = cls._recursive_resolve_config_value(val, config)
            if isinstance(val, ConfigValues):
                print('Warning: Unresolvable ConfigValues found in expression: ' + expr)
                return None
        return val
                    ...
                    for substitution in cls._find_substitutions(current_item):
                        ...
                        if prop_path[0] == key:
                            if isinstance(previous_item, ConfigValues) and not accept_unresolved:
                                ...
                                try:
                                    value = previous_item if len(prop_path) == 1 else cls._eval_or_get(
                                        ".".join(prop_path[1:]), previous_item)
                                    _, _, current_item = cls._do_substitute(substitution, value)
                                except ConfigMissingException as ex:
                                    print("Could not resolve substitution: " + substitution.variable, ex)
    ...
    @classmethod
    def resolve_substitutions(cls, config, accept_unresolved=False, max_retries: int = 5):
        for i in range(max_retries):
            try:
                has_unresolved = False
                cls._fixup_self_references(config, accept_unresolved or i < 4)
                ...
                if i == 4:
                    ...
                    print("Retrying HOCON resolve", e)
                    continue
