...
class ConfigTree(OrderedDict):
    ...
    def __getattr__(self, item):
        ...
        if val is NonExistentKey:
            ct_super = super(ConfigTree, self)
            if hasattr(ct_super, "__getattr__"):
                return ct_super.__getattr__(item)
            else:
                raise AttributeError(f"No such key: {item}. Available: {list(self.keys())}")
        return val
