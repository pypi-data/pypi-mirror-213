...
class A2CBuilder(NetworkBuilder):
    ...
    class Network(NetworkBuilder.BaseNetwork):
        def __init__(self, params, **kwargs):
            ...
            input_shape = kwargs.pop('input_shape')[-1:]
            ...
            # define get_mu_symmetry() for different robots
            if 'A1' in params['task_name'] or 'Humanoid' in params['task_name']:
                self.left_inds = params['left_inds']
                self.right_inds = params['right_inds']
                self.inds_mirror_action = params['inds_mirror_action']
                self.get_mu_symmetry = self.get_mu_symmetry_reflection
            elif 'Trifinger' in params['task_name'] or 'Ant' in params['task_name']:
                self.symm_agents_inds = params['symm_agents_inds']
                self.num_agents = params['actions_num_masa'][0]
                self.get_mu_symmetry = self.get_mu_symmetry_rotation
            else:
                assert False, f"task {params['task_name']} not exist"
        ...
        def forward(self, obs_dict):
            ...
            if self.separate:
                ...
                if self.is_continuous:
                    ...
                    mu_symmetry = mu.clone()
                    mu_symmetry = self.get_mu_symmetry(mu_symmetry)
                    if self.fixed_sigma:
                        sigma = self.sigma_act(self.sigma)
                    else:
                        raise ValueError("Only consider fixed sigma.")
                        # sigma = self.sigma_act(self.sigma(out))
                    return mu, mu_symmetry, mu*0 + sigma, value, states
        
        def get_mu_symmetry_reflection(self, mu):
            mu[:,self.inds_mirror_action] *= -1
            mu[:,self.left_inds+self.right_inds] = mu[:,self.right_inds+self.left_inds]
            return mu.unsqueeze(1)

        def get_mu_symmetry_rotation(self, mu):
            mus = []
            for i in range(1,self.num_agents):
                mu_agent = [mu[:,self.symm_agents_inds[(i+k)%self.num_agents]] for k in range(self.num_agents)]
                mus.append(torch.cat(mu_agent,dim=-1))
...
class A2CBuilderMasa(NetworkBuilder):
    def __init__(self, **kwargs):
        NetworkBuilder.__init__(self)
    ...
    class Network(NetworkBuilder.BaseNetwork):
        def __init__(self, params, **kwargs):
            actions_num = kwargs.pop('actions_num')
            actions_num_masa = params['actions_num_masa']
            self.num_shared_agent = actions_num_masa[0]
            # TODO: humanoid and humanoidSkate have different joint order!
            if 'Ant' in params['task_name']:
                if params['method'] == 'mappo':
                    self.reshape_actions = self.reshape_actions_ant
                else:
                    self.reshape_actions = self.reshape_actions_ant
            elif 'A1' in params['task_name']:
                self.left_act_inds_neg = params['left_act_inds_neg']
                if params['method'] == 'mappo':
                    self.reshape_actions = self.reshape_actions_a1_ma
                else:
                    self.reshape_actions = self.reshape_actions_a1
            elif 'Anymal' in params['task_name']:
                self.left_act_inds_neg = params['left_act_inds_neg']
                self.reshape_actions = self.reshape_actions_anymal
            elif 'HumanoidSkate' in params['task_name']:
                self.left_act_inds_neg = params['left_act_inds_neg']
                if params['method'] == 'mappo':
                    self.reshape_actions = self.reshape_actions_humanoidSkate_ma
                else:
                    self.reshape_actions = self.reshape_actions_humanoidSkate
            elif 'Humanoid' in params['task_name']:
                self.left_act_inds_neg = params['left_act_inds_neg']
                if params['method'] == 'mappo':
                    self.reshape_actions = self.reshape_actions_humanoid_ma
                else:
                    self.reshape_actions = self.reshape_actions_humanoid
            elif 'Trifinger' in params['task_name']:
                if params['method'] == 'mappo':
                    self.reshape_actions = self.reshape_actions_trifinger
                else:
                    self.reshape_actions = self.reshape_actions_trifinger
            elif 'TriFranka' in params['task_name']:
                self.reshape_actions = self.reshape_actions_trifranka
            elif 'Trifinger' in params['task_name']:
                self.reshape_actions = self.reshape_actions_trifinger
            else:
                assert False, f"input shape {params['input_shape']} not exist"
            if 'A1Football' in params['task_name']:
                self.with_depth = True
            else:
                self.with_depth = False
            self.input_shape = torch.tensor([params['input_shape'],])
            self.value_size = kwargs.pop('value_size', 1)
            self.num_seqs = num_seqs = kwargs.pop('num_seqs', 1)
            NetworkBuilder.BaseNetwork.__init__(self)
            self.load(params)
            ...
            mlp_input_shape = self._calc_input_size(self.input_shape, self.actor_cnn)
            ...
            self.actor_mlp = self._build_mlp(**mlp_args)
            # critic_mlp can share with actor_mlp
            # two critic values calculated with two agents observations,
                # these two critic values should be the same --> use attention + deep set

            if self.with_depth:
                self.cnn_a1 = self._build_cnn_a1()
                mlp_args_a1 = {
                    'input_size' : 3*10*10, 
                    'units' : [200,80,20], 
                    'activation' : self.activation, 
                ...
                self.mlp_a1 = self._build_mlp(**mlp_args_a1)
            
            if self.separate:
                self.critics_mlp = self._build_masa_mlp(**mlp_args)

            self.value = torch.nn.Linear(out_size, self.value_size)
            self.value_act = self.activations_factory.create(self.value_activation)

            if self.is_continuous:
                self.mu = torch.nn.Linear(out_size, actions_num_masa[1])
                # self.mus = nn.ModuleList([mu_net,] * len(actions_num_list))
                self.mu_act = self.activations_factory.create(self.space_config['mu_activation']) 
                mu_init = self.init_factory.create(**self.space_config['mu_init'])
                self.sigma_act = self.activations_factory.create(self.space_config['sigma_activation']) 
                sigma_init = self.init_factory.create(**self.space_config['sigma_init'])
                ...
                if self.fixed_sigma:
                    ...
                    self.sigma = torch.nn.Linear(out_size, actions_num_masa[1])
                    # self.sigmas = nn.ModuleList([sigma_net,] * len(actions_num_list))
            ...
            if self.is_continuous:
                ...
                if self.fixed_sigma:
                    ...
                    #     sigma_init(sigma.weight)  

        def _build_cnn_a1(self,):
            """ create cnn layers for a1 depth image input
            """
            layers = []
            layers.append(torch.nn.Conv2d(in_channels=3,out_channels=6,kernel_size=5))
            layers.append(self.activations_factory.create(self.activation))
            layers.append(torch.nn.MaxPool2d(2,2))
            layers.append(torch.nn.Conv2d(in_channels=6,out_channels=6,kernel_size=5))
            layers.append(self.activations_factory.create(self.activation))
            layers.append(torch.nn.MaxPool2d(2,2))
        ...
        def reshape_actions_humanoidSkate(self, out, is_mu):
            if is_mu:
                out[:,0,self.left_act_inds_neg] = - out[:,0,self.left_act_inds_neg]
            return torch.cat([(out[:,0,4:5]+out[:,1,4:5])/2, out[:,1,6:9], out[:,0,6:9], 
                                (out[:,0,0:2]+out[:,1,0:2])/2, out[:,1,9:10], out[:,0,9:10],
                                out[:,1,2:4], out[:,0,2:4], out[:,1,10:], out[:,0,10:],
                                out[:,1,5:6], out[:,0,5:6]],dim=-1)

        def reshape_actions_humanoid(self, out, is_mu):
            if is_mu:
                out[:,0,self.left_act_inds_neg] = - out[:,0,self.left_act_inds_neg]
            return torch.cat([(out[:,0,0:2]+out[:,1,0:2])/2, out[:,1,2:4], out[:,0,2:4], 
                                (out[:,0,4:5]+out[:,1,4:5])/2, out[:,1,5:6], out[:,0,5:6],
                                out[:,1,6:9], out[:,0,6:9], out[:,1,9:10], out[:,0,9:10],
                                out[:,1,10:12], out[:,0,10:12]],dim=-1)

        def reshape_actions_humanoid_ma(self, out, is_mu):
            return torch.cat([(out[:,0,0:2]+out[:,1,0:2])/2, out[:,1,2:4], out[:,0,2:4], 
                                (out[:,0,4:5]+out[:,1,4:5])/2, out[:,1,5:6], out[:,0,5:6],
                                out[:,1,6:9], out[:,0,6:9], out[:,1,9:10], out[:,0,9:10],
                                out[:,1,10:12], out[:,0,10:12]],dim=-1)

        def reshape_actions_a1(self, out, is_mu):
            if is_mu:
                out[:,0,self.left_act_inds_neg] = - out[:,0,self.left_act_inds_neg]
            return torch.transpose(out, 1,2).reshape(out.shape[0], -1)
        
        def reshape_actions_a1_ma(self, out, is_mu):
            return torch.transpose(out, 1,2).reshape(out.shape[0], -1)
        
        def reshape_actions_anymal(self, out, is_mu):
            if is_mu:
                out[:,0,self.left_act_inds_neg] = - out[:,0,self.left_act_inds_neg]
            return torch.cat((out[:,0].view(out.shape[0],3,2), out[:,1].view(out.shape[0],3,2)), dim=-1).view(out.shape[0],-1)

        """
        def forward_ant(self, obs_dict):
            obs = obs_dict['obs']
            states = obs_dict.get('rnn_states', None)
            ...
            if self.separate:
                ...
                if self.has_rnn:
                    assert False, "has_rnn not implemented"
                else:
                    out = self.actor_mlp(out)
                value = self.value_act(self.value(torch.mean(out, dim=-2)))

                if self.central_value:
                    assert False, "central_value not implemented"

                if self.is_continuous:
                    mu = self.mu_act(self.mu(out))
                    mu[:,[1,2],1] = - mu[:,[1,2],1]
                    mu = torch.cat((mu[...,0], mu[...,1]), dim=-1)
                    assert False, "the mu only contain actions of two legs!"
                    if self.fixed_sigma:
                        sigma = self.sigma_act(self.sigma)
                    else:
                        sigma = self.sigma_act(self.sigma(out))
    ...
    def build(self, name, **kwargs):
        net = A2CBuilderMasa.Network(self.params, **kwargs)
        return net
