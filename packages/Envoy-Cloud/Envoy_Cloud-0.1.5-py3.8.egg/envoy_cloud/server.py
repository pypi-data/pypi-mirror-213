...
from envoy.exts.hooks import hook_logging
from envoy.exts.token import check_access_token
from envoy.openai.auth import Auth0
...
class ChatBot:
    ...
    def __after_request(resp):
        resp.headers['X-Server'] = 'envoy-cloud/{}'.format(__version__)
        return resp
    ...
    async def chat(self, conversation_id=None):
        ...
        return render_template(template, envoy_sentry=self.sentry, api_prefix=self.api_prefix, props=props)
    ...
    async def error404(self):
        ...
        return render_template('404.html', envoy_sentry=self.sentry, api_prefix=self.api_prefix, props=props)
    ...
    async def share_detail(self, share_id):
        ...
        return render_template('share.html', envoy_sentry=self.sentry, api_prefix=self.api_prefix, props=props)
