...
def test_main_parser_helper(capsys):
    from backupp.main_funcs_to_load import main_parser_helper

    items = (
        ("--setup", 'xx'),
        ("--check", 'xx'),
        # ("some_folderx", "some_foldery"),
        # ("..", "some_foldery"),
        ('--set', ".."),
        (False, 'default_folder'),
    )

    def x_helper(tuple_):
        return main_parser_helper(*tuple_)
    with capsys.disabled():
        items = tuple(map(str, (map(x_helper, items))))
        print("\n")
        print("\n".join(items))
