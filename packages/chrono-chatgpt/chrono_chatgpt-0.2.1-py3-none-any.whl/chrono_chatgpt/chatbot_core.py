...
class ChatBotCore:
    ...
    def ask(self, prompt: str, role: str = "user", convo_id: str = "default", temperature=0, response_only = True, **kwargs) -> str:
        max_depth = 10
        for _ in range(max_depth):
            result = self.ask_core(prompt, role, convo_id, temperature)
            if "response" in result and response_only:
                return result["response"]
            else:
                return result
        return None

    def ask_full(self, prompt: str, role: str = "user", convo_id: str = "default", temperature=0, response_only = True, **kwargs) -> str:
        return self.ask(prompt, role, convo_id, temperature, response_only=False)
