...
def save_torch_dataloader(run_id, dataloader, dataloader_name):
  ...
  for X_batch, y_batch in dataloader:
    data.append(X_batch.detach().cpu().numpy())
  ...
  dataset_dict = {str(dataloader_name)+'_shape' : str(data.shape), str(dataloader_name)+'_mean': np.mean(data), str(dataloader_name)+'_median': np.median(data),
                str(dataloader_name)+'_min': np.min(data), str(dataloader_name)+'_max': np.max(data), str(dataloader_name)+'_zeros': np.count_nonzero(labels==0),
                str(dataloader_name)+'_ones': np.count_nonzero(labels==1)}
