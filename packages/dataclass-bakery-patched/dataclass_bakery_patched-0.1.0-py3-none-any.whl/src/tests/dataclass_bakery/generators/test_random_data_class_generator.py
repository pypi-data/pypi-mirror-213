...
class TestRandomDataClassGenerator(TestCase):
    ...
    def test_attrs(self):
        attrs = collect_all_attributes(GrandChild)
        self.assertEqual(len(attrs), 3)
        self.assertDictEqual(attrs, {"a": int, "b": str, "c": float})

    def test_generate_dataclass_ok(self):
        random_data_class = self.random_data_class_generator.generate(testing_dataclasses.Stuff)
    ...
    def test_generate_dataclass_correct_generator_ok(self):
        random_data_class = self.random_data_class_generator.generate(
            testing_dataclasses.Stuff, **{"id": {defaults.GENERATOR_ARG: RandomFloatGenerator}}
        )
        self.assertIsInstance(random_data_class, testing_dataclasses.Stuff)
    ...
    def test_generate_dataclass_dict_typing_ok(self):
        random_data_class = self.random_data_class_generator.generate(testing_dataclasses.StuffDict)
    ...
    def test_generate_dataclass_list_typing_ok(self):
        random_data_class = self.random_data_class_generator.generate(testing_dataclasses.StuffList)
