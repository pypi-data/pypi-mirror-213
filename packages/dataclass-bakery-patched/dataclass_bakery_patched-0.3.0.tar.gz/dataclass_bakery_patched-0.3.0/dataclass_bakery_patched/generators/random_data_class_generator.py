...
def collect_all_attributes(dclass: Any) -> Dict:
    ...
    for pclass in dclass.__bases__:
        attrs.update(collect_all_attributes(pclass))
...
class RandomDataClassGenerator:
    def generate(self, data_class: Any, *args, **kwargs) -> Any:
        random_data = {}
        for field_name, field_type in collect_all_attributes(data_class).items():
            ...
            if (
                isinstance(field_type, _GenericAlias) and field_type.__origin__ != Literal
            ):  # Is a Dict or List
            ...
            elif isinstance(field_type, _GenericAlias) and field_type.__origin__ == Literal:
