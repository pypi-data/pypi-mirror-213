...
def is_ci_mr_target_branch_main() -> bool:
    main_target = os.getenv("CI_MERGE_REQUEST_TARGET_BRANCH_NAME") == 'main'
    if main_target:
        print("CI merge request target branch is main")
        return main_target
    else:
        print("CI merge request target branch is not main")
        return main_target
