...
def generate_commits_range_to_check(
    ...
    If is_ci_mr_target_branch_main is True, the function returns the commit range
    starting from the second merge found.
    """
    merge_count = 0
    merge_commit_pipeline = False

    for i, commit in enumerate(commits):
        if (
            commit.title.startswith("Merge branch")
            and is_ci_mr_target_branch_main
            and merge_count == 0
        ):
            merge_commit_pipeline = True
        if merge_commit_pipeline:
