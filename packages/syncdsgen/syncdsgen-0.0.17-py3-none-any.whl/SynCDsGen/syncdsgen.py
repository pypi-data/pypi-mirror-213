...
def check_row_sum(tensor, tol=1e-2):
    """
...
class SyncCDsGeneratorConf:
    def __init__(self, bases=None, codons=None, nb_codons=None,
        ...
        if all(el is not None for el in [self.nb_codons, self.nb_AAs, self.codon_length]):
            ...
            self.transition_prob_t = torch.full((self.nb_AAs, self.nb_AAs), 1/(self.nb_AAs-nb_start_AAs-nb_stop_AAs))
            start_aas_inds = [i for i,x in enumerate(start_codons_prob_dist) if x==1]
            stop_aas_inds = [i for i,x in enumerate(stop_codons_prob_dist) if x==1]

            for i in start_aas_inds:
                for j in range(len(self.AAs)):
                    self.transition_prob_t[j][i] = 0

            for i in stop_aas_inds:
                for j in range(len(self.AAs)):
                    self.transition_prob_t[i][j] = 0
                    self.transition_prob_t[j][i] = 0
...
class SynCDsGenerator:
    def __init__(self, generatorConf:SyncCDsGeneratorConf):
        ...
        mask = torch.ones(self.generatorConf.transition_prob_t.size(0), dtype=bool)

        #compute the number of stop amino acids
        stop_codons_prob_dist = [self.generatorConf.is_AA_stop_AA(AA) for AA in self.generatorConf.AAs]
        stop_aas_inds = [i for i,x in enumerate(stop_codons_prob_dist) if x==1]
        mask[stop_aas_inds] = False
        assert check_row_sum(self.generatorConf.transition_prob_t[mask]), "Each row of the transition_prob_t should sum up to 1"
