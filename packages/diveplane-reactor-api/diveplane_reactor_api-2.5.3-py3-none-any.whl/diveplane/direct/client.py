...
from diveplane import utilities as util
from diveplane.client import AbstractDiveplaneClient
from diveplane.client.cache import TraineeCache
from diveplane.client.exceptions import DiveplaneError
from diveplane.openapi.models import (
    Session,
    Trainee,
)
import diveplane.openapi.models as client_models
...
class DiveplaneDirectClient(AbstractDiveplaneClient):
    """
    The direct Diveplane client.

    A client which provides access to the Diveplane core endpoints
    via a direct interface using dynamic libraries.

    Parameters
    ----------
    diveplane_core : diveplane.direct.DiveplaneCore, optional
        A specified diveplane core direct interface object.

        If None, an interface will be generated using the provided handle.
    debug : bool, default False
        Set debug output.
    handle : str, optional
        The diveplane core entity handle to use.

        If None, :attr:`DiveplaneDirectClient.DEFAULT_HANDLE` will be used.
    verbose : bool, default False
        Set verbose output.
    """

    #: The default Diveplane core entity handle.
    DEFAULT_HANDLE = "dp"
    ...
    def __init__(
        self,
        diveplane_core: Optional[DiveplaneCore] = None,
        *,
        debug: bool = False,
        ...
        **kwargs
        ...
        # Set the 'diveplane' logger level to debug
        if debug:
        ...
        if diveplane_core is None:
            if handle not in _core_cache:
                _core_cache[handle] = DiveplaneCore(
                    self.get_unique_handle(handle),
                    **kwargs
                )
            self.dp = _core_cache[handle]
        elif isinstance(diveplane_core, DiveplaneCore):
            self.dp = diveplane_core
        else:
            raise ValueError("The client parameter diveplane_core must be "
                             "an instance of DiveplaneCore")
    ...
    def active_session(self) -> Session:
        ...
        Returns
        -------
        diveplane.openapi.models.Session
    ...
    def trainee_cache(self) -> TraineeCache:
        ...
        Returns
        -------
        diveplane.client.cache.TraineeCache
            The trainee cache.
        """
    ...
    def get_unique_handle(handle: str) -> str:
        ...
        """
        return f"{handle}-{DiveplaneCore.random_handle()}"
    ...
    def get_entities(self) -> List[str]:
        ...
        """
        return self.dp.get_entities()

    def get_version(self) -> client_models.DiveplaneVersion:
        """
        Return the Diveplane version.

        Returns
        -------
        diveplane.openapi.models.DiveplaneVersion
           A version response that contains the version data for the current
           instance of Diveplane.
        """
        from diveplane.client import __version__ as client_version
        from diveplane.openapi import __api_version__ as api_version

        return client_models.DiveplaneVersion(api=api_version,
                                              client=client_version)

    def _output_version_in_trace(self, trainee: str):
        """
        Instruct Diveplane core to retrieve the version of the Trainee.
        ...
        ----------
        trainee : str
            The ID of the Trainee that should retrieve the Diveplane version.
        """
        from diveplane.client import __version__
        amlg_version = self.dp.amlg.get_version_string()
        self.dp.version()
        ...
        self.dp.get_trainee_version(trainee, trace_version)
    ...
    def create_trainee(  # noqa: C901
        ...
        *,
        ...
        """
        Create a Trainee on the Diveplane service.
        ...
        resources : diveplane.openapi.models.TraineeResources or dict, optional
        ...
        if trainee.name:
            ...
            proposed_path: Path = self.dp.default_save_path.joinpath(trainee.name)
            if success:
        ...
        result = self.dp.create_trainee(trainee_id)
        if not result:
            raise ValueError(
                f"Could not create the trainee with name {trainee_id}. "
                f"Possible causes - Diveplane couldn't find core "
        ...
        self.dp.set_metadata(trainee_id, metadata)
        self.dp.set_feature_attributes(trainee_id, trainee.features)
        trainee.features = self.dp.get_feature_attributes(trainee_id)

        self._output_version_in_trace(trainee_id)

        new_trainee = internals.postprocess_trainee(trainee)
        self.trainee_cache.set(new_trainee, entity_id=self.dp.handle)
        return new_trainee

    def update_trainee(self, trainee: Trainee):
        """
        Update an existing Trainee in the Diveplane service.
    ...
    def export_trainee(
        ...
        self.dp.export_trainee(trainee_id,
                               path_to_trainee,
                               decode_cases,
                               separate_files)
    ...
    def upgrade_trainee(
        ...
        self.dp.upgrade_trainee(trainee_id,
                                path_to_trainee,
                                separate_files)

    def get_trainee(self, trainee_id: str):
        """
        Gets a trainee loaded in the Diveplane service.
    ...
    def get_trainee_information(self, trainee_id: str) -> client_models.TraineeInformation:
        """
        ...
        Returns
        -------
        diveplane.openapi.models.TraineeInformation
            The Trainee information.
        """
        self._auto_resolve_trainee(trainee_id)
        trainee_version = self.dp.get_trainee_version(trainee_id)
        dpcore_version = self.dp.version()
        amlg_version = self.dp.amlg.get_version_string().decode()
        library_type = 'st'
        if self.dp.amlg.library_postfix:
            library_type = self.dp.amlg.library_postfix[1:]

        version = client_models.TraineeVersion(core=dpcore_version,
    ...
    def get_trainee_metrics(self, trainee_id: str) -> Never:
        """
        This endpoint is not implemented for the direct Diveplane client.
    ...
    def get_trainees(self, search_terms: Optional[str] = None):
        ...
        Returns
        -------
        list of diveplane.openapi.models.TraineeIdentity
        ...
        files = os.listdir(self.dp.default_save_path)
        for f in files:
            if not f.endswith(self.dp.ext):
                continue
    ...
    def delete_trainee(self, trainee_id: str):
        ...
        trainee_path = Path(self.dp.default_save_path, f'{trainee_id}{self.dp.ext}')
        trainee_ver_path = Path(self.dp.default_save_path, f'{trainee_id}Version.txt')
    ...
    def copy_trainee(
        ...
        *,
        ...
        """
        Copies a trainee to a new trainee id in the Diveplane service.
        ...
        output = self.dp.copy(trainee_id, new_trainee_id)
        ...
        if output and output.get('name') == new_trainee_id:
            ...
            self.dp.set_metadata(new_trainee_id, metadata)
            self.trainee_cache.set(new_trainee, entity_id=self.dp.handle)
            ...
            raise ValueError(
                ...
                f"causes - diveplane couldn't find core binaries/camls or "
    ...
    def acquire_trainee_resources(
        ...
        *,
        ...
        if trainee_id is None:
            raise DiveplaneError("A trainee id is required.")
        if self.verbose:
        ...
        if trainee_id in self.trainee_cache:
            ...
            if cache_item.get('entity_id') != self.dp.handle:
                raise DiveplaneError(
                    "Unable to acquire trainee resources for the trainee "
                    f"'{trainee_id}'. Trainee is already loaded in another "
                    "core entity. Use the DiveplaneClient instance with the "
                    f"entity handle '{self.dp.handle}' instead or release it "
                    "via the other client first."
                )
            return

        ret = self.dp.load(trainee_id)

        if ret is None:
            raise DiveplaneError(f"Trainee '{trainee_id}' not found.")

        trainee = self._get_trainee_from_core(trainee_id)
        self.trainee_cache.set(trainee, entity_id=self.dp.handle)

    def _get_trainee_from_core(self, trainee_id: str) -> Trainee:
        """
        Retrieve the core representation of a Trainee and return it in a Trainee object.
        ...
        DiveplaneError
            If no Trainee with the requested ID can be found.
        """
        metadata = self.dp.get_metadata(trainee_id)
        if metadata is None:
            raise DiveplaneError(f"Trainee '{trainee_id}' not found.")
        ...
        features = self.dp.get_feature_attributes(trainee_id)
    ...
    def release_trainee_resources(self, trainee_id: str):
        ...
        try:
            ...
            if cache_item.get('entity_id', self.dp.handle) == self.dp.handle:
                if trainee.persistence in ['allow', 'always']:
                    # Persist on unload
                    self.dp.persist(trainee_id)
    ...
    def persist_trainee(self, trainee_id: str):
        """
        Persists a Trainee in the Diveplane service storage.

        After persisting, the Trainee resources can be
        :func:`released <client.DiveplaneClient.release_trainee_resources>`.
        ...
        if trainee_id not in self.trainee_cache:
            ...
            if entity_id != self.dp.handle:
                raise DiveplaneError(
                    ...
                    "DiveplaneClient instance with the core entity handle "
                    f"'{self.dp.handle}' instead to access this trainee or "
    ...
    def remove_series_store(self, trainee_id: str, series: Optional[str] = None):
        ...
        self.dp.remove_series_store(trainee_id, series)
    ...
    def set_substitute_feature_values(
        ...
        if self.verbose:
            print('Setting substitute feature values for trainee with '
                  f'id: {trainee_id}')
        self.dp.set_substitute_feature_values(trainee_id, substitution_value_map)
    ...
    def get_substitute_feature_values(
        ...
        clear_on_get : bool, default True
            Clears the substitution values map in the Trainee upon retrieving them. This is done if it is desired to
            prevent the substitution map from being persisted. If set to False the model will not be cleared which
            preserves substitution mappings if the model is saved; representing a potential privacy leak should the
        ...
        if self.verbose:
            print(f'Getting substitute feature values from trainee with '
                  f'id: {trainee_id}')
        ret = self.dp.get_substitute_feature_values(trainee_id)
        if clear_on_get:
    ...
    def set_random_seed(self, trainee_id: str, seed: Union[int, float, str]):
        ...
        self.dp.set_random_seed(trainee_id, seed)
        self._auto_persist_trainee(trainee_id)
    ...
    def get_trainee_sessions(self, trainee_id: str) -> List[Dict[str, str]]:
        ...
        sessions = self.dp.get_sessions(trainee_id, attributes=['name', ])
    ...
    def delete_trainee_session(self, trainee_id: str, session: str):
        ...
        if self.verbose:
            print(f'Deleting session {session} from trainee with id: '
                  f'{trainee_id}')
        self.dp.remove_session(trainee_id, session)
        self._auto_persist_trainee(trainee_id)
    ...
    def get_sessions(self, search_terms: Optional[str] = None) -> List[Session]:
        ...
        Returns
        -------
        list of diveplane.openapi.models.Session
        ...
        for trainee_id in self.trainee_cache.ids():
            sessions = self.dp.get_sessions(
                trainee=trainee_id, attributes=list(Session.attribute_map))
            if not sessions:
                continue
    ...
    def get_session(self, session_id: str) -> Session:
        ...
        for trainee_id in loaded_trainees:
            try:
                session_data = self.dp.get_session_metadata(trainee_id,
                                                            session_id)
            ...
            break
        if session is None:
            raise DiveplaneError("Session not found")
        return session
    ...
    def update_session(self, session_id: str, *, metadata: Optional[Dict] = None) -> Session:
        ...
        for trainee_id in self.trainee_cache.ids():
            ...
            self.dp.set_session_metadata(trainee_id, session_id, session)
    ...
    def react_series(  # noqa: C901
        ...
        *,
        ...
        contexts: list of list of object or DataFrame
            See parameter ``contexts`` in :meth:`DiveplaneDirectClient.react`.
        action_features: iterable of str
            See parameter ``action_features`` in :meth:`DiveplaneDirectClient.react`.
        actions: list of list of object or DataFrame
            See parameter ``actions`` in :meth:`DiveplaneDirectClient.react`.
        context_features: iterable of str
            See parameter ``context_features`` in :meth:`DiveplaneDirectClient.react`.
        input_is_substituted : bool, default False
            See parameter ``input_is_substituted`` in :meth:`DiveplaneDirectClient.react`.
        substitute_output : bool
            See parameter ``substitute_output`` in :meth:`DiveplaneDirectClient.react`.
        details: dict, optional
            See parameter ``details`` in :meth:`DiveplaneDirectClient.react`.
        desired_conviction: float
            See parameter ``desired_conviction`` in :meth:`DiveplaneDirectClient.react`.
        weight_feature : str
            See parameter ``weight_feature`` in :meth:`DiveplaneDirectClient.react`.
        use_case_weights : bool
            See parameter ``use_case_weights`` in :meth:`DiveplaneDirectClient.react`.
        case_indices: iterable of sequence of str, int
            See parameter ``case_indices`` in :meth:`DiveplaneDirectClient.react`.
        preserve_feature_values : iterable of str
            See parameter ``preserve_feature_values`` in :meth:`DiveplaneDirectClient.react`.
        new_case_threshold : str
            See parameter ``new_case_threshold`` in :meth:`DiveplaneDirectClient.react`.
        leave_case_out : bool
            See parameter ``leave_case_out`` in :meth:`DiveplaneDirectClient.react`.
        use_regional_model_residuals : bool
            See parameter ``use_regional_model_residuals`` in :meth:`DiveplaneDirectClient.react`.
        feature_bounds_map: dict of dict
            See parameter ``feature_bounds_map`` in :meth:`DiveplaneDirectClient.react`.
        generate_new_cases : {"always", "attempt", "no"}
            See parameter ``generate_new_cases`` in :meth:`DiveplaneDirectClient.react`.
        ordered_by_specified_features : bool
            See parameter ``ordered_by_specified_features`` in :meth:`DiveplaneDirectClient.react`.
        suppress_warning : bool
            See parameter ``suppress_warning`` in :meth:`DiveplaneDirectClient.react`.
        ...
        if desired_conviction is None:
            ...
            if (
                ...
                raise DiveplaneError("`num_series_to_generate` must be an "
    ...
    def _react_series(self, trainee_id, react_params):
        ...
        """
        batch_result = self.dp.batch_react_series(trainee_id, **react_params)
    ...
    def append_to_series_store(
        ...
        *,
        ...
        self.dp.append_to_series_store(trainee_id,
                                       context_features=context_features,
                                       contexts=contexts,
                                       series=series)
        ...
        *,
        ...
        if desired_conviction is None:
            ...
            if (
                ...
                raise DiveplaneError("`num_cases_to_generate` must be an "
                                     "integer greater than 0.")
    ...
    def _react(self, trainee_id: str, react_params: Dict) -> Dict:
        ...
        dict
            The react response.
        """
        ret = self.dp.batch_react(trainee_id, **react_params)
    ...
    def _preprocess_react_parameters(
        ...
        *,
        ...
        Helper method to pre-process the parameters for `react` and `react_series`.
        Updates the passed-in parameters as necessary.

        Parameters
        ----------
        action_features: iterable of str
            See :meth:`DiveplaneDirectClient.react()`.
    ...
    def get_cases(
        ...
        Returns
        -------
        diveplane.openapi.models.Cases
    ...
    def react_group(
        ...
        *,
        ...
        features : iterable of str, optional
            An iterable of feature names to consider while calculating convictions.
    ...
    def get_feature_conviction(
        ...
        *,
        ...
        trainee_id : str
            The id of the trainee.
        features : iterable of str, optional
            An iterable of feature names to calculate convictions. At least 2 features are required to get
            familiarity conviction. If not specified all features will be used.
        action_features : iterable of str, optional
            An iterable of feature names to be treated as action features during conviction calculation in order
            to determine the conviction of each feature against the set of action_features.  If not specified,
    ...
    def get_feature_residuals(
        ...
        *,
        ...
        residuals = self.dp.get_feature_residuals(
            trainee_id,
    ...
    def get_feature_contributions(
        ...
        *,
        ...
        contributions = self.dp.get_feature_contributions(
            trainee_id,
            action_feature=action_feature,
            robust=robust,
            weight_feature=weight_feature)
    ...
    def get_marginal_stats(
        ...
        *,
        ...
        stats = self.dp.get_marginal_stats(
            trainee_id,
            weight_feature=weight_feature)
        return stats
    ...
    def get_extreme_cases(
        ...
        result = self.dp.retrieve_extreme_cases_for_feature(
            trainee_id, features, sort_feature, num)
        if result is None:
            result = dict()
        return client_models.Cases(features=result.get('features'),
                                   cases=result.get('cases'))
    ...
    def _preprocess_generate_parameters(  # noqa: C901
        ...
        *,
        ...
        """
        Helper method to pre-process generative react parameters for `react` and `react_series`.
        ...
        if desired_conviction is not None and desired_conviction <= 0:
            raise DiveplaneError("Desired conviction must be greater than 0.")
    ...
    def get_trainee_session_indices(self, trainee_id: str, session: str) -> List[int]:
        """
        Gets a list of all session indices for a specified session.
        ...
        """
        self._auto_resolve_trainee(trainee_id)
        return self.dp.get_session_indices(trainee_id, session)

    def get_trainee_session_training_indices(self, trainee_id: str, session: str) -> List[int]:
        """
        Gets a list of all session training indices for a specified session.
        ...
        """
        self._auto_resolve_trainee(trainee_id)
        result = self.dp.get_session_training_indices(trainee_id, session)
        if result is None:
            return []
        return result
    ...
    def get_label(self, entity_id: str, label: str) -> object:
        ...
        return self.dp.amlg.get_json_from_label(entity_id, label)
    ...
    def set_label(self, entity_id: str, label: str, label_value: str):
        ...
        return self.dp.amlg.set_json_to_label(entity_id, label,
                                              json.dumps(label_value))
    ...
    def execute_label(self, entity_id: str, label: str) -> object:
        ...
        return self.dp.amlg.execute_entity_json(entity_id, label, "{}")
    ...
    def get_pairwise_distances(  # noqa: C901
        ...
        *,
        ...
        result = self.dp.pairwise_distances(
            trainee_id,
            features=features,
            action_feature=action_feature,
            ...
            for column_offset in range(0, num_cases, page_size):
                ...
                if preallocate:
                    ...
                    try:
                        ...
                        raise DiveplaneError(mismatch_msg) from err
                else:
                    if column_offset == 0:
                        # Append new rows
                        ...
                        try:
                            ...
                            # Unexpected shape when populating array
                            raise DiveplaneError(mismatch_msg)
    ...
    def compute_feature_weights(
        ...
        weights = self.dp.compute_feature_weights(
    ...
    def set_feature_weights(
        ...
        """
        Set the weights for the features in the Trainee for the specified action_feature.

        If action_feature is not specified, it will set the passed in weights as targetless.
        ...
        use_feature_weights : bool, default True
            When set to true, forces the trainee to use the specified feature weights
        """
        self._auto_resolve_trainee(trainee_id)
        self.dp.set_feature_weights(trainee_id, feature_weights, action_feature, use_feature_weights)
        self._auto_persist_trainee(trainee_id)
    ...
    def set_feature_weights_matrix(
        ...
        feature_weights_matrix : dict
            A dictionary of feature names -> a dictionary of feature names -> weight values.
            Ex {"a" : {"a", 1.0, "b": 0.1, "c": 0.5, ... , "z": 1.0} }
        use_feature_weights : bool, default True
            When set to true, forces the trainee to use the specified feature weights
        """
        self._auto_resolve_trainee(trainee_id)
        self.dp.set_feature_weights_matrix(trainee_id, feature_weights_matrix, use_feature_weights)
        self._auto_persist_trainee(trainee_id)
    ...
    def get_feature_weights_matrix(
        ...
        -------
        dict
            A dictionary of action feature -> dictionary of features -> feature weight

        """
        self._auto_resolve_trainee(trainee_id)
        return self.dp.get_feature_weights_matrix(trainee_id)

    def get_feature_attributes(self, trainee_id: str) -> List:
        """
        ...
        -------
        list
            A list of dictionaries of feature attributes
        ...
        if self.verbose:
            print('Getting feature attributes from trainee with '
                  f'id: {trainee_id}')
        feature_attributes = self.dp.get_feature_attributes(trainee_id)
    ...
    def set_feature_attributes(
        ...
        fixed_attribs = internals.preprocess_feature_attributes(
            feature_attributes)
        self.dp.set_feature_attributes(trainee_id, fixed_attribs)
        self._auto_persist_trainee(trainee_id)
    ...
    def evaluate(
        ...
        *,
        ...
        return self.dp.evaluate(trainee_id, **evaluate_params)
