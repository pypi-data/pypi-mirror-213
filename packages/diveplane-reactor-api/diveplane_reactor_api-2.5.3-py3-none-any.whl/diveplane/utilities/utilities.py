...
def replace_none_with_nan(dat: Mapping) -> List[Dict]:
    ...
    For use when retrieving data from Diveplane via the scikit module to
...
def replace_nan_with_none(dat):
    ...
    For use when feeding data to Diveplane from the scikit module to account
    for the different ways diveplane and sklearn represent missing values.
...
def validate_case_indices(case_indices: CaseIndices, thorough=False) -> None:
    """
    Validate the case_indices parameter to the react() method of a Diveplane client.
...
class UserFriendlyExit:
    ...
    def __call__(self, msg="An unexpected exit occurred.", exception=None):
        ...
        None
        """
        print("Diveplane Client Error: " + msg)
