...
try:
    ...
    Geminai = None
from diveplane.utilities import StopExecution, Timer
from diveplane.utilities.locale import get_default_locale
from diveplane.utilities.posix import PlatformError, sysctl_by_name
...
LOG_FILE = "diveplane_stacktrace.txt"
...
class InstallationCheckRegistry:
    ...
    def __init__(self):
        ...
        # And the next check which builds a dp client
        self.add_check(
            name="Diveplane Client: Configuration",
    ...
    def add_check(self, name: str,
        ...
        other_requirements : Iterable of classes or objects, default None
            Optional. Other required classes, E.g., `Geminai`. Note, these
    ...
    @property
    def client(self) -> AbstractDiveplaneClient:
        """
        ...
        AbstractDiveplaneClient
            A instance of AbstractDiveplaneClient determined by the
            user's configuration.
        """
        if self._client is None:
            self._client = DiveplaneClient(debug=0)
        return self._client
    ...
    def _check_client_configuration(registry):
        """
        Check that the Diveplane client can be instantiated.
        ...
        """
        try:
            registry._client = DiveplaneClient(debug=0)
        except DiveplaneLicenseAcceptanceException as e:
            return (Status.CRITICAL, e.message)
        except DiveplaneConfigurationError:
            traceback.print_exc(file=registry.logger)
            return (
                Status.CRITICAL,
                "The diveplane configuration file was not found in the "
                "location that was specified in the `DP_CONFIG` environment "
                "variable. Please see the Diveplane Client installation "
            ...
            return (
                Status.CRITICAL,
                "Diveplane Client could not be started due to file "
                "permissions. Please see the Diveplane Client installation "
                "documentation for further details."
            )
            ...
            return (
                Status.CRITICAL,
                "Unable to connect to a Diveplane Platform. Please ensure "
                "that you have a valid `diveplane.yml` file in the correct "
                "location. Please see the Diveplane Client installation "
                "documentation for further details."
            )
        except ValueError:
            traceback.print_exc(file=registry.logger)
            return (Status.CRITICAL,
                    "The client was unable for find Diveplane core binaries. "
                    "Please see the Diveplane Client installation "
                    "documentation for further details.")
            ...
            if ConnectionError is not None and isinstance(e, ConnectionError):
                ...
                return (
                    Status.CRITICAL,
                    "Unable to connect to the Diveplane Platform "
                    "configured in your `diveplane.yml` file. Please check for "
                ...
                return (
                    Status.CRITICAL,
                    "There was a problem instantiating the Diveplane client. "
                    "Please see the Diveplane Client installation documentation "
                    "for further details."
                )
    ...
    def run_checks(self):  # noqa: C901
        ...
        if not issues:
            print("[bold green]You are ready to use Diveplane:registered:!")
            print("[default]" + diveplane_banner)
        else:
            print("[bold yellow]There were one or more issues. Please review "
                  ...
                  "Diveplane:registered: representative.")
...
def generate_dataframe(*, client: AbstractDiveplaneClient,
                       num_samples: int = 150,
                       timeout: Optional[int] = None
                       ) -> pd.DataFrame:
    """
    Use DiveplaneClient to create a dataframe of random data.

    Parameters
    ----------
    client : AbstractDiveplaneClient
        The Diveplane client instance to use.
...
def check_basic_synthesis(*, registry: InstallationCheckRegistry,
                          source_df=None):
    """
    Validate that GeminAI can perform a basic synthesis.
    ...
    try:
        ...
        with Geminai(client=registry.client, privacy_override=True) as g:
            g.train(source_df, features)
            source_df = g.synthesize_cases(n_samples=100)
...
def check_geminai_create_delete(*, registry: InstallationCheckRegistry,
                                source_df: pd.DataFrame = None):
    """
    ...
    try:
        ...
        features = infer_feature_attributes(source_df)
        g = Geminai(client=registry.client, privacy_override=True)

        g.train(source_df[:50], features)
        n = g.cl.get_num_training_cases(g.trainee.id)
        if n != 50:
            return (Status.ERROR,
                    f"Training did not produce the correct number of "
                    f"training cases ({n}). Diveplane GeminAI might not be "
                    "installed correctly. "
                    "Try: `pip install --upgrade diveplane-geminai`.")

        g.cl.delete_trainee(g.trainee.id)
    except Exception:  # noqa: Deliberately broad
        traceback.print_exc(file=registry.logger)
        return (Status.CRITICAL,
                "Could not complete check. Check installation. "
    ...
    try:
        ...
        if num_rows < warning_threshold:
            return (Status.WARNING,
                    ...
                    "Ensure a good network connection and that Diveplane "
    ...
    try:
        ...
        if num_seconds > warning_threshold:
            return (
                ...
                "Diveplane's recommended hardware specifications. "
                "In auto-scaling installations this may be due to slow node "
                "start-ups.")
            ...
            return (
                ...
                "equipment that meets Diveplane's recommended specifications. "
...
def check_reactor_operation(*, registry: InstallationCheckRegistry,
                            source_df: pd.DataFrame = None):
    """
    Ensure that Diveplane Reactor operates as it should.
    ...
    try:
        ...
        try:
            reactor.delete_trainee(trainee.id)
        except Exception:  # noqa: Deliberately broad
            pass


def check_dqt_enterprise_desirability(*, registry: InstallationCheckRegistry,
                                      source_df: pd.DataFrame = None,
                                      desirability_threshold: float = 2.0):
    """
    Ensure that DQT-Enterprise operates as it should, returning relatively high desirability.
    ...
    desirability_threshold : float, default 3.0
        The desirability threshold. If the run of DQT-E returns a desirability
        less than this value, the returned Status will be WARNING.
    ...
    tuple
        ...
        str
            A message to display about the WARNING, ERROR or CRITICAL result.
    """
    if isinstance(DataQuality, Exception):
        print(DataQuality, file=registry.logger)
        return (Status.CRITICAL,
                "Diveplane Data Quality Tool was not installed correctly. "
...
def configure(registry: InstallationCheckRegistry):
    ...
    """
    registry.add_check(
        name="Diveplane Local: Timezone support",
        fn=check_tzdata_installed,
        client_required="DiveplaneDirectClient",
    )

    registry.add_check(
        name="Diveplane Client: Basic react",
        fn=partial(check_generate_dataframe, threshold=10.0),
        client_required="DiveplanePlatformClient",
    )

    registry.add_check(
        name="Diveplane Local: Basic react",
        fn=partial(check_generate_dataframe, threshold=1.0),
        client_required="DiveplaneDirectClient",
    )

    registry.add_check(
        name="Diveplane Client: Network latency",
    ...
    registry.add_check(
        name="Diveplane Client: System performance",
    ...
    registry.add_check(
        name="Diveplane Local: System performance",
    ...
    registry.add_check(
        name="Diveplane Local: Save model",
        fn=check_save,
        client_required="DiveplaneDirectClient",
    )

    registry.add_check(
        name="Diveplane Reactor™: Basic operations",
        fn=check_reactor_operation,
        client_required="AbstractDiveplaneClient",
        other_requirements=[reactor],
    )

    registry.add_check(
        name="Diveplane GeminAI™: Supported system locales",
        fn=check_locales_available,
        client_required="AbstractDiveplaneClient",
        other_requirements=[Geminai],
    )

    registry.add_check(
        name="Diveplane GeminAI: Basic operations",
        fn=partial(check_geminai_create_delete),
        client_required="AbstractDiveplaneClient",
        other_requirements=[Geminai],
    )

    registry.add_check(
        name="Diveplane GeminAI: Basic synthesis",
    ...
    registry.add_check(
        name="Diveplane DQT-E: Desirability",
        fn=partial(check_dqt_enterprise_desirability, desirability_threshold=2.0),
        client_required="AbstractDiveplaneClient",
        other_requirements=[Geminai, DataQuality],
    )
