...
from diveplane.client.exceptions import DiveplaneApiError, DiveplaneError
...
from diveplane.reactor.client import get_client
from diveplane.reactor.project import Project
from diveplane.reactor.session import Session
from diveplane.utilities import CaseIndices
...
class Trainee(BaseTrainee):
    ...
    client : AbstractDiveplaneClient, optional
    ...
    def __init__(
        ...
        *,
        ...
        client: Optional[AbstractDiveplaneClient] = None,
    ...
    @client.setter
    def client(self, client: AbstractDiveplaneClient) -> None:
        """
        ...
        client : AbstractDiveplaneClient
            The client instance. Must be a subclass of AbstractDiveplaneClient
            and DiveplanePandasClientMixin.
        ...
        if not isinstance(client, AbstractDiveplaneClient):
            raise DiveplaneError(
                "`client` must be a subclass of AbstractDiveplaneClient"
            )
        if not isinstance(client, DiveplanePandasClientMixin):
            raise DiveplaneError("`client` must be a DiveplanePandasClient")
        self._client = client
    ...
    @classmethod
    def from_openapi(
        cls, trainee: BaseTrainee, *, client: Optional[AbstractDiveplaneClient] = None
    ) -> "Trainee":
        """
    ...
    def __exit__(self, exc_type, exc_val, exc_tb) -> None:
        ...
        try:
            if self.persistence != "never" and (self.name or self._was_saved):
                ...
                self.delete()
        except DiveplaneApiError as error:
            if error.status != 404:
                raise


def delete_trainee(
    name_or_id: str, *, client: Optional[AbstractDiveplaneClient] = None
) -> None:
    """
...
def list_trainees(
    ...
    *,
    ...
    trainees, not full reactor Trainee instances. To get a Trainee instance
    that can be used with the reactor API call `get_trainee`.
