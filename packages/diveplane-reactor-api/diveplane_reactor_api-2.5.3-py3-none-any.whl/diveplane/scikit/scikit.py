...
from diveplane.client.exceptions import (
    DiveplaneApiError,
    DiveplaneError,
    DiveplaneNotUniqueError,
)
import diveplane.utilities as utils
...
class DiveplaneEstimator(BaseEstimator):
    """
    ...
    range of Diveplane capabilities, please use the client specified in the
    diveplane.client module.
    ...
    client : AbstractDiveplaneClient, default None
        A subclass of AbstractDiveplaneClient used to interface with Diveplane.
    method : str
    ...
    def __init__(self, client: Optional[AbstractDiveplaneClient] = None,
        ...
        if features is None and targets or features and targets is None:
            raise ValueError(
                ...
                "access to a wider range of Diveplane capabilities, please "
                "use the client specified in the diveplane.client module.")
        ...
        if client is None and client_params is None:
            self.client = DiveplanePandasClient(verbose=self.verbose, debug=self.debug)
        ...
        if getattr(self, 'trainee_id', None):
            ...
            if trainee_name:
                try:
                    ...
                    live_trainee = reactor.get_trainee(self.trainee_id)
                    ...
                    if isinstance(e, AssertionError):
                        print("The Diveplane estimator's trainee was not "
                              "permitted to be saved. It will be deleted.")
                    else:
                        print("An error prevented the saving of the Diveplane "
                              "estimator's trainee. It will be deleted.")
                    try:
                        self.trainee.delete()
                    except Exception:  # noqa: Deliberately broad
                        print("The Diveplane estimator could not delete "
                              "its trainee.")
                    else:
                        if self.verbose:
                            print("The Diveplane estimator has successfully "
                                  "deleted its trainee.")
                else:
                    if self.verbose:
                        print(f'The Diveplane estimator\'s trainee with name '
                ...
                try:
                    ...
                    print("The Diveplane estimator did not successfully "
                          "delete its trainee.")
                else:
            ...
            print("The Diveplane estimator has no trainees to delete.")
    ...
    def trainee_name(self, name: str = ''):
        ...
        try:
            ...
            if (
                    isinstance(exc, DiveplaneApiError) and
                    getattr(exc, 'status', 0) == 409
            ):
                raise DiveplaneNotUniqueError(
                    f'Unable to set the name of the Diveplane estimator\'s '
    ...
    def fit(self, X, y, optimize=True) -> "DiveplaneEstimator":
        ...
        # In Diveplane 'features' can be either predictors or targets.
        if not self.trainee_params:

            self.trainee = reactor.Trainee(
    ...
    def get_case_conviction(self, X, features=None) -> List:
        ...
        if type(X) is not list:
            raise DiveplaneError("Cases must be a list.")
    ...
    def _set_random_name(self, retries: int = RENAME_RETRIES):
        ...
        for _ in range(max(1, retries)):
            try:
                self.trainee_name = f'dp-estimator-{uuid.uuid4()}'
            except DiveplaneNotUniqueError as exception:
    ...
    def __getstate__(self) -> Dict:
        ...
        Additionally saves the model on the Diveplane server for later loading
        when unpickling.
    ...
    def __setstate__(self, state: Dict):
        ...
        Diveplane server to load the model.

        Parameters
        ----------
        state : dict
            The state of the DiveplaneEstimator.
        """
        for attr in state:
    ...
    def delete(self):
        """Delete this trainee from the diveplane cloud service."""
        if self.trainee_id:
            self.trainee.delete()
    ...
    def save(self) -> None:
        ...
        if self.persistence == 'never':
            raise DiveplaneError("The Diveplane estimator's trainee was not "
    ...
    def feature_add(self, feature: str = None,
        ...
        if feature is None:
            ...
            raise DiveplaneError(f"Feature name '{feature}' already exists in "
                                 f"this trainee.")
...
class DiveplaneClassifier(DiveplaneEstimator):
    """
    ...
    client_params : dict, default None
        The parameters with which to instantiate the client.
    trainee_params : dict, default None
        The parameters with which to instantiate the client. Intended for use by `DiveplaneEstimator.get_params`.
    """
    ...
    def __init__(self, client: Optional[AbstractDiveplaneClient] = None,
        ...
        super(DiveplaneClassifier, self).__init__(client=client,
                                                  features=features,
                                                  targets=targets,
    ...
    def partial_fit(self, X: np.ndarray, y: np.ndarray):
        ...
        y : numpy.ndarray, shape (n_samples,)
            Target. Will be cast to X's dtype if necessary
        """
        DiveplaneEstimator.partial_fit(self, X, y)
        self._populate_classes(y)
    ...
    def load(self, trainee_id: str):
        ...
        """
        DiveplaneEstimator.load(self, trainee_id)
        self.classes_ = np.empty((0,), dtype=str)
...
class DiveplaneRegressor(DiveplaneEstimator):
    """
    ...
    trainee_params : dict, default None
        ...
        by `DiveplaneEstimator.get_params`.
    """
    ...
    def __init__(self, client=None, features: Optional[Dict] = None,
        ...
        super(DiveplaneRegressor, self).__init__(client=client,
                                                 features=features,
                                                 targets=targets,
                                                 verbose=verbose, debug=debug,
