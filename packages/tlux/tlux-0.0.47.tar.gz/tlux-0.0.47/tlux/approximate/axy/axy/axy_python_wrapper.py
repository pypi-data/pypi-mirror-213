...
_ordered_dependencies = ['pcg32.f90', 'axy_random.f90', 'axy_matrix_operations.f90', 'axy_sort_and_select.f90', 'axy.f90', 'axy_c_wrapper.f90']
_symbol_files = []# 
# --------------------------------------------------------------------
#               AUTO-COMPILING
#
...
!  the aggregator and fixed inputs. Store in (2,_) arrays.'''
    ...
    def fetch_data(self, config, agg_iterators_in, ax_in, ax, axi_in, axi, sizes_in, sizes, x_in, x, xi_in, xi, y_in, y, yw_in, yw):
        ...
        # Return final results, 'INTENT(OUT)' arguments only.
        return config, agg_iterators_in, ax, axi, sizes, x, xi, y, yw, na.value, nm.value
...
!  gradient by aggregating over the repeated occurrences of the embedding.'''
    ...
    def basis_gradient(self, config, model, y, x, ax, sizes, m_states, a_states, ay, grad, nt, batcha_starts, batcha_ends, agg_starts, fix_starts, batchm_starts, batchm_ends):
...
!  (ensure that mean squared error is effectively reduced).'''
    ...
    def fit_check(self, config, model, rwork, iwork, lwork, ax_in, axi_in, sizes_in, x_in, xi_in, y_in, yw_in, yw, agg_iterators, steps, sum_squared_error, record=None):
...
! TODO: Take an output file name (STDERR and STDOUT are handled).'''
        ...
        if (type(sum_squared_error) is not ctypes.c_float): sum_squared_error = ctypes.c_float(sum_squared_error)
    ...
    def fit_model(self, config, model, rwork, iwork, lwork, ax_in, axi_in, sizes_in, x_in, xi_in, y_in, yw_in, yw, agg_iterators, steps, record=None, continuing=None):
