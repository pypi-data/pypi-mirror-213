...
class DecoratorBase(metaclass=DecoratorMeta):
    ...
    def wrapped(self):
        ...
        if self._wrapped is None:
            ...
            if self._obj:
                ...
                return wrap
            signature = inspect.Signature.from_callable(self._func)
            first_arg_name = next(iter(signature.parameters), None)
            if first_arg_name == "self":
                # Signature 1.3: _func is a method but _obj is None
                wrap = self._wrap_method(self._func)
            elif first_arg_name == "cls":
                # Signature x.3: _func is a classmethod
                wrap = self._wrap_method(self._func)
            else:
                # Signature x.1 / x.2
    ...
    def _wrap_method(self, func):
        """
        To implement for user. Function is method or classmethod (has one functional argument)
