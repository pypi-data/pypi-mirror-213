...
class EspressoWebSocket(object):
    ROOT_URI = "wss://streams.myespresso.com/espstream/api/stream"
    ...
    def __init__(self, access_token, api_key):
                try:
                    self.root = f"{self.ROOT_URI}?ACCESS_TOKEN={access_token}&API_KEY={api_key}"
                    self.access_token = access_token
                    self.api_key = api_key
                    print(self.root)
                    if self.api_key == None or self.access_token == None:
                        return "access_token and api_key is missing"
