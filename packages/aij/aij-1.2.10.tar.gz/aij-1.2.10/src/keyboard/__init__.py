...
class Keyboard():
    ...
    def fill(self):
        ...
        for x in range(rows):
            for y in range(cols):
                pos_x = x * Keyboard.KEYBOARD_KEY_SIZE[0] + Keyboard.KEYBOARD_START_POS[0]
                pos_y = y * Keyboard.KEYBOARD_KEY_SIZE[1] + 100 + Keyboard.KEYBOARD_START_POS[1]
                button_components.append(
                    Button((pos_x, pos_y), Keyboard.KEYBOARD_KEY_SIZE[0], Keyboard.KEYBOARD_KEY_SIZE[1], self.keys[x][y]))
