...
class DaskCalculator(Calculator[dd.DataFrame]):
    ...
    def join_data_frames(
        ...
        for cleft, cright in on:
            if left[cleft].dtype != right[cright].dtype:
                left[cleft] = left[cleft].astype(right[cright].dtype)
    ...
    def has_duplicates(self, key_columns: List[str], dataframe: dd.DataFrame) -> bool:
        """
        ...
        df[temp_col_name] = df.apply(
            lambda x: tuple(x[col] for col in key_columns), axis=1, meta=pd.DataFrame
        )
        df = df[temp_col_name].value_counts().reset_index(drop=False)
        df = df[df[temp_col_name] > 1]
        return df.size.compute() > 0
