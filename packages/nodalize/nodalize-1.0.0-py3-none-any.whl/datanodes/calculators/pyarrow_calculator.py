...
class PyarrowCalculator(Calculator[pa.Table]):
    ...
    def _join_non_empty_tables(
        self, left: pa.Table, right: pa.Table, how: str, on: List[Tuple[str, str]]
    ) -> pa.Table:
        """
        ...
        for cleft, cright in on:
            if left[cleft].type != right[cright].type:
                left = self.add_column(
                    left, cleft, pc.cast(left[cleft], right[cright].type)
                )

        joined_table = left.join(
            right,
            join_type=join_type,
            keys=[p[0] for p in on],
            right_keys=[p[1] for p in on],
            left_suffix="",
            right_suffix="_right",
            coalesce_keys=False,
        )
        ...
        for k1, k2 in on:
            if k1 == k2:
                new_col = pa.compute.coalesce(
                    joined_table[f"{k1}"], joined_table[f"{k1}_right"]
                )
                joined_table = joined_table.drop([k1, f"{k1}_right"])
                joined_table = joined_table.append_column(k1, new_col)
    ...
    def has_duplicates(self, key_columns: List[str], dataframe: pa.Table) -> bool:
        """
        ...
        temp_col_name = generate_possible_name(
            lambda n: n not in key_columns and f"{n}_count" not in key_columns
        )
        count_col_name = f"{temp_col_name}_count"

        df = df.append_column(temp_col_name, pa.array([1] * len(df)))
        df = df.group_by(key_columns).aggregate([(temp_col_name, "count")])
        df = df.filter(pa.compute.greater(df[count_col_name], 1))
        return len(df) > 0
    ...
    def has_nulls(self, column_names: List[str], dataframe: pa.Table) -> bool:
        """
        ...
        for col in column_names:
            nulls = pc.is_null(dataframe[col])
            if pc.any(nulls).as_py():
                return True
    ...
    def compute_correlations(
        ...
        """
        # Note: PyArrow does not offer any function to compute correlation or even covariance.
        # The simplest way then is to use Pandas.
        pandas_calc = PandasCalculator(self.app_name)
        return pandas_calc.compute_correlations(
            key_columns, value_columns, dataframe1.to_pandas(), dataframe2.to_pandas()
        )
