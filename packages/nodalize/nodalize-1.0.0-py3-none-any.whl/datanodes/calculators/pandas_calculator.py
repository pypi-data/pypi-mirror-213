...
class PandasCalculator(Calculator[pd.DataFrame]):
    ...
    def join_data_frames(
        ...
        for cleft, cright in on:
            if left[cleft].dtype != right[cright].dtype:
                left[cleft] = left[cleft].astype(right[cright].dtype)  # type: ignore
    ...
    def has_duplicates(self, key_columns: List[str], dataframe: pd.DataFrame) -> bool:
        """
        ...
        Returns:
            bool
        """
        df = dataframe[key_columns]
        duplicates = df.duplicated()
        return True in duplicates.values
    ...
    def has_nulls(self, column_names: List[str], dataframe: pd.DataFrame) -> bool:
        """
        ...
        return cast(bool, dataframe.isnull().values.any())
    ...
    def compute_correlations(
        ...
        for col in value_columns:
            ...
            if pd.api.types.is_string_dtype(merged_data[col_left]):
                match = (merged_data[col_left] == merged_data[col_right]) | (
                    (merged_data[col_left] != merged_data[col_left])
                    & (merged_data[col_right] != merged_data[col_right])
                )
                correl = match.sum() / len(merged_data)
            else:
                df = merged_data[[col_left, col_right]].corr()
                correl = df[col_left][col_right]
