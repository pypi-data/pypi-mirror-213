...
class SparkCalculator(Calculator[DataFrame]):
    ...
    def join_data_frames(
        self, left: DataFrame, right: DataFrame, how: str, on: List[Tuple[str, str]]
    ) -> DataFrame:
        """
        Join 2 data frames.
        ...
        for c in common_columns:
            right = right.withColumnRenamed(c, f"{c}_right")

        join_clauses = []
        for k1, k2 in on:
            if k1 == k2:
                join_clauses.append(
                    f.col(k1).eqNullSafe(f.col(f"{k1}_right"))  # type: ignore
                )
            else:
                join_clauses.append(f.col(k1).eqNullSafe(f.col(k2)))  # type: ignore

        join_on = join_clauses[0]
        for clause in join_clauses[1:]:
            join_on = join_on & clause

        joined_df = left.join(right, how=how, on=join_on)

        for k1, k2 in on:
            if k1 == k2:
                joined_df = joined_df.withColumn(k1, f.coalesce(k1, f"{k1}_right"))
                joined_df = joined_df.drop(f"{k1}_right")
    ...
    def has_duplicates(self, key_columns: List[str], dataframe: DataFrame) -> bool:
        """
        ...
        Returns:
            bool
        """
        df = dataframe.select(key_columns)  # type: ignore
        ...
        df = df.withColumn(temp_col_name, f.lit(1))
        df = df.groupBy(key_columns).agg(f.count(temp_col_name).alias(count_col_name))  # type: ignore
        df = df.filter(f.col(count_col_name) > 1)  # type: ignore
        return df.count() > 0
    ...
    def has_nulls(self, column_names: List[str], dataframe: DataFrame) -> bool:
        """
        ...
        for col in column_names:
            if dataframe.filter(f.col(col).isNull() | f.isnan(f.col(col))).count() > 0:
                return True
    ...
    def compute_correlations(
        ...
        dataframe1 = dataframe1.select(all_columns)  # type: ignore
        dataframe2 = dataframe2.select(all_columns)  # type: ignore
        merged_data = self.join_data_frames(
            dataframe1, dataframe2, "outer", [(c, c) for c in key_columns]
        )

        correlations = {}  # type: Dict[str, float]
        for col in value_columns:
            col_left = col
            col_right = f"{col}_right"

            if merged_data.schema[col].dataType == StringType():
                match = merged_data.filter(
                    f.col(col).eqNullSafe(f.col(col_right))  # type: ignore
                ).count()
                correl = match / merged_data.count()
            else:
                filtered = merged_data.filter(
                    f.col(col_left).isNotNull() & f.col(col_right).isNotNull()
                )
                correl = filtered.stat.corr(col_left, col_right)
