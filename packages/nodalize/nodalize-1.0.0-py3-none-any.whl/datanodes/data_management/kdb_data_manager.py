...
class KdbDataManager(DatabaseDataManager):
    ...
    def _build_base_load_query(
        ...
        Args:
            ...
            datetime_column_name: name of the inserted datetime column
            schema: table schema
        ...
        return (
            ...
            f"{datetime_column_name}=(max;{datetime_column_name}) fby ([]{key_list})"
        )
    ...
    def load_data_from_database(
        ...
        Args:
            ...
            table_name: table name
            datetime_column_name: name of the inserted datetime column
            batch_id_column_name: name of the batch id column
            schema: table schema
            columns: optional list of columns to load, if None then load all
        ...
        Returns:
             data frame (actual class type depends on type of calculator)
        """
        query = self._build_base_load_query(
            table_name, datetime_column_name, schema, columns
        )

        kdb_filters = ""
        if batch_ids is not None:
            batch_id_list = ",".join([str(b) for b in batch_ids])
            kdb_filters += f", {batch_id_column_name} in ({batch_id_list})"
    ...
    def load_data_frame_with_lookback(
        ...
        Args:
            calculator: Calculator
            table_name: table name
            date_column: name of the date column on which to apply the lookback
            datetime_column_name: name of the inserted datetime column
            batch_id_column_name: name of the batch id column
        ...
        kdb_filters = f", {date_column}>={start_date},{date_column}<={end_date}"
        ...
        if group_lookback or not any(
            ...
            for name, (_, cat) in schema.items()
            if cat in [ColumnCategory.KEY, ColumnCategory.PARAMETER]
            and name != date_column
        ):
            query = f"select from ({query}) where {date_column}=max {date_column}"
        else:
            key_list = self._build_query_key_list(schema, [date_column])
            query = f"select from ({query}) where {date_column}=(max;{date_column}) fby ([]{key_list})"

        pandas_df = self.execute_query(query, read_only=True, use_pandas=True)
        pandas_df = self._parse_results(schema, pandas_df)
        pandas_df[date_column] = loading_date
        return calculator.from_pandas(pandas_df)
