...
class DeltaLakeDataManager(DatabaseDataManager):
    ...
    def load_data_from_database(
        ...
        if batch_ids is not None:
            if filters is None:
                filters = [[(batch_id_column_name, "in", batch_ids)]]
            else:
                for sub_filter in filters:
                    sub_filter.append((batch_id_column_name, "in", batch_ids))
        ...
        if calculator.calc_type == "spark":
            ...
            df = calculator.filter_in_max_values(df, [datetime_column_name], keys)
            ...
            if columns is not None:
                column_pre_filter = columns + [datetime_column_name] + keys

                if batch_ids is not None:
                    column_pre_filter.append(batch_id_column_name)
