"""Generic class to manage data files."""
...
class LocalFileDataManager(DataManager):
    ...
    def __init__(self, database_location: str) -> None:
        ...
        Args:
            database_location: root folder of the database
        """
    ...
    def _populate_files_to_load(
        ...
        if batch_ids is not None:
            ...
            folder = os.path.join(self._database_location, table_name)
            if os.path.isdir(folder):
                return [
                    os.path.join(folder, sub)
                    for sub in os.listdir(folder)
                    if sub.endswith(".parquet")
                ]
            else:
                return []
