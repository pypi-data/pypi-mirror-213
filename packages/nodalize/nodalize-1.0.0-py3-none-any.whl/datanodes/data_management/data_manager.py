...
class DataManager(ABC):
    ...
    def load_data_frame_with_lookback(
        ...
        Args:
            calculator: Calculator
            table_name: table name
            date_column_name: name of the date column on which to apply the lookback
            datetime_column_name: name of the inserted datetime column
            batch_id_column_name: name of the batch id column
        ...
        if len(final_filters) == 1:
            ...
            final_filters[0].append((date_column_name, ">=", start_date))
            final_filters[0].append((date_column_name, "<=", end_date))
        ...
        if group_lookback or len(key_columns_ex_date) == 0:
            ...
            max_date = max(
                calculator.extract_unique_column_values(date_column_name, parent_df)
            )
            parent_df = calculator.apply_filter(
                parent_df,
                (date_column_name, "=", max_date),
            ...
            if batch_ids is not None:
                ...
                parent_df = calculator.join_data_frames(
                    parent_df,
                    keys_df,
                    "left",
                    [(c, c) for c in key_columns_ex_date],
                )
                parent_df = calculator.apply_filter(
                    parent_df,
                    (dummy_col, "=", 1),
        ...
        parent_df = calculator.add_column(
            parent_df, date_column_name, loading_date, literal=True
        )
