    ...
    def load_data_from_database(
        ...
        if batch_ids is not None:
            if filters is None:
                ...
                for f in filters:
                    f.append((batch_id_column_name, "in", batch_ids))
        ...
        joins.append(f"A.{datetime_column_name}=B.{max_datetime_col_name}")
        ...
        query = f"""
WITH [{cte_name}] AS (
    SELECT {column_list}, MAX({datetime_column_name}) AS {max_datetime_col_name}
    FROM [{table_name}]
