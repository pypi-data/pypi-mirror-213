...
class DataNode:
    ...
    @property
    def batch_id_column(self):
        """
        Return name of batch id column.

        Returns:
            column name
        """
        return column_names.BATCH_ID
    ...
    def date_column(self):
        ...
        Returns:
            column name
        """
        return column_names.DATA_DATE

    @property
    def inserted_datetime_column(self):
        """
        Return name of inserted datetime column.

        Returns:
            column name
        """
        return column_names.INSERTED_DATETIME
    ...
    def get_enriched_schema(self) -> Dict[str, Tuple[type, ColumnCategory]]:
        ...
        if self._enriched_schema is None:
            ...
            enriched_schema[self.date_column] = (date, ColumnCategory.PARAMETER)
            enriched_schema[self.inserted_datetime_column] = (
                int,
                ColumnCategory.GENERIC,
            )
            enriched_schema[self.batch_id_column] = (int, ColumnCategory.GENERIC)
    ...
    def column(self, df: Any, column_name: str) -> Any:
        """
        Get column object from data frame.
        ...
        Returns:
            column
        """
        return self.calculator.get_column(df, column_name)
    ...
    def add(
        ...
        return self.calculator.add_column(
            dataframe, column_name, value, literal=literal, override=override
        )
    ...
    def join(
        ...
        left: Any,
        right: Any,
        on: Union[str, Tuple[str, str], List[Tuple[str, str]]],
        how: str = "outer",
    ) -> Any:
        """
        Join 2 data frames.
        ...
        if isinstance(on, str):
            ...
            join_on = [(j, j) if isinstance(j, str) else j for j in on]
        else:
            raise AssertionError(f"Invalid object for 'on' parameter: {on}")

        return self.calculator.join_data_frames(left, right, how, join_on)
    ...
    def load_and_compute(
        ...
        data = self.calculator.add_column(
            data, self.batch_id_column, batch_id, override=False, literal=True
        )
