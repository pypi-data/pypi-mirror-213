...
from getajob.exceptions import KafkaEventTopicNotProvidedError
...
from .models import EntityModels, PaginatedResponse, BaseDataModel
...
class BaseRepository:
    ...
    def _produce_repository_kafka_event(
        self, event_type: KafkaEventType, object_id: str, data: BaseModel | None = None
    ):
        if (
            not self.kafka
    ...
    def batch_create(
        ...
        batch_action = [
            FirestoreBatchAction(
                action=FirestoreBatchActionType.CREATE,
                parent_collections=parent_collections,
                collection_name=self.collection_name,
                document_data=item.dict(),
                document_id=generate_random_short_code(),
            )
            for item in data
        ]
    ...
    def batch_update(
        ...
        batch_action = [
            FirestoreBatchAction(
                action=FirestoreBatchActionType.UPDATE,
                parent_collections=parent_collections,
                collection_name=self.collection_name,
                document_data=item.dict(),
                document_id=item.id,
    ...
    def batch_delete(
        ...
        batch_action = [
            FirestoreBatchAction(
                action=FirestoreBatchActionType.DELETE,
                parent_collections=parent_collections,
                collection_name=self.collection_name,
                document_data=item.dict(),
                document_id=item.id,
