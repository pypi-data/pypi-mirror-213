...
class ActionFilter(object):
    ...
    def init_history_with_motion(self, x):
        """
        x should be numpy with (N, n_joint) shape
        """
        num_motion = x.shape[0]
        for i in range(self.hist_len):
            rev_idx = min(0, num_motion - 1 - i)
            self.xhist[i] = x[rev_idx].reshape((self.num_joints, 1)).copy()
            self.yhist[i] = x[rev_idx].reshape((self.num_joints, 1)).copy()
