...
def get_journey_list(ps_df):
    ...
    #check if only to return data for specific journeys
    if apply_ps_filters == 'yes':
        journey_df = journey_df[journey_df['journey_name'].str.lower().str.contains(journeys_to_include, regex=True)]
    
    #extract segment population and query rule for journey stage Entry Criteria
    ...
    final_df['stage_rule'] = journey_df.apply(lambda row: f"SELECT cdp_customer_id FROM cdp_audience_{str(row['audience_id'])}.journey_{str(row['journey_id'])} WHERE intime_stage_{str(row['stage_idx'])} IS NOT NULL AND outtime_stage_{str(row['stage_idx'])} IS NULL", axis=1)
