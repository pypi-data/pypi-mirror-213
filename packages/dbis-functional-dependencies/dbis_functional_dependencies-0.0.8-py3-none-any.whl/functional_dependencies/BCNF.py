        ...
        for keys in self.attribute_combinations(self.attributes):
            ...
            if(closure == self.attributes):
                ...
                if not any([x.issubset(k) for x in ans]):
                    generatedTasks.append((f"Es ist {k} ein Schlüsselkandidat. Also ist {self.attributes} voll funktional abhängig von {k}.", True))
                    generatedTasks.append((f"Die Attributhülle von {k} ist {self.attributes}. Aus diesem Grund ist {self.attributes} voll funktional abhängig von {k}.", False))

                    if verbose:
                        print(f"found candidate keys {k}")
                    ans.append(k)
                else:
                    generatedTasks.append((f"Es ist {k} ein Superschlüssel, aber kein Schlüsselkandidat.", True))
                    generatedTasks.append((f"Es ist {k} ein Superschlüssel. Also ist {self.attributes} voll funktional abhängig von {k}.", False))
            else:
                generatedTasks.append((f"Es ist {k} weder Superschlüssel noch Schlüsselkandidat.", True))
                if len(k) <= 2:
                    generatedTasks.append((f"Aus {k} leitet man {closure} ab.", True))

        if genEx:
            print(f"=======================================")
            print(f"Aussagen zum Finden von Schlüsselkandidaten und Berechnen von Attributhüllen:")
    ...
    def decompose2(self, verbose:bool=False, genEx:bool=False):
        ...
        while not_bcnf:
            ...
            if fds.isBCNF(verbose=verbose):
                generatedTasks.append((f"Die Abhängigkeitsmenge {fds} ist in BCNF.", True))
                bcnf.append(fds.attributes)
                if verbose:
                    print(f"fdset {fds} is in BCNF")
            else:
                generatedTasks.append((f"Die Abhängigkeitsmenge {fds} ist in BCNF.", False))
                if verbose:
                ...
                for dep in fds.dependencies:
                    left, right = dep
                    if (fds.get_attr_closure(left) != fds.attributes) and (left != right) and (len(left.intersection(right)) == 0):
                        step += 1
                        generatedTasks.append((f"In Schritt {step} zerlegen wir entlang der Abhängigkeit {dep}.", True))
                        if verbose:
                        ...
                        generatedTasks.append((f"Wir zerlegen entlang der Abhängigkeit {dep}.", False))
                not_bcnf.append(new_fds1)
                not_bcnf.append(new_fds2)
                if verbose:
                    print(f"new fdsets with attributes {new_fds1.attributes} and {new_fds2.attributes}")
        self.tables = bcnf

        generatedTasks.append((f"Die Zerlegung enthält mehr als drei Relationen.", len(self.tables) > 3))
        ...
        for dep in self.dependencies:
            ...
            for attr_set in self.tables:
                ...
                if newset.issubset(attr_set) and newset != attr_set:
                    if verbose:
                        print(f"splitting {attr_set} into {attr_set.difference(right)} and {newset}")
...
'''
        ...
        while remaining_deps:
            ...
            for attr in sorted(pre):
                if post <= self.get_attr_closure(pre - {attr}):
                    generatedTasks.append((f"Das Attribut {attr} kann aus der Abhängigkeit {dep} reduziert werden.", True))
                    if not wasDepAlreadyReduced:
                        generatedTasks.append((f"Die Abhängigkeit {self.stringify_dependencies(fdsToStringify=[dep])} kann nicht reduziert werden, da die Attributhülle von {Set.stringify_set(post, self.notation)} nicht alle Attribute aus {Set.stringify_set(pre, self.notation)} enthält.", False))
                        generatedTasks.append((f"Die Abhängigkeit {self.stringify_dependencies(fdsToStringify=[dep])} kann reduziert werden, da die Attributhülle von {Set.stringify_set(post, self.notation)} nicht alle Attribute aus {Set.stringify_set(pre, self.notation)} enthält.", False))
                        reductionCounter += 1
                    else:
                        generatedTasks.append((f"Die Abhängigkeit {self.stringify_dependencies(fdsToStringify=[dep])} wird mehrmals reduziert.", True))
                    if verbose:
                        print(f"removed {attr} from lhs of dependency {dep}, new lhs {(pre - {attr})}")
                    self.remove_dependency(pre, post)
                    self.add_dependency(pre - {attr}, post)
                    pre = pre - {attr}
                    wasDepAlreadyReduced = True
                else:
                    generatedTasks.append((f"Das Attribut {attr} kann aus der Abhängigkeit {self.stringify_dependencies(fdsToStringify=[dep])} reduziert werden.", False))
            if not wasDepAlreadyReduced:
                generatedTasks.append((f"Die Abhängigkeit {self.stringify_dependencies(fdsToStringify=[dep])} kann nicht reduziert werden.", True))
            else:
                generatedTasks.append((f"Die Linksreduktion beinhaltet die Abhängigkeit {self.stringify_dependencies(fdsToStringify=[dep])}.", True))

        generatedTasks.append((f"Es kann nur eine der ursprünglichen funktionalen Abhängigkeiten reduziert werden.", reductionCounter == 1))
        generatedTasks.append((f"Es können mehr als zwei der ursprünglichen funktionalen Abhängigkeiten reduziert werden.", reductionCounter > 2))
    ...
    def isCorrectLeftReduction(self, proposedReduction, verbose:bool=False):
        ...
        if len(proposedReduction.dependencies) != len(self.dependencies):
            if verbose:
                print(f"FAILURE: The solution had a differing amount of dependencies: \n \t Task: {len(self.dependencies)}\n\tSolution: {len(proposedReduction.dependencies)}")
            return False
        if verbose:
            print(f"Both the task and the solution have {len(self.dependencies)} dependencies\n")
            print(f"2. Checking whether the solution dependencies could result from a task dependency:")
        ...
        for propDep in proposedReduction.dependencies:
            ...
            if not leftProp:
                if verbose:
                    print(f"FAIL: One of the dependencies has an empty left side. This cannot happen.")
                return False
            isCorrect = False
            for oriDep in self.dependencies:
                leftOri, rightOri = oriDep
                if verbose:
                    print(f"\tComparing solution dependency {leftProp} -> {rightProp} with task dependency {leftOri} -> {rightOri}")
                if rightProp == rightOri and leftProp <= leftOri:
                    isCorrect = True
                    if verbose:
                        print(f"\tThe solution dependency can result from the task dependency")
                    break
            if not isCorrect:
                if verbose:
                    print(f"FAILURE: The dependency {leftProp} -> {rightProp} can't result from any task dependency")
                return False
        if verbose:
            print(f"All solution dependencies can result from the task dependencies\n")
            print(f"3. Checking whether the closures of all attribute subsets have remained the same:")
    ...
    def isAllAttributeClosuresSame(self, fdsetToCompare, verbose:bool=False):
        ...
        for comb in combs:
            ...
            if self.get_attr_closure(comb) != fdsetToCompare.get_attr_closure(comb):
                if verbose:
                    print(f"FAILURE: The closure for the subset {set(comb)} changed from {self.get_attr_closure(comb)} to {fdsetToCompare.get_attr_closure(comb)}")
                return False
        if (verbose):
        ...
        while remaining_deps:
            ...
            for attr in sorted(post):
                ...
                if {attr} <= set(self.get_attr_closure(pre)):
                    if not wasDepAlreadyReduced:
                        generatedTasks.append((f"Die Abhängigkeit {self.stringify_dependencies(fdsToStringify=[dep])} kann reduziert werden, da die Attributhülle von {post} nicht alle Attribute aus {pre} enthält.", False))
                        generatedTasks.append((f"Das Attribut {attr} kann aus der Abhängigkeit {self.stringify_dependencies(fdsToStringify=[dep])} reduziert werden.", True))
                        generatedTasks.append((f"Das Attribut {attr} kann aus der Abhängigkeit {self.stringify_dependencies(fdsToStringify=[dep])} reduziert werden, da bei Entfernen von {attr} von der rechten Seite der Abhängigkeit die Attributhülle von {pre} immer noch {attr} enthält.", True))
                        generatedTasks.append((f"Das Attribut {attr} kann aus der Abhängigkeit {self.stringify_dependencies(fdsToStringify=[dep])} reduziert werden, da bei Entfernen von {attr} von der rechten Seite der Abhängigkeit die Attributhülle von {post - {attr}} immer noch {attr} enthält.", True))
                        reductionCounter += 1
                    else:
                        generatedTasks.append((f"Die Abhängigkeit {self.stringify_dependencies(fdsToStringify=[dep])} kann mehrmals reduziert werden.", True))
                    if verbose:
                        print(f"removed {attr} from rhs of dependency {dep}, new rhs {(post - {attr})}")
                    post = post - {attr}
                    wasDepAlreadyReduced = True
                else:
                    generatedTasks.append((f"Das Attribut {attr} kann aus der Abhängigkeit {self.stringify_dependencies(fdsToStringify=[dep])} reduziert werden.", False))
            ...
            if wasDepAlreadyReduced:
                generatedTasks.append((f"Die Abhängigkeit {self.stringify_dependencies(fdsToStringify=[dep])} wird insgesamt zu {self.stringify_dependencies(fdsToStringify=[(pre, post)])} reduziert.", True))
            else:
                generatedTasks.append((f"Die Abhängigkeit {self.stringify_dependencies(fdsToStringify=[dep])} kann nicht reduziert werden.", True))
    ...
    def remove_empty_fds(self, verbose:bool=False, genEx:bool=False):
        ...
        if genEx:
            ...
            print((f"Es werden zwei Abhängigkeiten entfernt, da sie eine leere rechte Seite haben.", removeCounter == 2))
            print((f"Es werden zwei Abhängigkeiten entfernt, da sie eine leere linke Seite haben.", False))
            print((f"Es werden mehr als zwei Abhängigkeiten entfernt.", removeCounter > 2))
            print(f"=======================================")
    ...
    def isCorrectRemovingEmptyFDs(self, proposedSolution, verbose: bool=False):
        ...
        '''
        if verbose:
            print("CHECKING: Is the proposed removal of empty dependencies of self correct:\n")
            print("1. Does the solution have an equal or less amount of dependencies than self:")
        
        # Equal amount of dependencies
        if len(proposedSolution.dependencies) <= len(self.dependencies):
            if verbose:
                print(f"FAILURE: The solution had more dependencies: \n \t Task: {len(self.dependencies)}\n\tSolution: {len(proposedSolution.dependencies)}")
        if verbose:
            print(f"The proposed solution has equal or less dependencies ({len(proposedSolution.dependencies)}) than self ({len(self.dependencies)})\n")
            print(f"2. Checking if every complete dependency in self is in the proposed solution:")
        ...
        for dep in self.dependencies:
            ...
            if right != Set() and (left, right) not in proposedSolution.dependencies:
                if verbose:
                    print(f"FAILURE: Dependency {left} -> {right} is not in the proposed solution.")
                    return False
            elif verbose:
        ...
        while self.dependencies:
            ...
            for dep in deps_copy:
                ...
                if left == pre:
                    ...
                    if verbose:
                        print(f"combined dependencies {dep} and {(pre, post)} to new dependency {(pre, new_post)}")
                    self.remove_dependency(left, right)
                    generatedTasks.append((f"Die Abhängigkeiten {self.stringify_dependencies(fdsToStringify=[dep])} und {self.stringify_dependencies(fdsToStringify=[(pre, post)])} werden zu {self.stringify_dependencies(fdsToStringify=[(pre, new_post)])} zusammengefasst.", True))
                    generatedTasks.append((f"Die Abhängigkeiten {self.stringify_dependencies(fdsToStringify=[dep])} und {self.stringify_dependencies(fdsToStringify=[(pre, new_post)])} werden zu {self.stringify_dependencies(fdsToStringify=[(pre, post)])} zusammengefasst.", False))
            combined_dependencies.append((pre, new_post))
        self.dependencies = combined_dependencies
        generatedTasks.append((f"Es werden an zwei oder mehr Stellen Abhängigkeiten zusammengefasst.", combineCounter >= 2))

        for dep in self.dependencies:
            pre, post = dep
            generatedTasks.append((f"Die kanonische Überdeckung enthält die funktionale Abhängigkeit {self.stringify_dependencies(fdsToStringify=[dep])}.", True))
            for attr in post:
                generatedTasks.append((f"Die kanonische Überdeckung enthält die funktionale Abhängigkeit {self.stringify_dependencies(fdsToStringify=[(pre, post - {attr})])}.", False))
    ...
    def isCorrectCombinationOfDependencies(self, proposedSolution, verbose: bool=False):
        '''
        Checks a given removing of empty fds of self for correctness
        ...
        '''
        if verbose:
            print("CHECKING: Is the proposed combination of dependencies of self correct:\n")
            print("1. Does the solution have an equal or less amount of dependencies than self:")
        ...
        if verbose:
            print(f"The proposed solution has equal or less dependencies ({len(proposedSolution.dependencies)}) than self ({len(self.dependencies)})\n")
            print(f"2. Checking if every combined dependency in self is in the proposed solution:")
        ...
        for dep in proposedSolution.dependencies:
            ...
            for leftDep in leftDeps:
                if leftDep[1] <= originalRight:
                    ...
                    if verbose:
                        print(f"FAILURE: Dependecy {left} -> {leftDep[1]} is not part of the combined dependency {left} -> {right}.")
                    return False
                if verbose:
                    print(f"\tThe dependency {left} -> {right} cannot be combined further.")
            if len(right) > 0:
                if verbose: 
                    print(f"FAILURE: Right side {dep[1]} contains too many attributes {right}.")
                return False
    ...
    def isCorrectCanonicalCover(self, proposedSolution, verbose:bool=False):
        ...
        if not proposedSolution.isAllAttributeClosuresSame(self, verbose=verbose):
            if verbose:
                print("FAILURE: The closures of self and proposedSolution are different.")
        if verbose:
    ...
    def create_new_fdsets(self, verbose:bool=False, genEx:bool=False):
        ...
        while deps:
            ...
            generatedTasks.append((f"Es wird eine neue Relation mit Attributmenge {new_attributes} erstellt.", True))
            i += 1
            for dep in new_deps:
                left,right=dep
            ...
            if len(fds.dependencies) > 1:
                ...
                generatedTasks.append((f"Es gibt eine Abhängigkeit, die in mehreren Relationen enthalten ist.", True))
            new_fdsets.append(fds)
        
        if existsDepWithMultipleFDs:
            generatedTasks.append((f"Es gibt eine Abhängigkeit, die in mehreren Relationen enthalten ist.", True))
        else:
            generatedTasks.append((f"Es gibt eine Abhängigkeit, die in mehreren Relationen enthalten ist.", False))


        generatedTasks.append((f"Es wird eine neue Relation pro Abhängigkeit in der kanonischen Überdeckung erstellt.", True))
        generatedTasks.append((f"Es wird eine neue Relation pro Abhängigkeit in der ursprünglichen Menge funktionaler Abhängigkeiten erstellt.", False))
        generatedTasks.append((f"Es wird für jeden Schlüsselkandidaten eine neue Relation erstellt.", False))
        generatedTasks.append((f"Es werden insgesamt drei neue Relationen erstellt.", len(new_fdsets) == 3))
        generatedTasks.append((f"Die Abhängigkeit {self.stringify_dependencies(fdsToStringify=[fds.dependencies[0]])} ist in keiner neuen Relation enthalten.", False))
    ...
    def isCorrectCreationOfNewFDS(self, fdsets:list, verbose:bool=False):
        ...
        if len(fdsets) > len(self.dependencies):
            if verbose:
                print(f"FAILURE: The number of fdsets, {len(fdsets)}, if bigger than the number of dependencies, {len(self.dependencies)}.")
        ...
        for fds in fdsets:
            for dep in fds.dependencies:
                ...
                if not left | right <= fds.attributes:
                    if verbose: 
                        print(f"FAILURE: The dependency, {left} -> {right}, has more attributes than {fds.attributes}")
                    return False
                if (left, right) in self.dependencies:
                    ...
                    if verbose: 
                        print(f"FAILURE: The dependency, {left} -> {right}, is not in self.dependencies.")
                    return False
        if numpy.prod(depsCopy) != 1:
            if verbose:
                print(f"FAILURE: {len(depsCopy)} dependencies where not part of the solution.")
            return False
        if verbose:
    ...
    def synthesize(self, verbose:bool=False, genEx:bool=False):
        ...
        generatedTasks.append((f"Wir erhalten zum Schluss die gleiche Anzahl an Relationen wie vor Schritt 3.", len(fdsets_before_key_relation) == len(reduced_fdsets)))
    ...
    def create_optional_key_scheme(self, keys, fdsets, verbose:bool=False, genEx:bool=False):
        '''
        ...
        for key in keys:
            for fds in fdsets:
                if set(key) <= fds.attributes:
                    generatedTasks.append((f"Es braucht keine Schlüsselrelation hinzugefügt werden, da die Attribute von mindestens einem der {len(key) + 1} Schlüsselkandidaten in mindestens einer neuen Relation enthalten sind.", False))
                    generatedTasks.append((f"Wir können eine neue Schlüsselrelation mit der Attributmenge {set(keys[0])} hinzufügen.", False))
                    generatedTasks.append((f"Die Relation mit Attributmenge {fds.attributes} enthält alle Attribute eines Schlüsselkandidaten.", True))
                    if genEx:
                        print(f"=======================================")
                        ...
                        print(f"=======================================")
                    return fdsets
        generatedTasks.append((f"Es muss eine neue Schlüsselrelation hinzugefügt werden, da keine aktuelle Relation die Attribute eines Schlüsselkandidaten enthält.", True))
        generatedTasks.append((f"Die hinzuzufügende Schlüsselrelation ist in der Attributmenge eindeutig.", not (len(keys) > 1)))
        key = set(keys[0])
        generatedTasks.append((f"Wir können eine neue Schlüsselrelation mit der Attributmenge {key} hinzufügen.", True))
    ...
    def remove_subset_relations(self, fdsets, verbose:bool=False, genEx:bool=False):
        '''
        ...
        for fds in fdsets.copy():
            ...
            if conflict is not None:
                fdsets.remove(fds)
                removeCounter += 1
                generatedTasks.append((f"Die Relation mit Attributmenge {fds.attributes} wird entfernt.", True))
                if len(fds.dependencies) > 0:
                    generatedTasks.append((f"Es muss die Relation entfernt werden, die die Abhängigkeit {fds.dependencies[0]} enthält.", True))
                ...
                generatedTasks.append((f"Die Relation mit Attributmenge {fds.attributes} wird entfernt.", False))
        
        generatedTasks.append((f"Es gibt Relationen, deren Attributmengen Teilmengen voneinander sind.", removeCounter > 0))
