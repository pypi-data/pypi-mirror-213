...
class PlotterAnalysis:
    ...
    def _U_gnd_turns(self, mat_file_obj, order_list):
        ...
        Umax_gnd_half_turns = np.abs(Uground_half_turns).max(axis=0)
        plot_x = []
    ...
    def plot_selected_vs_el_order(self, use_markers=False, save=False, print_only=False):
            ...
            for value, value_dict in magnet_data.items():
                for sim_nr, sim_data in value_dict.items():
                    ...
                    for key, plot_x in self.plot_x_el_order[magnet_label]['plot_x'][sim_nr].items():
                        ax.plot(plot_x, self.data[magnet_label][value][sim_nr][key], markevery=20, marker=marker, label=f'{label}-{key}')
                    i += 1
...
if __name__ == "__main__":
    ...
    for model_names, model_labels in zip(model_names_list, model_labels_list):
        pa.read_data(model_names, model_labels, sim_nr_list_of_lists, values, el_order_lookup)
    #     #pa.plot_selected_vs_time(use_markers=0, save=0)
        pa.plot_selected_vs_el_order(use_markers=0, save=0)
