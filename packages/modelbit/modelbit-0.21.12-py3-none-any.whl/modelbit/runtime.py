...
class Runtime:
    ...
    resp = getJsonOrPrintError(
        "jupyter/v1/runtimes/create", {
            "runtime": {
                ...
                "dataFiles": dataFiles,
                "pyState": {
                    "sourceFile": makeSourceFile(rtProps, DefaultModuleName).asDict(),
                    "valueFiles": {},
                    "name": rtProps.name,
  ...
  def _choosePackagesIfNotSpecified(self):
    ...
    if self._systemPackages is None:
      self.set_system_packages(systemPackagesForPips(self._pythonPackages))
