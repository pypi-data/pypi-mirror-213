...
def _fn_fetch(args: Args) -> None:
    ...
    fetch_job_artifacts(
        ...
        params_no_check=args.params_no_check
        and {k: v for p in (args.params_no_check) for k, v in p.items()},
        dependency_paths=list(filter(bool, args.dependency_paths))
            ...
            return Build(
                client.get_build_info(
                    ...
                    build_id_from_queue_item(
                        client,
                        client.build_job(
                            job_full_path, {**(params or {}), **(params_no_check or {})}
                        ),
                    ),
                )
            )
