...
class World:
    ...
    def create_occlusions(self, entropy: float, max_walk_length: int = 1, tries: int = 100):
        ...
        def is_connected(cells, connection_pattern, cellmap: Cell_map, source: int, destination: int, visited=[]):
            ...
            for c in connection_pattern:
                ...
                if i >= 0 and i not in visited:
                    if is_connected(cells, connection_pattern, cellmap, i, destination, visited + [i]) :
                        return True
            return False
        ...
        for t in range(tries):
            ...
            if self.is_connected(self.cells,self.configuration.connection_pattern,m,0,len(self.cells)-1):
                break
        return occlusions
