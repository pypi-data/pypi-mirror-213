...
class World:
    ...
    def create_occlusions(self, entropy: float = -1, occlusion_count: int = -1, max_walk_length: int = 1, tries: int = 100, verbose: bool = False):
        ...
        for t in range(tries):
            ...
            while len(self.cells.occluded_cells()) < occlusion_count:
                if walk == 0:
            ...
            visited = Visited()
            if is_connected(self.cells, self.configuration.connection_pattern, m, 0, len(self.cells)-1, visited) and len(visited.cells) == len(self.cells.free_cells()):
                if verbose:
                    print("Success")
                return occlusions
            else:
