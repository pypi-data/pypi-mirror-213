...
class World:
    ...
    def create_occlusions(self, entropy: float = -1, occlusion_count: int = -1, max_walk_length: int = 1, tries: int = 100, verbose: bool = True):
        ...
        class Visited:
            def __init__(self):
                self.cells = []
        def is_connected(cells, connection_pattern, cellmap: Cell_map, source: int, destination: int, visited = Visited()):
            ...
            for c in connection_pattern:
                ...
                if i == destination:
                    return True
                if i >= 0 and i not in visited.cells:
                    visited.cells.append(i)
                    if is_connected(cells, connection_pattern, cellmap, i, destination, visited) :
                        return True
        ...
        for t in range(tries):
            ...
            for i in range(occlusion_count):
                if walk == 0:
            ...
            if is_connected(self.cells, self.configuration.connection_pattern, m, 0, 330):
                if verbose:
                    print("Success")
                break
            else:
