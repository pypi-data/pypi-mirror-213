...
class TestAvailableDeployments(unittest.TestCase):
    ...
    def getSuccessResponse():
        res = Response(200, json={"total_pages": 1, "deployments":[{"role": "something", "deployment":DEPLOYMENT}]})
        return res
    ...
    def test_success_request(self, sync):
        deployment = DeploymentComplete.from_dict(DEPLOYMENT)
        self.assertEqual(self.client.available_deployments().deployments[0].deployment, deployment)
