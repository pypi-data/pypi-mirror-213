...
class TestRepoWithService(unittest.TestCase):
    ...
    def test_bad_request(self, sync):
        try:
            self.client.get_repo(owner="Repo Owner", name="Repo Name")
            assert False
    ...
    def test_success_request(self, sync):
        obj = Repo.from_dict(USER_REPO)
        self.assertEqual(self.client.get_repo(owner="Repo Owner", name="Repo Name"), obj)
