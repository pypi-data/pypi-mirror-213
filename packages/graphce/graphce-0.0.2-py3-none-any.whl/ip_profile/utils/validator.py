from ip_profile.exceptions import InvalidIpException
from ipaddress import ip_network, ip_address


def _argument_error(f, *args):
    if len(args) != 2 and (f.__name__ != "update_ip_info" and f.__name__ != "add"):
        type_error = "{}() takes exactly one argument ({} given)".format(f.__name__, len(args) - 1)
        raise TypeError(type_error)
    elif len(args) != 3 and f.__name__ == "update_ip_info":
        type_error = "{}() takes exactly two argument ({} given)".format(f.__name__, len(args) - 1)
        raise TypeError(type_error)
