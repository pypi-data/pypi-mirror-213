...
import itertools
import os
from docstruct import VisLine, VisLineOrientation
...
class VisLineOpenCV:
    ...
    def get_line_width_not_in_use(self, line: dict, orientation: VisLineOrientation):
        ...
        for point in points:
            ...
            orthogonal_line = get_line(
                *point,
                length=max_length,
                orientation=VisLineOrientation.get_orthogonal(orientation),
            )
    ...
    def convert_to_vis_line(
        self, openCV_line: np.array, orientation: VisLineOrientation
    ) -> VisLine:
        ...
        if orientation == VisLineOrientation.HORIZONTAL:
...
def estimate_width_from_pixels(pixels: list[int]):
    # TODO - implement a width estimation algorithm
    return 10


def get_line(center_x: float, center_y: float, length: float, orientation: VisLineOrientation):
    if orientation == VisLineOrientation.HORIZONTAL:
