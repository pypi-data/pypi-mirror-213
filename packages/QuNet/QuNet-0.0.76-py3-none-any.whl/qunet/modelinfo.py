...
class ModelInfo:
    ...
    def reset(self):
        ...
        for n, p in self.model.named_parameters():                        
            self.params[n] = {
                'numel': p.numel(),
                'shape': tuple(p.shape),
                'data' : torch.square(p.data).sum(),
                'min'  : p.data.abs().min(),
                'max'  : p.data.abs().max(),                              
                'grad' : 0,
            }
            if p.grad is not None:                    
                self.params[n]['grad'] = torch.square(p.grad).sum() 
    ...
    def update(self):
        ...
        for n, p in model.named_parameters():
            param = self.params[n]
            param['data'] = w1 * param['data'] + w2 * torch.square(p.data).sum()
            param['min']  = w1 * param['min']  + w2 * p.data.abs().min()
            param['max']  = w1 * param['max']  + w2 * p.data.abs().max()
            if p.grad is not None:                    
                if 'grad' not in param:
                    param['grad'] = torch.square(p.grad).sum()
                else:
                    param['grad'] = w1 * param['grad'] + w2 * torch.square(p.grad).sum()
    ...
    def info(self, agg=None):
        ...
        for name, group in groups.items():
            nm = name + " "*(w-len(name))
            print(f"{nm}  {group['numel']:8.0f} | {group['data']:8.3f}  [{group['min']:8.3f}, {group['max']:8.3f}]  {group['grad']:8.1e}  {group['shape']}  ")

        print("-"*(w+50))
        n1, n2, n3 = self.num_params(True),  self.num_params(False),  self.num_params(None)
        print(f"{'trainable'+' '*(w-9)}: {n1:7d}")
        if n2 or n3 != n1:
            print(f"{'other'+' '*(w-5)}: {n1:7d}")
            print(f"{'total'+' '*(w-5)}: {n3:7d}")
