...
class ModelState:
    ...
    def layers(self, info=0, is_names=True, input_size=None, input_data=None):        
        ...
        if len(self.__layers) == 0:
            ...
            self.get_layers_descr(info = info)
        ...
        for ln in self.__layers:            
            ...
            if len(ln['params']) > 0:
                ...
                num_st  = f"{num:10n}"  if num > 0 else ""
        ...
        print(f"{'trainable:'+' '*(ma-15)}     {n1:12n}")
        if n2 or n3 != n1:
            print(f"{'other:'+' '*(ma-11)}     {n1:12n}")
            print(f"{'total:'+' '*(ma-11)}     {n3:12n}")
    ...
    def params(self, agg=None):
        ...
        for i, (name, group) in enumerate(groups.items()):
            ...
            print(f"{i:3d}: {nm}  {group['numel']:9n} {prc}  {group['data']:8.3f}  [{group['min']:8.3f}, {group['max']:8.3f}]  {group['grad']:8.1e}  {group['shape']}  ")

        print("="*(w+12+4))
        n1, n2, n3 = self.num_params(True),  self.num_params(False),  self.num_params(None)
        print(f"{'trainable:'+' '*(w-15+4)}     {n1:12n}")
        if n2 or n3 != n1:
            print(f"{'other:'+' '*(w-11+4)}     {n1:12n}")
            print(f"{'total:'+' '*(w-11+4)}     {n3:12n}")
