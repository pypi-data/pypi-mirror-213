...
class Trainer:
    ...
    def fit_one_epoch(self, model, data,  train=True, accumulate=1, info=None, verbose=1):
        """
        Args
        ------------
        train (bool=True):
    ...
    def fit(self,  epochs=None,  samples=None,
        ...
        ------------
            ...
            info (ModelInfo=None)
                an instance of the ModelInfo class that accumulates information about gradients on model parameters
            period_info (int=0)
                period after which the model info  plot is displayed (in epochs)
        ...
        for epoch in range(1, epochs+1):
            ...
            losses, scores, counts, (samples_trn,steps_trn,tm_trn) = self.fit_one_epoch(self.model, self.data.trn, train=True, info=info)            
            ...
            if period_plot > 0 and (self.epoch % period_plot == 0 or epoch == epochs):
                ...
                self.stat()
            if (info is not None) and period_info > 0 and  (self.epoch % period_info == 0 or epoch == epochs):
                info.plot()            
            ...
            if patience is not None and patience > 0 and  self.epoch - last_best > patience:
                ...
                if period_plot > 0 and len(self.hist.trn.epochs) > 1:
                    ...
                    for callback in self.callbacks:
                        callback.on_after_plot(self, self.model)
                    self.stat()
                if period_info > 0 and info is not None:
                    info.plot()
                break
