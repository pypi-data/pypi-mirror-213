...
class NerEngine(TaskEngine):
    ...
    def pipeline(self) -> Any:
        if self._pipeline is None:
            self._pipeline = AutoNerPipeline.create(
...
class AutoNerEngine(BaseParent):

    @classmethod
    def create(cls, class_key, model_type, **kwargs):
        return NerEngine(model_type, class_key, **kwargs)
