        ...
        try:
            ...
            try:
                exec_two_to_three(
                    ...
                    args=["-w", file_full_path, "--no-diffs", "--nobackups", "-j", "2"],
                )
            except Exception:
    ...
    def check_config(self, app, config):
        ...
        if "distsearch" in config:
            executable_files = list(
                set(executable_files) - set(app.get_non_distributed_files(config))
            )
        ...
        def check_report_binary_file_in_code(ast_node, file_name):
            ...
            if app.file_exists(inspect_file_name):
                binary_status = _check_binary_status(
                    app.get_filename(inspect_file_name)
                )
                if binary_status["error"]:
                    yield WarningMessage(
                        binary_status["error"],
                    ...
                    if name == "CDLL" or name == "LoadLibrary":
                        for ctypes_usage in ctypes_usages:
                            if hasattr(ctypes_usage, "args") and ctypes_usage.args:
                                yield from check_report_binary_file_in_code(
