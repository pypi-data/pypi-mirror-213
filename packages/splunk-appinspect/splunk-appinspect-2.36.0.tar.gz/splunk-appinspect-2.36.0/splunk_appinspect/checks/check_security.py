...
def _secret_disclosure_values_whitelist(line_message):
    ...
    for key, value in key_value_set.items():
        if re.match(rf"\s*{key}\s*=\s*((<|\"|\'|\\\"|\\\')?{value}(>|\"|\'|\\\"|\\\')?)\s*", line_message):
            return True

    return False


def _secret_disclosure_values_savedsearches_allowlist(file_path, line_message):
    savedsearches_files = [
    ...
    if re.match(r"\s*(alert.[\w-]+.params.[\w-]+)\s*=\s*((<|\"|\'|\\\"|\\\')?([\w]+)(>|\"|\'|\\\"|\\\')?)\s*", line_message):
        return False
...
@splunk_appinspect.tags("splunk_appinspect", "security", "cloud", "private_app", "private_victoria", "migration_victoria", "private_classic")
...
@splunk_appinspect.tags("cloud", "private_app", "splunk_9_0", "private_victoria", "migration_victoria", "private_classic", "pura")
