...
class CheckKosAreAccessible(Check):
    ...
    def check_metadata_file(self, app, path_in_app):
        ...
        if app.file_exists(path_in_app):
            # escape the slash or backslash so it's valid for Windows and *nix
            # -> `metadata\\` (Windows) or `metadata\/` (*nix)
            file_name = re.sub('metadata\\' + os.path.sep, '', path_in_app)
            meta_files.append(app.get_meta(file_name))
