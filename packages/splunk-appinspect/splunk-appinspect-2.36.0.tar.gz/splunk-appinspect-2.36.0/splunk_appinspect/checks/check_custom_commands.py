...
class CheckCommandScriptsPythonVersion(Check):
    ...
    def check_config(self, app, config):
        ...
        for command in custom_commands_conf.sections():
            ...
            if (not command_chunked or command_chunked == "false"
            ) and command_file.value.endswith(".py"):

                if not python_version:
                    yield FailMessage(
                        f"Custom command {command.name} doesn't define python.version, python.version should be explicitly set to python3.",
                        file_path,
                    )
                    continue
                if python_version != "python3":
                    yield FailMessage(
                        f"Custom command {command.name} doesn't define python.version as python3, python.version should be explicitly set to python3.",
                        file_name=file_path,
                ...
                if sys.version_info >= (3,):
                    for directory, filename, file_extension in app.iterate_files():
                        if file_extension == ".py" or file_extension == ".pl":
                            file_path = os.path.join(directory, filename)
                            python_script_resource = os.path.join(app.app_dir, file_path)

                            # The following logic will only take effect when running interpreter
                            # is Python 3

                            if os.path.exists(python_script_resource):
                                import ast
                                fd = open(python_script_resource)
                                try:
                                    ast.parse(fd.read())
