...
def print_result_records(
    ...
    if max_messages == splunk_appinspect.main.MAX_MESSAGES_DEFAULT:
        click.echo(
            f"A default value of {str(max_messages)} for max-messages will be used."
        )
        ...
        if checks_with_errors:
            ...
            for group, check, reporter in checks_with_errors:
                if print_group_documentation:
                    formatted_group_documentation = format_cli_string(
                        group.doc(), left_padding=0
                    )
                    click.echo(painter.paint.green(formatted_group_documentation))
                    print_group_documentation = False

                formatted_check_documentation = format_cli_string(
                    check.doc(), left_padding=4
                )
                ...
                for report_record in reporter.report_records(max_records=max_messages):
                    if report_record.result in message_types:
                        format_string = format_cli_string(
                            report_record.message, left_padding=12
                        ).lstrip()

                        fmt = result_colors[report_record.result]
                        result_message = (
                            f"        {report_record.result.upper()}: {format_string}"
                        )
                        click.secho(result_message, **fmt)
...
def format_cli_string(string_to_format, left_padding=4, column_wrap=80):
    ...
    for split_string in split_strings:
        ...
        new_index = (
            current_index + current_word_length + 1
        )  # The plus 1 is for the space that is added at the end
