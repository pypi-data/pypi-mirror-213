...
from splunk_appinspect.validation_report import (
...
class Validator(object):
    ...
    def __init__(
        ...
        if app_class is None:
            ...
            logger_output = (
                f"The custom app_class '{app_class}' was provided to the"
    ...
    def __execute_check(self, context, app, check):
        ...
        while reporter.state() == "error" and retries < MAX_CHECK_RETRIES:
            retries += 1
            logger.info(
                f"Retrying check because it produced an error, check_name={check.name}"
            )
    ...
    def __dispatch_check(self, threadpool, context, app, check):
