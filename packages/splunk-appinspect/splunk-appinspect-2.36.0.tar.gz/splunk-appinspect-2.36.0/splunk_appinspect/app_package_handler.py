...
class TarAppPackage(CompressedAppPackage):
    ...
    def _perform_extraction(self, compressed_application_path, temporary_directory):
        ...
        """
        try:
            tarsec = TarSec(filename=compressed_application_path)
        except tarfile.TarError:
            logger.info("Encountered tarfile exception.", compressed_application_path)
            return True

        if not tarsec.is_safe():
            logger.info(
                "The tar file is considered unsafe.",
                compressed_application_path
            )
            return True
