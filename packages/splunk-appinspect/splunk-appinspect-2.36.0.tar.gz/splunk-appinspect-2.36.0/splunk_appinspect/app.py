...
class App(object):
    ...
    def _get_app_info(self, stanza, option, app_conf_dir="default"):
        ...
        try:
            ...
            logger_output = (
                f"The `app.conf` file does not exist." f" Error: {error_message}"
            )
            logger.error(logger_output)
            ...
            error_message = repr(exception)
            logger_output = logger_error_message.format(
                error=error_message, santa=stanza, property=option
            )
            logger.error(logger_output)
            property_to_return = (
                f"[MISSING `{app_conf_dir}/app.conf` stanza `{stanza}`]"
            )
            raise exception
    ...
    def package_id(self):
        ...
        """
        if self.file_exists("default", "app.conf") and self.app_conf().has_option(
            "package", "id"
        ):
                    ...
                    for file_name in files:
                        ...
                        if (
                            ...
                            while current_frame:
                                ...
                                if (
                                    name == "run"
                                    and "self" in varnames
                                    and issubclass(type(f_locals["self"]), Check)
                                ):
                            ...
                            if not self._filter_by_trusted_libs(
                                dir_in_app, file_name, check_name
                            ):
    ...
    def search_for_patterns(
        ...
        for (dir_in_app, file_name, _) in files_iterator:
            ...
            file_to_inspect = inspected_file.InspectedFile.factory(
                self.get_filename(dir_in_app, file_name)
            )
            ...
            for execute_file in command.executable_files:
                if execute_file.is_path_pointer:
                    executable_files.extend(
                        custom_commands.find_pointer_scripts(execute_file)
                    )
                else:
    ...
    def get_non_distributed_files(self, config):
        ...
        if (
            not conf_file.has_section("replicationBlacklist") and
            not conf_file.has_section("replicationDenylist")
            ):
            return []
        ...
        for directory, filename, _ in self.iterate_files():
            ...
            regex_file_path = os.path.join(
                "apps", self.package.origin_package_name, file_path
            )
    ...
    def default_config(self):
        ...
        if self._default_config is None:
            self._default_config = configuration_file.ConfigurationProxy(
                self, "default"
            )
    ...
    def merged_config(self):
        ...
        if self._merged_config is None:
            return configuration_file.MergedConfigurationProxy(
                self.local_config, self.default_config
            )
