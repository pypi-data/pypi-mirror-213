...
class CustomVisualizations:
    ...
    def create_custom_visualization(self, stanza_section):
        ...
        default_height_option = (
            stanza_section.get_option("default_height")
            if stanza_section.has_option("default_height")
            else None
        )
        disabled_option = (
            stanza_section.get_option("disabled")
            if stanza_section.has_option("disabled")
            else None
        )

        custom_visualization = (
            splunk_appinspect.custom_visualizations.CustomVisualization(
                self.app,
                stanza_section.name,
                description_option,
                label_option,
    ...
    def does_visualizations_directory_exist(self):
        return self.app.directory_exists(
    ...
    def get_raw_configuration_file(self):
        return self.app.get_raw_conf(
            self.CONFIGURATION_FILE_NAME, dir=self.CONFIGURATION_DIRECTORY_PATH
        )
