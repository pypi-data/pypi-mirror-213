...
class ModularInputs(object):
    ...
    def has_specification_file(self):
        ...
        return self.app.file_exists(
            self.specification_directory_path, self.specification_filename
        )
    ...
    def get_raw_specification_file(self):
        ...
        return self.app.get_raw_conf(
            self.specification_filename, dir=self.specification_directory_path
        )
        ...
        for arch in self.app.arch_bin_dirs:
            for bin_dir in self.app.arch_bin_dirs[arch]:
                ...
                for directory, filename, file_extension in self.app.iterate_files(
                    ...
                    found_file_matching_mod_input_name = (
                        re.search(file_name_regex_object, file_base_name) is not None
                    )
    ...
    def get_modular_inputs(self, case_sensitive=True):
        ...
        for section in self.get_specification_file().sections():
            ...
            if mod_input:
                for key, value, lineno in self.get_specification_file().items(
                    section.name
                ):
                ...
                for file_resource in self.find_exes(
                    mod_input.name, case_sensitive=case_sensitive
                ):
                ...
                mod_input.cross_plat_exes = list(
                    filter(
                        lambda exe: self.app.DEFAULT_ARCH in exe.tags
                ...
                mod_input.win_arch_exes = list(
                    filter(
                        lambda exe: self.app.WIN_ARCH in exe.tags
                        and self.WINDOWS_EXE_TAG in exe.tags,
                        files,
                    )
                )
                ...
                mod_input.linux_arch_exes = list(
                    filter(
                        lambda exe: self.app.LINUX_ARCH in exe.tags
                        and self.NIX_EXE_TAG in exe.tags,
                        files,
                    )
                )
                ...
                mod_input.darwin_arch_exes = list(
                    filter(
                        lambda exe: self.app.DARWIN_ARCH in exe.tags
                        and self.NIX_EXE_TAG in exe.tags,
                        files,
                    )
                )
