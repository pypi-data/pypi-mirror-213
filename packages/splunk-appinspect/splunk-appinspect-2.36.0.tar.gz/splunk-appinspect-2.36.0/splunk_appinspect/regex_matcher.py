...
class RegexMatcher(object):
    ...
    def match_file(self, filepath, regex_option=0, exclude_comments=True):
        """return all match results in (lineno, result) tuple and in sorted order"""
    ...
    def match_results_iterator(
        self, app_dir, file_iterator, regex_option=0, exclude_comments=True
    ):
...
class JSSplunkSplunkFrontendUtils(RegexMatcher):
    def __init__(self):
        possible_splunkutils_regex_patterns = [
            RegexBundle(r"@splunk\/(moment|splunk-utils|ui-utils|search-job)")
        ]
        super(JSSplunkSplunkFrontendUtils, self).__init__(
            possible_splunkutils_regex_patterns
        )
...
class JSSplunkDashboardCoreMatcher(RegexMatcher):
    def __init__(self):
        ...
        super(JSSplunkDashboardCoreMatcher, self).__init__(
            possible_splunkdashboardcore_regex_patterns
        )
...
class JSSplunkVisualizationsMatcher(RegexMatcher):
    def __init__(self):
        possible_splunkvisualizations_regex_patterns = [
            RegexBundle(r"@splunk\/visualization(s)?(-\w+)?")
        ]
        super(JSSplunkVisualizationsMatcher, self).__init__(
            possible_splunkvisualizations_regex_patterns
        )
...
class SecretDisclosureInNonPythonFilesMatcher(RegexMatcher):
    def __init__(self):
        bundle = RegexBundle(
            r"(?i).{0,30}(login|passwd|password|community|privpass|apikey)\s*=\s*[^\s]+"
        )
        bundle.exception(
