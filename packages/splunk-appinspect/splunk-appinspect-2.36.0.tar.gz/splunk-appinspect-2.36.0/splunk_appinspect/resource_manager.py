...
"""Splunk AppInspect Resource Abstraction module. In Splunk AppInspect API service, clamav resource are provided."""
...
class ResourceManagerContext(object):
    ...
    def get(self, attrname):
        with self._locks[attrname]:
            if (
                attrname in self.manager.resource_types
                and attrname not in self._resources
            ):
                self._resources[attrname] = self.manager.resource_types[attrname](
                    self.context
                )
            ...
            raise KeyError(
                f"No such resource defined in context: {attrname} ([{self.manager.resource_types.keys()}])"
            )
