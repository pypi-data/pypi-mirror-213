...
class InspectedFile(object):
    ...
    def search_for_patterns(self, patterns, exclude_comments=True, regex_option=0):
        """
        :param patterns: regex patterns array
        :param exclude_comments: excluded comment from test
        ...
        inspected_file = open(self._path, "r", encoding="utf-8", errors="ignore")
        line_no = 0
    ...
    def search_for_pattern(self, pattern, exclude_comments=True, regex_option=0):
    ...
    def search_for_crossline_patterns(
        self, patterns, exclude_comments=True, cross_line=10
    ):
        """
        :param patterns: regex patterns array
        :param exclude_comments: excluded comment from test
        ...
        for line_no in range(0, lines_count):
            ...
            start_line = line_no
            end_line = (
                (start_line + cross_line)
                if (start_line + cross_line) <= lines_count
                else lines_count
            )
    ...
    def search_for_crossline_pattern(
        self, pattern, exclude_comments=True, cross_line=10
    ):
