...
class AstInfoQuery(object):
    ...
    def _post_traverse(self, filter_logic_node, all_node_set):
        if isinstance(filter_logic_node, BinaryLogicNode):
            ...
            raise Exception(f"Unknown BinaryLogicNode {type(filter_logic_node)}")
        if isinstance(filter_logic_node, MultiLogicNode):
            ...
            raise Exception(f"Unknown MultiBinaryLogicNode {type(filter_logic_node)}")
        if isinstance(filter_logic_node, DataSetNode):
            ...
            raise Exception(f"Unknown DataSetNode {type(filter_logic_node)}")
        if isinstance(filter_logic_node, SingleLogicNode):
            ...
            raise Exception(f"Unknown SingleLogicNode {type(filter_logic_node)}")

        raise Exception(f"Unsupported Node {type(filter_logic_node)}")
