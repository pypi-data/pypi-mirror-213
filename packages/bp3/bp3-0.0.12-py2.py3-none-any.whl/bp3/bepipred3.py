...
class Antigens():
    ...
    def get_esm2_represention_on_accs_seqs(self):
        ...
        #go through batches and create ESM-2 encodings, do BepiPred-3.0 scores...
        #Or just save the ESM2 encodings in .pt files, with some identifier, append to list that keeps track of which encodings 
        #in the original accessino paths correspond to...
        enc_id = 0
        enc_paths = []
...
#            self.classification_thresholds = classification_thresholds
    ...
    def create_csvfile_and_top_preds(self, outfile_path):
        try:
        ...
        if not self.bp3_ensemble_run:
            ...
            with open(outfile_path  / f"Bcell_epitope_top_{self.top_pred_pct*100}pct_preds.fasta", "w") as outfile: outfile.write(outfile_content)
            outfile_content = linepi_toppct_outfile_content[:-1]
            with open(outfile_path  / f"Bcell_linepitope_top_{self.top_pred_pct*100}pct_preds.fasta", "w") as outfile: outfile.write(outfile_content)
