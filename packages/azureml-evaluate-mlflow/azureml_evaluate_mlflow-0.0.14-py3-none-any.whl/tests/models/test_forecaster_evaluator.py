...
from azureml.metrics._regression import NormRMSE
import mlflow
...
class TestForecasterEvaluator(unittest.TestCase):
    ...
    def _assert_metrics(self, eval_result, expected_nrmse, expect_artifacts, test_len=10):
        ...
        # Check artifacts
        if expect_artifacts:
            self.assertIn(constants.FORECASTING_MAPE, eval_result.artifacts)
            mape = eval_result.artifacts[constants.FORECASTING_MAPE].content
    ...
    def _do_test_model_evaluator(self, use_X_train):
        ...
        expected_nrmse = NormRMSE(
            ...
            y_min=np.min(y_train if use_X_train else y_test),
            y_max=np.max(y_train if use_X_train else y_test),
            y_std=np.std(y_train if use_X_train else y_test)).compute()
        self._assert_metrics(eval_result, expected_nrmse, use_X_train)

    def test_model_evaluator(self):
        """Test model evaluator without X_train."""
        self._do_test_model_evaluator(False)

    def test_model_evaluator_with_train(self):
        """Test model evaluator with X_train."""
        self._do_test_model_evaluator(True)

    def _do_test_model_evaluator_with_grains(self, forecast_flavor):
    ...
    def test_model_evaluator_with_grains(self):
        """Test model evaluation if there are grain columns in a data set."""
        self._do_test_model_evaluator_with_grains(ForecastFlavors.RECURSIVE_FORECAST)

    @patch('azureml.evaluate.mlflow.models.evaluation.azureml.azureml_evaluator.Run')
    def test_model_evaluator_with_grains_rolling(self, mock_run):
        """Test model evaluation if there are grain columns in a data set."""
        mock_run_instance = MagicMock()
        mock_run_instance.upload_file = TestForecasterEvaluator._check_attached_file
        mock_run.get_context.return_value = mock_run_instance
        self._do_test_model_evaluator_with_grains(ForecastFlavors.ROLLING_FORECAST)
