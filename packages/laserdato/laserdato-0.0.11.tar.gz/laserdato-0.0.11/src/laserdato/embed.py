...
def init_process(encoder: SentenceEncoder, devices: list[int]):
...
class MultiGpuEncoder:
    def __init__(self, devices: list[int], encoder: SentenceEncoder):
...
def EncodeText(
    encoder, sentences, fp16=False, verbose=False, devices: list[int] = None, cpu=False
):
    t = time.time()
    # do I need a buffer ?
    if verbose:
        logger.info(f" - Beginning of the encoding")
    multiGPU = devices is not None and len(devices) > 1
    if multiGPU:
        multiGpuEncoder = MultiGpuEncoder(devices, encoder)
        encoded = multiGpuEncoder.encode_sentences(sentences)
        multiGpuEncoder.delete()
    else:
        device = devices[0] if devices else None
        encoder._choose_encoder_device(cpu=cpu, device=device)
