...
class Strategies:
    ...
    async def long_straddle(self, stock_code, strike_price, qty, expiry_date, exchange_code, stoploss = "", put_price = "0", call_price = "0",product_type = "options", order_type = "market", validity = "day", validity_date = datetime.utcnow().strftime('%Y-%m-%dT%H:%M:%S.%fZ')):
        ...
        if(firstresponse.get('Success',None)!=None and secondresponse.get('Success',None)==None):
            ...
            print("----lets wait for 100 seconds for getting the executed status---")
            time.sleep(100)
