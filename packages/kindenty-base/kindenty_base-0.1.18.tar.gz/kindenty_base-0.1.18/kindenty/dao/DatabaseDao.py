...
class StrategyDao(BaseDao):
    ...
    updateEnableByIds = UpdateMethod("update strategy set enable=':enable', stop_time=date_format(now(),'%Y-%m-%d %H:%i:%S') where id in (:ids)")
    ...
    queryStrategyByNotSellAll = QueryMethod("""
        ...
        where ((ifnull(b.ct,0)-ifnull(c.ct,0))>0 and a.enable=0) or enable=1 or (a.version='1.4' and a.enable_sell = 1)""", Strategy)
...
class TradeLogDao(BaseDao):
    ...
    deleteBySharesCodeAndVersion = UpdateMethod("delete from trade_log where strategy_id in (select id from strategy where shares_code=':sharesCode' and version=':version')")
