...
from pydantic import StrictStr, Field, root_validator, validator, StrictInt
...
class OverPartition(Node):
    ...
    label_: StrictStr = Field("over partition", const=True, alias='label')
...
class OverOrder(Node):
    label_: StrictStr = Field("over order", const=True, alias='label')
...
class OverFrame(Node):
    label_: StrictStr = Field("over frame", const=True, alias='label')
...
class OverFilter(Node):
    label_: StrictStr = Field("over filter", const=True, alias='label')
...
class Window(Node):
    ...
    label_: StrictStr = Field("over", const=True, alias='label')
    parents_: tuple = Field(alias='parents')

    @root_validator
    def _validate_over(cls, values):
        if len(values['parents_']) != 4:
            raise ValueError(
                'There are missing inputs. Over must have four parent nodes: partition, order by, frame and filter'
            )

        partition, order_by, frame, filter = values['parents_']
