...
class SqlTestCase(unittest.TestCase):
    ...
    def make_table_parameter(self, meta: TableParamMeta) -> Parameter:
        ...
        table = Table(meta=table_meta, client=self.make_dummy_client(), parameters=tuple())
    ...
    def make_table(self, name='My.Test.Table', n_cols=10, n_params=3, n_tv_params=0) -> Table:
        ...
        return Table(meta=meta, client=self.make_dummy_client(), parameters=parameters + table_parameters)
    ...
    def assertErrorsWithMessage(self, fn, err_type, exp):
        with self.assertRaises(err_type) as err:
            fn()

        obs = str(err.exception)
        self.assertLineByLineEqual(exp, obs)

    def assertLineByLineEqual(self, exp, obs):
