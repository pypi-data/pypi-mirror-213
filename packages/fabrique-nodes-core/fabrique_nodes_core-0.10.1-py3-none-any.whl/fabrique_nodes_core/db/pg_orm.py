...
class DB(BaseDB, metaclass=Singleton):
    ...
    def get_project_rec(self, rec_id: str = '', project_id: str = '', version: str = '') -> ProjectRecordModel:
        if not rec_id:
    ...
    def list_user_projects(self, user_id) -> list[str]:
        recs = self.execute("""
        WITH ranked_projects AS (
            SELECT
                rec_id,
    ...
    def delete_project(self, rec_id: str = '', project_id: str = '', version: str = ''):
        if not rec_id:
            rec_id = project_id + version
        self.execute('DELETE FROM users WHERE rec_id = %s;', (rec_id, ), commit=True)
