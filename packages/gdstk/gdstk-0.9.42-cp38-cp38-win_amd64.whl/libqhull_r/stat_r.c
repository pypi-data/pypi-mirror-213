...
void qh_allstatA(qhT *qh) {
    ...
    qh->qhstat.precision =
        qh->qhstat.next; /* usually call qh_joggle_restart, printed if Q0 or QJn */
...
void qh_allstatF(qhT *qh) {
    ...
    zdef_(wmax, Wmaxoutside, "max distance of vertex or coplanar point above facet (w/roundoff)",
          -1);
    ...
    zdef_(zinc, Zredundantmerge,
          "  detected by qh_test_nonsimplicial_merge instead of qh_test_redundant_neighbors", -1);
...
void qh_allstatI(qhT *qh) {
    qh->qhstat.vridges =
        qh->qhstat.next; /* printed in qh_produce_output2 if non-zero Zridge or Zridgemid */
...
void qh_allstatistics(qhT *qh) {
    ...
    for (i = ZEND; i--;) qh->qhstat.printed[i] = False;
} /* allstatistics */
...
void qh_collectstatistics(qhT *qh) {
    ...
    if (!qh_checklists(qh, qh->facet_list)) {
        qh_fprintf(qh, qh->ferr, 6373,
                   "qhull internal error: qh_checklists failed on qh_collectstatistics\n");
        if (!qh->ERREXITcalled) qh_errexit(qh, qh_ERRqhull, NULL, NULL);
    }
    ...
    FORALLfacets {
        ...
        if (facet->ridges) {
            zadd_(Zmemridges, (int)sizeof(setT) + SETelemsize * sizridges +
                                  sizridges *
                                      ((int)sizeof(ridgeT) + (int)sizeof(setT) +
        ...
        FOREACHneighbor_(facet) {
            if (neighbor == qh_DUPLICATEridge || neighbor == qh_MERGEridge || neighbor->seen ||
                !facet->normal || !neighbor->normal)
                continue;
            dotproduct = qh_getangle(qh, facet->normal, neighbor->normal);
            zinc_(Zangle);
            wadd_(Wangle, dotproduct);
            wmax_(Wanglemax, dotproduct) wmin_(Wanglemin, dotproduct)
        }
        if (facet->normal) {
...
void qh_initstatistics(qhT *qh) {
    ...
    if (qh->qhstat.next > (int)sizeof(qh->qhstat.id)) {
        qh_fprintf_stderr(
            6184,
            "qhull internal error (qh_initstatistics): increase size of qhstat.id[].  qhstat.next %d should be <= sizeof(qh->qhstat.id) %d\n",
...
void qh_printstatistics(qhT *qh, FILE *fp, const char *string) {
    ...
    if (qh->num_points != qh->num_vertices || zval_(Zpbalance) == 0) {
        ...
        wval_(Wpbalance2) =
            qh_stddev(qh, zval_(Zpbalance), wval_(Wpbalance), wval_(Wpbalance2), &ave);
    if (zval_(Zprocessed) == 0)
        wval_(Wnewbalance2) = 0.0;
    else
        wval_(Wnewbalance2) =
            qh_stddev(qh, zval_(Zprocessed), wval_(Wnewbalance), wval_(Wnewbalance2), &ave);
...
",
               qh->MAXabs_coord, qh->DISTround, qh->ANGLEround, qh->MINoutside, qh->MINvisible,
               qh->MAXcoplanar, qh->WIDEfacet);
 ...
 %6.2g max. distance for near-inside points\n",
    ...
    if (qh->premerge_cos < REALmax / 2)
        qh_fprintf(qh, fp, 9353,
                   "\
 %6.2g max. cosine for pre-merge angle\n",
                   qh->premerge_cos);
    if (qh->PREmerge)
        qh_fprintf(qh, fp, 9354,
                   "\
 %6.2g radius of pre-merge centrum\n",
                   qh->premerge_centrum);
    if (qh->postmerge_cos < REALmax / 2)
        qh_fprintf(qh, fp, 9355,
                   "\
 %6.2g max. cosine for post-merge angle\n",
                   qh->postmerge_cos);
    if (qh->POSTmerge)
        qh_fprintf(qh, fp, 9356,
                   "\
 ...
 %6.2g min. denominator for division\n\
  zero diagonal for Gauss: ",
    ...
    for (k = 0; k < qh->hull_dim; k++) qh_fprintf(qh, fp, 9358, "%6.2e ", qh->NEARzero[k]);
...
void qh_printstatlevel(qhT *qh, FILE *fp, int id) {
    if (id >= ZEND || qh->qhstat.printed[id]) return;
    if (qh->qhstat.type[id] == zdoc) {
        qh_fprintf(qh, fp, 9360, "%s\n", qh->qhstat.doc[id]);
        return;
    }
    if (qh_nostatistic(qh, id) || !qh->qhstat.doc[id]) return;
    ...
    if (qh->qhstat.count[id] != -1 &&
        ...
        qh_fprintf(
            qh, fp, 9363, "%7.2g",
            qh->qhstat.stats[id].r / qh->qhstat.stats[(unsigned char)(qh->qhstat.count[id])].i);
    else if (qh->qhstat.type[id] < ZTYPEreal && qh->qhstat.count[id] == -1)
        qh_fprintf(qh, fp, 9364, "%7d", qh->qhstat.stats[id].i);
    else if (qh->qhstat.type[id] < ZTYPEreal && qh->qhstat.count[id] != -1)
        qh_fprintf(qh, fp, 9365, "%7.3g",
                   (realT)qh->qhstat.stats[id].i /
                       qh->qhstat.stats[(unsigned char)(qh->qhstat.count[id])].i);
...
void qh_printstats(qhT *qh, FILE *fp, int idx, int *nextindex) {
    ...
    if (qh_newstats(qh, idx, &nexti)) {
        ...
        for (j = idx; j < nexti; j++) qh_printstatlevel(qh, fp, qh->qhstat.id[j]);
    }
...
realT qh_stddev(qhT *qh, int num, realT tot, realT tot2, realT *ave) {
    ...
    if (num <= 0) {
        qh_fprintf(
            qh, qh->ferr, 7101,
            "qhull warning (qh_stddev): expecting num > 0.  Got num %d, tot %4.4g, tot2 %4.4g.  Returning 0.0\n",
            num, tot, tot2);
        return 0.0;
    }
    *ave = tot / num;
