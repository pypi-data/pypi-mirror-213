...
int qh_compare_facetvisit(const void *p1, const void *p2) {
    ...
    if (!(i = (int)a->visitid)) i = (int)(0 - a->id); /* sign distinguishes id from visitid */
    if (!(j = (int)b->visitid)) j = (int)(0 - b->id);
    return (i - j);
} /* compare_facetvisit */
  ...
  design:
    for all facets on facetlist and in facets set
      unless facet is skipped or visible (i.e., will be deleted)
        ...
        update counts
*/
void qh_countfacets(qhT *qh, facetT *facetlist, setT *facets, boolT printall, int *numfacetsp,
                    int *numsimplicialp, int *totneighborsp, int *numridgesp, int *numcoplanarsp,
    ...
    FORALLfacet_(facetlist) {
        if ((facet->visible && qh->NEWfacets) || (!printall && qh_skipfacet(qh, facet)))
            facet->visitid = 0;
        else {
            ...
            if (facet->simplicial) {
                numsimplicial++;
                if (facet->keepcentrum && facet->tricoplanar) numtricoplanars++;
            } else
                numridges += qh_setsize(qh, facet->ridges);
            if (facet->coplanarset) numcoplanars += qh_setsize(qh, facet->coplanarset);
        }
    }
...
pointT *qh_detvnorm(qhT *qh, vertexT *vertex, vertexT *vertexA, setT *centers, realT *offsetp) {
    ...
    for (k = 0; k < dim; k++) midpoint[k] = (vertex->point[k] + vertexA->point[k]) / 2;
    ...
    if (numcenters > dim) {
        ...
        if (unbounded) qh_setappend(qh, &simplex, midpoint);
        qh_maxsimplex(qh, dim, points, NULL, 0, &simplex);
        qh_setdelnth(qh, simplex, 0);
    } else if (numcenters == dim) {
        if (unbounded) qh_setappend(qh, &points, midpoint);
        simplex = points;
    } else {
        qh_fprintf(
            qh, qh->ferr, 6216,
    ...
    FOREACHpoint_(simplex) {
        if (qh->IStracing >= 4)
            qh_printmatrix(qh, qh->ferr, "qh_detvnorm: Voronoi vertex or midpoint", &point, 1, dim);
        if (point != point0) {
            qh->gm_row[i++] = gmcoord;
            coord = point0;
    ...
    normal = gmcoord;
    qh_sethyperplane_gauss(qh, dim, qh->gm_row, point0, True, normal, &offset, &nearzero);
    /* nearzero is true for axis-parallel hyperplanes (e.g., a bounding box).  Should detect
     * degenerate hyperplanes.  See 'Tv' check following */
            ...
            FOREACHpoint_i_(qh, points) {
                if (!qh_setin(simplex, point)) {
                    ...
                    trace4((qh, qh->ferr, 4016,
                            ...
                            pointidA, facet->visitid, dist));
                }
            }
        }
    }
    *offsetp = offset;
    if (simplex != points) qh_settempfree(qh, &simplex);
...
setT *qh_detvridge(qhT *qh, vertexT *vertex) {
    ...
    qsort(SETaddr_(centers, facetT), (size_t)qh_setsize(qh, centers), sizeof(facetT *),
      ...
      for each unvisited vertex
        if atvertex and vertex share more than d-1 neighbors
          ...
          if printvridge defined
            ...
            call printvridge
*/
int qh_eachvoronoi(qhT *qh, FILE *fp, printvridgeT printvridge, vertexT *atvertex, boolT visitall,
...
*/
int qh_eachvoronoi_all(qhT *qh, FILE *fp, printvridgeT printvridge, boolT isUpper,
                       qh_RIDGE innerouter, boolT inorder) {
    facetT *facet;
    vertexT *vertex;
    ...
    FORALLfacets {
        if (facet->upperdelaunay == isUpper) facet->visitid = (unsigned int)(numcenters++);
    }
    FORALLvertices vertex->seen = False;
    FORALLvertices {
        if (qh->GOODvertex > 0 && qh_pointid(qh, vertex->point) + 1 != qh->GOODvertex) continue;
        totridges += qh_eachvoronoi(qh, fp, printvridge, vertex, !qh_ALL, innerouter, inorder);
    }
    return totridges;
} /* eachvoronoi_all */
...
setT *qh_facetvertices(qhT *qh, facetT *facetlist, setT *facets, boolT allfacets) {
    ...
    if (facetlist == qh->facet_list && allfacets && !facets) {
        ...
        FORALLfacet_(facetlist) {
            if (!allfacets && qh_skipfacet(qh, facet)) continue;
...
void qh_geomplanes(qhT *qh, facetT *facet, realT *outerplane, realT *innerplane) {
    ...
    if (qh->MERGING || qh->JOGGLEmax < REALmax / 2) {
        ...
        if (qh->JOGGLEmax < REALmax / 2)
            radius -= qh->JOGGLEmax *
                      sqrt((realT)qh->hull_dim); /* already accounted for in qh_outerinner() */
        *outerplane += radius;
        *innerplane -= radius;
...
void qh_markkeep(qhT *qh, facetT *facetlist) {
    ...
    FORALLfacet_(facetlist) {
        if (!facet->visible && facet->good) qh_setappend(qh, &facets, facet);
    }
    size = qh_setsize(qh, facets);
    if (qh->KEEParea) {
        qsort(SETaddr_(facets, facetT), (size_t)size, sizeof(facetT *), qh_compare_facetarea);
    ...
    if (qh->KEEPmerge) {
        qsort(SETaddr_(facets, facetT), (size_t)size, sizeof(facetT *), qh_compare_nummerge);
...
*/
setT *qh_markvoronoi(qhT *qh, facetT *facetlist, setT *facets, boolT printall, boolT *isLowerp,
                     int *numcentersp) {
    int numcenters = 0;
    facetT *facet, **facetp;
    setT *vertices;
    ...
    FORALLfacet_(facetlist) {
        if (printall || !qh_skipfacet(qh, facet)) facet->visitid = (unsigned int)(numcenters++);
    }
    FOREACHfacet_(facets) {
        if (printall || !qh_skipfacet(qh, facet)) facet->visitid = (unsigned int)(numcenters++);
    }
...
/*-<a                             href="qh-io_r.htm#TOC"
  ...
  notes:
    ...
    for 4-d and higher
      ...
      for example, use qh_markvoronoi (e.g., qh_printvornoi) or qh_countfacets (e.g.,
  qh_printvneighbors)
...
void qh_order_vertexneighbors(qhT *qh, vertexT *vertex) {
    ...
    if (qh->hull_dim == 2) {
        ...
        qsort(SETaddr_(vertex->neighbors, facetT), (size_t)numneighbors, sizeof(facetT *),
              qh_compare_facetvisit);
    }
...
/*-<a                             href="qh-io_r.htm#TOC"
  ...
  qh_prepare_output(qh )
    prepare for qh_produce_output2(qh) according to
      qh.KEEPminArea, KEEParea, KEEPmerge, GOODvertex, GOODthreshold, GOODpoint, ONLYgood,
  SPLITthresholds does not reset facet->good
...
void qh_prepare_output(qhT *qh) {
    ...
    if (qh->TRIangulate && !qh->hasTriangulation) {
        qh_triangulate(qh);
        if (qh->VERIFYoutput && !qh->CHECKfrequently) qh_checkpolygon(qh, qh->facet_list);
    }
    qh_findgood_all(qh, qh->facet_list);
    if (qh->GETarea) qh_getarea(qh, qh->facet_list);
    if (qh->KEEParea || qh->KEEPmerge || qh->KEEPminArea < REALmax / 2)
        qh_markkeep(qh, qh->facet_list);
    if (qh->PRINTstatistics) qh_collectstatistics(qh);
}
...
void qh_printafacet(qhT *qh, FILE *fp, qh_PRINT format, facetT *facet, boolT printall) {
    ...
    if (!printall && qh_skipfacet(qh, facet)) return;
    ...
    switch (format) {
        ...
        case qh_PRINTgeom: /* either 2 , 3, or 4-d by qh_printbegin */
            ...
            if (qh->PRINTdim != qh->hull_dim) qh_normalize2(qh, color, 3, True, NULL, NULL);
            if (qh->hull_dim <= 2)
        ...
        case qh_PRINTneighbors:
            ...
            FOREACHneighbor_(facet) qh_fprintf(
                qh, fp, 9024, " %d", neighbor->visitid ? neighbor->visitid - 1 : 0 - neighbor->id);
        ...
        case qh_PRINTpointintersect:
            ...
            if (facet->offset < -qh->MINdenom) {
                ...
                for (k = qh->hull_dim; k--;) {
                    *(coordp++) = qh_divzero(*(normp++), facet->offset, qh->MINdenom_1, &zerodiv) +
                                  *(feasiblep++);
                    if (zerodiv) {
            ...
            break;
        LABELprintinfinite:
            for (k = qh->hull_dim; k--;) qh_fprintf(qh, fp, 9026, qh_REAL_1, qh_INFINITE);
        ...
        case qh_PRINTpoints: /* VORONOI only by qh_printbegin */
            if (qh->CDDoutput) qh_fprintf(qh, fp, 9029, "1 ");
            qh_printcenter(qh, fp, format, NULL, facet);
            break;
...
*/
void qh_printbegin(qhT *qh, FILE *fp, qh_PRINT format, facetT *facetlist, setT *facets,
                   boolT printall) {
    ...
    qh->printoutnum = 0;
    qh_countfacets(qh, facetlist, facets, printall, &numfacets, &numsimplicial, &totneighbors,
                   &numridges, &numcoplanars, &numtricoplanars);
    switch (format) {
        case qh_PRINTnone:
            break;
        case qh_PRINTarea:
        ...
        case qh_PRINTcoplanars:
            ...
            break;
        case qh_PRINTcentrums:
            if (qh->CENTERtype == qh_ASnone) qh_clearcenters(qh, qh_AScentrum);
        ...
        case qh_PRINTgeom:
            ...
            if (qh->PRINTdim != 4 && (qh->PRINTcoplanar || qh->PRINTspheres || qh->PRINTcentrums)) {
                ...
                if (qh->PRINTcoplanar || qh->PRINTcentrums) {
                    ...
                    if (qh->PRINTcoplanar && !qh->PRINTspheres) {
                        FOREACHvertex_(vertices) qh_printpointvect2(
                            qh, fp, vertex->point, NULL, qh->interior_point, qh->PRINTradius);
                    }
                    FORALLfacet_(facetlist) {
                        if (!printall && qh_skipfacet(qh, facet)) continue;
        ...
        case qh_PRINTtriangles:
            ...
            if (format == qh_PRINToff || qh->hull_dim == 2)
                ...
                qh->printoutvar =
                    qh->num_points + qh_setsize(qh, qh->other_points); /* first centrum */
                if (qh->DELAUNAY) num--;                               /* drop last dimension */
                qh_fprintf(qh, fp, 9061, "%d\n%d %d %d\n", num,
                           qh->printoutvar + numfacets - numsimplicial, numsimplicial + numridges,
                           totneighbors / 2);
            }
            FORALLpoints qh_printpointid(qh, qh->fout, NULL, num, point, qh_IDunknown);
            FOREACHpoint_(qh->other_points)
...
void qh_printcenter(qhT *qh, FILE *fp, qh_PRINT format, const char *string, facetT *facet) {
    ...
    if (string) qh_fprintf(qh, fp, 9066, string);
    if (qh->CENTERtype == qh_ASvoronoi) {
        num = qh->hull_dim - 1;
        if (!facet->normal || !facet->upperdelaunay || !qh->ATinfinity) {
            if (!facet->center) facet->center = qh_facetcenter(qh, facet->vertices);
            for (k = 0; k < num; k++) qh_fprintf(qh, fp, 9067, qh_REAL_1, facet->center[k]);
        } else {
            for (k = 0; k < num; k++) qh_fprintf(qh, fp, 9068, qh_REAL_1, qh_INFINITE);
        }
    } else /* qh.CENTERtype == qh_AScentrum */ {
        num = qh->hull_dim;
        if (format == qh_PRINTtriangles && qh->DELAUNAY) num--;
        if (!facet->center) facet->center = qh_getcentrum(qh, facet);
        for (k = 0; k < num; k++) qh_fprintf(qh, fp, 9069, qh_REAL_1, facet->center[k]);
    }
...
*/
void qh_printend(qhT *qh, FILE *fp, qh_PRINT format, facetT *facetlist, setT *facets,
                 boolT printall) {
    int num;
    facetT *facet, **facetp;

    if (!qh->printoutnum) qh_fprintf(qh, qh->ferr, 7055, "qhull warning: no facets printed\n");
    switch (format) {
        case qh_PRINTgeom:
        ...
        case qh_PRINTnormals:
        case qh_PRINTouter:
            if (qh->CDDoutput) qh_fprintf(qh, fp, 9080, "end\n");
            break;
        case qh_PRINTmaple:
        ...
        case qh_PRINTmathematica:
            ...
            break;
        case qh_PRINTpoints:
            if (qh->CDDoutput) qh_fprintf(qh, fp, 9083, "end\n");
            break;
        default:
            break;
    }
            ...
            if (neighbor->visitid != qh->visit_id) {
                if (fp)
                    qh_fprintf(qh, fp, 9084, "3 %d %d %d %8.4g %8.4g %8.4g 1 # f%d f%d\n", 3 * num,
                               3 * num + 1, 3 * num + 2, color[0], color[1], color[2], facet->id,
                               neighbor->id);
                num++;
            }
        }
...
void qh_printextremes(qhT *qh, FILE *fp, facetT *facetlist, setT *facets, boolT printall) {
    ...
    FOREACHpoint_i_(qh, points) {
        if (point) qh_fprintf(qh, fp, 9087, "%d\n", point_i);
    }
    qh_settempfree(qh, &points);
} /* printextremes */
...
void qh_printextremes_2d(qhT *qh, FILE *fp, facetT *facetlist, setT *facets, boolT printall) {
    ...
    qh_countfacets(qh, facetlist, facets, printall, &numfacets, &numsimplicial, &totneighbors,
                   &numridges, &numcoplanars, &numtricoplanars); /* marks qh->visit_id */
    vertices = qh_facetvertices(qh, facetlist, facets, printall);
...
void qh_printextremes_d(qhT *qh, FILE *fp, facetT *facetlist, setT *facets, boolT printall) {
    ...
    FOREACHvertex_(vertices) {
        if (vertex->seen) qh_fprintf(qh, fp, 9092, "%d\n", qh_pointid(qh, vertex->point));
    }
    qh_settempfree(qh, &vertices);
} /* printextremes_d */
...
void qh_printfacet(qhT *qh, FILE *fp, facetT *facet) {
    qh_printfacetheader(qh, fp, facet);
    if (facet->ridges) qh_printfacetridges(qh, fp, facet);
} /* printfacet */
...
*/
void qh_printfacet2geom_points(qhT *qh, FILE *fp, pointT *point1, pointT *point2, facetT *facet,
...
void qh_printfacet2math(qhT *qh, FILE *fp, facetT *facet, qh_PRINT format, int notfirst) {
    ...
    qh_facet2point(qh, facet, &point0, &point1, &mindist);
    if (notfirst) qh_fprintf(qh, fp, 9096, ",");
    if (format == qh_PRINTmaple)
...
void qh_printfacet3geom_nonsimplicial(qhT *qh, FILE *fp, facetT *facet, realT color[3]) {
    ...
    qh_settempfree(qh, &vertices);
    if ((qh->DOintersections || qh->PRINTridges) && (!facet->visible || !qh->NEWfacets)) {
        facet->visitid = qh->visit_id;
        FOREACHridge_(facet->ridges) {
            neighbor = otherfacet_(ridge, facet);
            if (neighbor->visitid != qh->visit_id) {
...
*/
void qh_printfacet3geom_points(qhT *qh, FILE *fp, setT *points, facetT *facet, realT offset,
                               realT color[3]) {
    ...
    FOREACHpoint_(printpoints) {
        ...
        if (printpoints != points) qh_memfree(qh, point, qh->normal_size);
        qh_fprintf(qh, fp, 9101, "\n");
    }
    if (printpoints != points) qh_settempfree(qh, &printpoints);
    qh_fprintf(qh, fp, 9102, "%d ", n);
    for (i = 0; i < n; i++) qh_fprintf(qh, fp, 9103, "%d ", i);
...
void qh_printfacet3math(qhT *qh, FILE *fp, facetT *facet, qh_PRINT format, int notfirst) {
    ...
    if (notfirst) qh_fprintf(qh, fp, 9105, ",\n");
    vertices = qh_facet3vertex(qh, facet);
...
void qh_printfacet3vertex(qhT *qh, FILE *fp, facetT *facet, qh_PRINT format) {
    ...
    vertices = qh_facet3vertex(qh, facet);
    if (format == qh_PRINToff) qh_fprintf(qh, fp, 9111, "%d ", qh_setsize(qh, vertices));
    FOREACHvertex_(vertices) qh_fprintf(qh, fp, 9112, "%d ", qh_pointid(qh, vertex->point));
    ...
    FOREACHridge_(facet->ridges) {
        ...
        if (qh->DOintersections)
            ...
            FOREACHvertex_(ridge->vertices) {
                ...
                point = qh_projectpoint(qh, vertex->point, facet, dist);
                for (k = 0; k < qh->hull_dim; k++) {
                    if (k != qh->DROPdim) qh_fprintf(qh, fp, 9116, "%8.4g ", point[k]);
                }
...
void qh_printfacet4geom_simplicial(qhT *qh, FILE *fp, facetT *facet, realT color[3]) {
    ...
    FOREACHneighbor_(facet) {
        ...
        if (qh->DOintersections)
            ...
            FOREACHvertex_(vertices) {
                for (k = 0; k < qh->hull_dim; k++) {
                    if (k != qh->DROPdim) qh_fprintf(qh, fp, 9121, "%8.4g ", vertex->point[k]);
                }
...
void qh_printfacetNvertex_nonsimplicial(qhT *qh, FILE *fp, facetT *facet, int id, qh_PRINT format) {
    ...
    FOREACHridge_(facet->ridges) {
        if (format == qh_PRINTtriangles) qh_fprintf(qh, fp, 9124, "%d ", qh->hull_dim);
...
void qh_printfacetheader(qhT *qh, FILE *fp, facetT *facet) {
    ...
    if (facet->simplicial) qh_fprintf(qh, fp, 9140, " simplicial");
    if (facet->tricoplanar) qh_fprintf(qh, fp, 9141, " tricoplanar");
    if (facet->upperdelaunay) qh_fprintf(qh, fp, 9142, " upperDelaunay");
    if (facet->visible) qh_fprintf(qh, fp, 9143, " visible");
    if (facet->newfacet) qh_fprintf(qh, fp, 9144, " newfacet");
    if (facet->tested) qh_fprintf(qh, fp, 9145, " tested");
    if (!facet->good) qh_fprintf(qh, fp, 9146, " notG");
    if (facet->seen && qh->IStracing) qh_fprintf(qh, fp, 9147, " seen");
    if (facet->seen2 && qh->IStracing) qh_fprintf(qh, fp, 9418, " seen2");
    if (facet->isarea) qh_fprintf(qh, fp, 9419, " isarea");
    if (facet->coplanarhorizon) qh_fprintf(qh, fp, 9148, " coplanarhorizon");
    if (facet->mergehorizon) qh_fprintf(qh, fp, 9149, " mergehorizon");
    if (facet->cycledone) qh_fprintf(qh, fp, 9420, " cycledone");
    if (facet->keepcentrum) qh_fprintf(qh, fp, 9150, " keepcentrum");
    if (facet->dupridge) qh_fprintf(qh, fp, 9151, " dupridge");
    if (facet->mergeridge && !facet->mergeridge2) qh_fprintf(qh, fp, 9152, " mergeridge1");
    if (facet->mergeridge2) qh_fprintf(qh, fp, 9153, " mergeridge2");
    if (facet->newmerge) qh_fprintf(qh, fp, 9154, " newmerge");
    if (facet->flipped) qh_fprintf(qh, fp, 9155, " flipped");
    if (facet->notfurthest) qh_fprintf(qh, fp, 9156, " notfurthest");
    if (facet->degenerate) qh_fprintf(qh, fp, 9157, " degenerate");
    if (facet->redundant) qh_fprintf(qh, fp, 9158, " redundant");
...
void qh_printfacetridges(qhT *qh, FILE *fp, facetT *facet) {
    ...
    if (facet->visible && qh->NEWfacets) {
        ...
        FOREACHridge_(facet->ridges) {
            if (!ridge->seen) qh_printridge(qh, fp, ridge);
        }
    }
} /* printfacetridges */
...
*/
void qh_printfacets(qhT *qh, FILE *fp, qh_PRINT format, facetT *facetlist, setT *facets,
                    boolT printall) {
    int numfacets, numsimplicial, numridges, totneighbors, numcoplanars, numtricoplanars;
    facetT *facet, **facetp;
    setT *vertices;
    ...
    qh->RANDOMdist = False;
    if (qh->CDDoutput &&
        (format == qh_PRINTcentrums || format == qh_PRINTpointintersect || format == qh_PRINToff))
        qh_fprintf(
            qh, qh->ferr, 7056,
    ...
    if (format == qh_PRINTnone)
        ...
        qh_fprintf(qh, fp, 9193, "10 %d %d %d %d %d %d %d %d %d %d\n2 ", qh->hull_dim,
                   qh->num_points + qh_setsize(qh, qh->other_points), qh->num_vertices,
                   qh->num_facets - qh->num_visible, qh_setsize(qh, vertices), numfacets,
                   numcoplanars, numfacets - numsimplicial, zzval_(Zdelvertextot), numtricoplanars);
        qh_settempfree(qh, &vertices);
...
void qh_printline3geom(qhT *qh, FILE *fp, pointT *pointA, pointT *pointB, realT color[3]) {
    ...
    if ((fabs(pA[0] - pB[0]) > 1e-3) || (fabs(pA[1] - pB[1]) > 1e-3) ||
        ...
        for (k = 0; k < 3; k++) qh_fprintf(qh, fp, 9205, "%8.4g ", pB[k]);
        qh_fprintf(qh, fp, 9206, " # p%d\n", qh_pointid(qh, pointB));
    } else
        qh_fprintf(qh, fp, 9207, "VECT 1 1 1 1 1\n");
    for (k = 0; k < 3; k++) qh_fprintf(qh, fp, 9208, "%8.4g ", pA[k]);
...
/*-<a                             href="qh-io_r.htm#TOC"
  ...
  notes:
    ...
    bumps qh.visit_id
*/
void qh_printneighborhood(qhT *qh, FILE *fp, qh_PRINT format, facetT *facetA, facetT *facetB,
                          boolT printall) {
    ...
    for (facet = facetA; facet; facet = ((facet == facetA) ? facetB : NULL)) {
        ...
        FOREACHneighbor_(facet) {
            ...
            if (printall || !qh_skipfacet(qh, neighbor)) qh_setappend(qh, &facets, neighbor);
        }
    }
...
void qh_printpointid(qhT *qh, FILE *fp, const char *string, int dim, pointT *point, int id) {
    ...
    if (string) {
        ...
        if (id != qh_IDunknown && id != qh_IDnone) qh_fprintf(qh, fp, 9212, " p%d: ", id);
    }
    for (k = dim; k--;) {
        r = *point++;
        if (string)
...
void qh_printpoint3(qhT *qh, FILE *fp, pointT *point) {
    ...
    for (k = 0; k < 3; k++) qh_fprintf(qh, fp, 9216, "%8.4g ", p[k]);
...
void qh_printpoints_out(qhT *qh, FILE *fp, facetT *facetlist, setT *facets, boolT printall) {
    ...
    FOREACHpoint_i_(qh, points) {
        if (point) {
            if (qh->CDDoutput) qh_fprintf(qh, fp, 9220, "1 ");
            qh_printpoint(qh, fp, NULL, point);
        }
    }
    if (qh->CDDoutput) qh_fprintf(qh, fp, 9221, "end\n");
    qh_settempfree(qh, &points);
} /* printpoints_out */
...
*/
void qh_printpointvect(qhT *qh, FILE *fp, pointT *point, coordT *normal, pointT *center,
    ...
    if (center) qh_normalize2(qh, diff, qh->hull_dim, True, NULL, NULL);
...
*/
void qh_printpointvect2(qhT *qh, FILE *fp, pointT *point, coordT *normal, pointT *center,
                        realT radius) {
...
void qh_printridge(qhT *qh, FILE *fp, ridgeT *ridge) {
    ...
    if (ridge->tested) qh_fprintf(qh, fp, 9223, " tested");
    if (ridge->nonconvex) qh_fprintf(qh, fp, 9224, " nonconvex");
    if (ridge->mergevertex) qh_fprintf(qh, fp, 9421, " mergevertex");
    if (ridge->mergevertex2) qh_fprintf(qh, fp, 9422, " mergevertex2");
    if (ridge->simplicialtop) qh_fprintf(qh, fp, 9425, " simplicialtop");
    if (ridge->simplicialbot) qh_fprintf(qh, fp, 9423, " simplicialbot");
...
*/
void qh_printvdiagram(qhT *qh, FILE *fp, qh_PRINT format, facetT *facetlist, setT *facets,
                      boolT printall) {
    setT *vertices;
...
/*-<a                             href="qh-io_r.htm#TOC"
  ...
  see:
    qh_eachvoronoi_all()
*/
int qh_printvdiagram2(qhT *qh, FILE *fp, printvridgeT printvridge, setT *vertices,
                      qh_RIDGE innerouter, boolT inorder) {
    int totcount = 0;
...
void qh_printvertex(qhT *qh, FILE *fp, vertexT *vertex) {
    ...
    if (vertex->deleted) qh_fprintf(qh, fp, 9237, " deleted");
    if (vertex->delridge) qh_fprintf(qh, fp, 9238, " delridge");
    if (vertex->newfacet) qh_fprintf(qh, fp, 9415, " newfacet");
    if (vertex->seen && qh->IStracing) qh_fprintf(qh, fp, 9416, " seen");
    if (vertex->seen2 && qh->IStracing) qh_fprintf(qh, fp, 9417, " seen2");
    qh_fprintf(qh, fp, 9239, "\n");
    if (vertex->neighbors) {
        qh_fprintf(qh, fp, 9240, "  neighbors:");
        FOREACHneighbor_(vertex) {
            if (++count % 100 == 0) qh_fprintf(qh, fp, 9241, "\n     ");
...
*/
void qh_printvertexlist(qhT *qh, FILE *fp, const char *string, facetT *facetlist, setT *facets,
                        boolT printall) {
    vertexT *vertex, **vertexp;
    setT *vertices;
...
void qh_printvneighbors(qhT *qh, FILE *fp, facetT *facetlist, setT *facets, boolT printall) {
    ...
    qh_countfacets(qh, facetlist, facets, printall, &numfacets, &numsimplicial, &totneighbors,
                   &numridges, &numcoplanars, &numtricoplanars); /* sets facet->visitid */
    ...
    FOREACHvertex_(vertices) qh_point_add(qh, vertex_points, vertex->point, vertex);
    FORALLfacet_(facetlist) {
        FOREACHpoint_(facet->coplanarset) qh_point_add(qh, coplanar_points, point, facet);
    }
    FOREACHfacet_(facets) {
        FOREACHpoint_(facet->coplanarset) qh_point_add(qh, coplanar_points, point, facet);
    }
    FOREACHvertex_i_(qh, vertex_points) {
        if (vertex) {
            ...
            FOREACHneighbor_(vertex) qh_fprintf(
                qh, fp, 9250, " %d", neighbor->visitid ? neighbor->visitid - 1 : 0 - neighbor->id);
...
*/
void qh_printvoronoi(qhT *qh, FILE *fp, qh_PRINT format, facetT *facetlist, setT *facets,
                     boolT printall) {
    ...
    if (format == qh_PRINTgeom)
        ...
        qh_fprintf(qh, fp, 9255, "%d\n%d %d 1\n", qh->hull_dim - 1, numcenters,
                   qh_setsize(qh, vertices));
    if (format == qh_PRINTgeom) {
        for (k = qh->hull_dim - 1; k--;) qh_fprintf(qh, fp, 9256, qh_REAL_1, 0.0);
        qh_fprintf(qh, fp, 9257, " 0 # infinity not used\n");
    } else {
        for (k = qh->hull_dim - 1; k--;) qh_fprintf(qh, fp, 9258, qh_REAL_1, qh_INFINITE);
        qh_fprintf(qh, fp, 9259, "\n");
    }
    FORALLfacet_(facetlist) {
        if (facet->visitid && facet->visitid < numfacets) {
            if (format == qh_PRINTgeom) qh_fprintf(qh, fp, 9260, "# %d f%d\n", vid++, facet->id);
            qh_printcenter(qh, fp, format, NULL, facet);
        }
    }
    FOREACHfacet_(facets) {
        if (facet->visitid && facet->visitid < numfacets) {
            if (format == qh_PRINTgeom) qh_fprintf(qh, fp, 9261, "# %d f%d\n", vid++, facet->id);
            qh_printcenter(qh, fp, format, NULL, facet);
        }
    }
    FOREACHvertex_i_(qh, vertices) {
...
/*-<a                             href="qh-io_r.htm#TOC"
  ...
  see:
    ...
    qh_eachvoronoi()
*/
void qh_printvnorm(qhT *qh, FILE *fp, vertexT *vertex, vertexT *vertexA, setT *centers,
                   boolT unbounded) {
    pointT *normal;
    realT offset;
    int k;
    ...
    qh_fprintf(qh, fp, 9271, "%d %d %d ", 2 + qh->hull_dim, qh_pointid(qh, vertex->point),
               qh_pointid(qh, vertexA->point));
    for (k = 0; k < qh->hull_dim - 1; k++) qh_fprintf(qh, fp, 9272, qh_REAL_1, normal[k]);
...
*/
void qh_printvridge(qhT *qh, FILE *fp, vertexT *vertex, vertexT *vertexA, setT *centers,
                    boolT unbounded) {
    facetT *facet, **facetp;
    QHULL_UNUSED(unbounded);

    qh_fprintf(qh, fp, 9275, "%d %d %d", qh_setsize(qh, centers) + 2, qh_pointid(qh, vertex->point),
               qh_pointid(qh, vertexA->point));
...
coordT *qh_readpoints(qhT *qh, int *numpoints, int *dimension, boolT *ismalloc) {
    ...
    while (!numinput && (s = fgets(firstline, qh_MAXfirst, qh->fin))) {
        linecount++;
        if (!memcmp(s, "begin", (size_t)5) || !memcmp(s, "BEGIN", (size_t)5)) wasbegin = True;
        while (*s) {
            while (isspace(*s)) s++;
            if (!*s) break;
            if (!isdigit(*s)) {
    ...
    *ismalloc = True;                   /* use malloc since memory not setup */
    ...
    while ((s = (isfirst ? s : fgets(qh->line, qh->maxline, qh->fin)))) {
        ...
        while (*s) {
            ...
            value = qh_strtod(s, &t);
            if (s == t) {
                if (!*qh->rbox_command) strncat(qh->rbox_command, s, sizeof(qh->rbox_command) - 1);
        ...
        if (!isfirst && s - qh->line >= qh->maxline) {
            qh_fprintf(qh, qh->ferr, 6078,
                       "qhull input error: line %d contained more than %d characters\n", linecount,
                       (int)(s - qh->line)); /* WARN64 */
            qh_errexit(qh, qh_ERRinput, NULL, NULL);
        }
        isfirst = False;
    }
    if (qh->rbox_command[0])
        qh->rbox_command[strlen(qh->rbox_command) - 1] =
