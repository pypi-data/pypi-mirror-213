...
int qh_new_qhull(qhT *qh, int dim, int numpoints, coordT *points, boolT ismalloc, char *qhull_cmd,
    ...
    /* gcc may issue a "might be clobbered" warning for dim, points, and ismalloc [-Wclobbered].
       ...
       See
       http://stackoverflow.com/questions/7721854/what-sense-do-these-clobbered-variable-warnings-make
     */
...
void qh_errexit(qhT *qh, int exitcode, facetT *facet, ridgeT *ridge) {
    ...
    if (qh->FORCEoutput && (qh->QHULLfinished || (!facet && !ridge)))
        ...
        if (qh->PRINTprecision) qh_printstats(qh, qh->ferr, qh->qhstat.precision, NULL);
    }
    if (!exitcode)
...
void qh_errprint(qhT *qh, const char *string, facetT *atfacet, facetT *otherfacet, ridgeT *atridge,
    ...
    if (qh->fout && qh->FORCEoutput && atfacet && !qh->QHULLfinished && !qh->IStracing) {
        ...
        for (i = 0; i < qh_PRINTEND; i++) /* use fout for geomview output */
            qh_printneighborhood(qh, qh->fout, qh->PRINTout[i], atfacet, otherfacet, !qh_ALL);
    }
} /* errprint */
...
void qh_printfacetlist(qhT *qh, facetT *facetlist, setT *facets, boolT printall) {
    ...
    if (facetlist) qh_checklists(qh, facetlist);
    ...
    if (facetlist) {
        ...
        FORALLfacet_(facetlist) qh_printafacet(qh, qh->ferr, qh_PRINTfacets, facet, printall);
    }
    if (facets) {
        qh_fprintf(qh, qh->ferr, 9414, "printfacetlist: %d facets\n", qh_setsize(qh, facets));
        FOREACHfacet_(facets) qh_printafacet(qh, qh->ferr, qh_PRINTfacets, facet, printall);
    }
