...
void qh_appendvertex(qhT *qh, vertexT *vertex) {
    ...
    qh->num_vertices++;
    trace4((qh, qh->ferr, 4045,
            "qh_appendvertex: append v%d to qh.newvertex_list and set v.newfacet\n", vertex->id));
} /* appendvertex */
...
void qh_attachnewfacets(qhT *qh /* qh.visible_list, qh.newfacet_list */) {
    ...
    FORALLnew_facets {
        ...
        if (horizon->simplicial) {
            ...
            if (visible) {
                ...
                qh_fprintf(
                    qh, qh->ferr, 6102,
                    "qhull internal error (qh_attachnewfacets): could not find visible facet for horizon f%d of newfacet f%d\n",
    ...
    trace4((
        qh, qh->ferr, 4094,
        "qh_attachnewfacets: clear f.ridges and f.neighbors for visible facets, may become invalid before qh_deletevisible\n"));
    FORALLvisible_facets {
        if (visible->ridges) SETfirst_(visible->ridges) = NULL;
...
boolT qh_checkflipped(qhT *qh, facetT *facet, realT *distp, boolT allerror) {
    ...
    if ((allerror && dist >= -qh->DISTround) || (!allerror && dist > 0.0)) {
        facet->flipped = True;
        trace0((qh, qh->ferr, 19,
                "qh_checkflipped: facet f%d flipped, allerror? %d, distance= %6.12g during p%d\n",
                facet->id, allerror, dist, qh->furthest_id));
        if (qh->num_facets >
            qh->hull_dim + 1) { /* qh_initialhull reverses orientation if !qh_checkflipped */
...
void qh_delfacet(qhT *qh, facetT *facet) {
    ...
    if (facet->ridges) qh_setfree(qh, &(facet->ridges));
    qh_setfree(qh, &(facet->vertices));
    if (facet->outsideset) qh_setfree(qh, &(facet->outsideset));
...
void qh_deletevisible(qhT *qh /* qh.visible_list */) {
    ...
    for (visible = qh->visible_list; visible && visible->visible;
        ...
        numvisible++;
        qh_delfacet(qh, visible); /* f.ridges deleted or moved elsewhere, deleted f.vertices on
                                     qh.del_vertices */
    }
    ...
    FOREACHvertex_(qh->del_vertices) qh_delvertex(qh, vertex);
...
*/
setT *qh_facetintersect(qhT *qh, facetT *facetA, facetT *facetB, int *skipA, int *skipB,
                        int prepend) {
    setT *intersect;
    ...
    if (i >= dim || j >= dim) {
        qh_fprintf(
            qh, qh->ferr, 6104,
            "qhull internal error (qh_facetintersect): f%d or f%d not in other's neighbors\n",
            facetA->id, facetB->id);
...
#pragma warning(disable : 4311) /* 'type cast': pointer truncation from 'void*' to 'ptr_intT' */
    ...
    switch (size - firstindex) {
        ...
        case 3:
            hash =
                (ptr_intT)(*elemp) + (ptr_intT)elemp[1] + (ptr_intT)elemp[2] - (ptr_intT)skipelem;
            break;
        case 4:
    ...
    if (hashsize < 0) {
        qh_fprintf(qh, qh->ferr, 6202,
                   "qhull internal error: negative hashsize %d passed to qh_gethash [poly_r.c]\n",
                   hashsize);
...
facetT *qh_getreplacement(qhT *qh, facetT *visible) {
    ...
    while (result && result->visible) {
        ...
        if (count++ > qh->facet_id) qh_infiniteloop(qh, visible);
    }
    return result;
}
...
facetT *qh_makenewfacet(qhT *qh, setT *vertices, boolT toporient, facetT *horizon) {
    ...
    if (horizon) qh_setappend(qh, &(newfacet->neighbors), horizon);
...
void qh_makenewplanes(qhT *qh /* qh.newfacet_list */) {
    ...
    trace4((qh, qh->ferr, 4074,
            "qh_makenewplanes: make new hyperplanes for facets on qh.newfacet_list f%d\n",
            qh->newfacet_list->id));
    FORALLnew_facets {
        if (!newfacet->mergehorizon) qh_setfacetplane(qh, newfacet); /* updates Wnewvertexmax */
    }
    if (qh->JOGGLEmax < REALmax / 2) minimize_(qh->min_vertex, -wwval_(Wnewvertexmax));
} /* makenewplanes */
...
facetT *qh_makenew_nonsimplicial(qhT *qh, facetT *visible, vertexT *apex, int *numnew) {
    ...
    FOREACHridge_(visible->ridges) {
        ...
        if (neighbor->visible) {
            if (!qh->NEWtentative) {
                if (neighbor->visitid == qh->visit_id) {
                    if (qh->traceridge == ridge) qh->traceridge = NULL;
            ...
            if (qh->NEWtentative) {
                if (!neighbor->simplicial) qh_setappend(qh, &(newfacet->ridges), ridge);
...
facetT *qh_makenew_simplicial(qhT *qh, facetT *visible, vertexT *apex, int *numnew) {
    ...
    FOREACHneighbor_(visible) {
        if (!neighbor->seen && !neighbor->visible) {
            ...
            if (!qh->NEWtentative) SETelem_(neighbor->neighbors, horizonskip) = newfacet;
            trace4((
                qh, qh->ferr, 4049,
                "qh_makenew_simplicial: create facet f%d top %d from v%d and horizon f%d skip %d top %d and visible f%d skip %d, flip? %d\n",
                newfacet->id, toporient, apex->id, neighbor->id, horizonskip, neighbor->toporient,
                visible->id, visibleskip, flip));
        }
    }
    return newfacet;
} /* makenew_simplicial */
    ...
    for (scan = hash; (facet = SETelemt_(qh->hash_table, scan, facetT));
        ...
        if (qh_matchvertices(qh, 1, newfacet->vertices, newskip, facet->vertices, &skip, &same)) {
            if (SETelem_(newfacet->vertices, newskip) == SETelem_(facet->vertices, skip)) {
                ...
                qh_fprintf(
                    qh, qh->ferr, 7084,
                    "qhull topology warning (qh_matchneighbor): will merge vertices to undo new facets -- f%d and f%d have the same vertices (skip %d, skip %d) and same horizon ridges to f%d and f%d\n",
                    facet->id, newfacet->id, skip, newskip,
                    SETfirstt_(facet->neighbors, facetT)->id,
                    SETfirstt_(newfacet->neighbors, facetT)->id);
                /* will rename a vertex (QH3053).  The fault was duplicate ridges (same vertices) in
                 * different facets due to a previous rename.  Expensive to detect beforehand */
            }
            ...
            if (!qh->PREmerge && !qh->MERGEexact) {
                ...
                qh_fprintf(
                    qh, qh->ferr, 6107,
                    "qhull topology error: facets f%d, f%d and f%d meet at a ridge with more than 2 neighbors.  Can not continue due to no qh.PREmerge and no 'Qx' (MERGEexact)\n",
            ...
            if (matchfacet != qh_DUPLICATEridge) {
                ...
                if (matchfacet) {
                    ...
                    if (matchskip < 0) {
                        qh_fprintf(
                            qh, qh->ferr, 6260,
                            "qhull topology error (qh_matchneighbor): matchfacet f%d is in f%d neighbors but not vice versa.  Can not continue.\n",
                            matchfacet->id, facet->id);
                        qh_errexit2(qh, qh_ERRtopology, matchfacet, facet);
                    }
                    SETelem_(matchfacet->neighbors, matchskip) =
                        qh_DUPLICATEridge; /* matchskip>=0 by QH6260 */
            ...
            trace4((
                ...
                (matchfacet == qh_DUPLICATEridge ? -2 : getid_(matchfacet)), ismatch, hash));
            return; /* end of duplicate ridge */
        }
    }
    if (!newfound) SETelem_(qh->hash_table, scan) = newfacet; /* same as qh_addhash */
    (*hashcount)++;
...
coordT qh_matchnewfacets(qhT *qh /* qh.newfacet_list */) {
    ...
    FORALLnew_facets {
        if (!newfacet->simplicial) {
            qh_fprintf(
                qh, qh->ferr, 6377,
                "qhull internal error (qh_matchnewfacets): expecting simplicial facets on qh.newfacet_list f%d for qh_matchneighbors, qh_matchneighbor, and qh_matchdupridge.  Got non-simplicial f%d\n",
                ...
                if (newfacet->dupridge) {
                    FOREACHneighbor_i_(qh, newfacet) {
                        if (neighbor == qh_DUPLICATEridge) {
                            maxdist2 =
                                qh_matchdupridge(qh, newfacet, neighbor_i, hashsize, &hashcount);
    ...
    if (qh->IStracing >= 3) {
        ...
        qh_fprintf(
            qh, qh->ferr, 3063,
            "qh_matchnewfacets: maxdupdist %2.2g, new facets %d, unused hash entries %d, hashsize %d\n",
...
int qh_pointid(qhT *qh, pointT *point) {
    ...
    else if (point >= qh->first_point && point < qh->first_point + qh->num_points * qh->hull_dim) {
...
void qh_removefacet(qhT *qh, facetT *facet) {
    ...
    qh->num_facets--;
    trace4((qh, qh->ferr, 4057,
            "qh_removefacet: removed f%d from facet_list, newfacet_list, and visible_list\n",
            facet->id));
} /* removefacet */
...
/*-<a                             href="qh-poly_r.htm#TOC"
  ...
  returns:
    ...
    if not qh.VERTEXneighbors (not merging)
      interior vertices of visible facets added to qh.del_vertices for later partitioning as
  coplanar points

  notes
    [jan'19] split off qh_update_vertexneighbors_cone.  Optimize the remaining cases in a future
  release called by qh_triangulate_facet after triangulating a non-simplicial facet, followed by
  reset_lists called by qh_triangulate after triangulating null and mirror facets called by
  qh_all_vertexmerges after calling qh_merge_pinchedvertices
...
void qh_update_vertexneighbors(qhT *qh /* qh.newvertex_list, newfacet_list, visible_list */) {
    ...
    if (qh->VERTEXneighbors) {
        trace3((
            qh, qh->ferr, 3013,
            "qh_update_vertexneighbors: update v.neighbors for qh.newvertex_list (v%d) and qh.newfacet_list (f%d)\n",
        ...
        FORALLvertex_(qh->newvertex_list) {
            ...
            if (neighborcount) {
                trace4((
                    qh, qh->ferr, 4046,
                    "qh_update_vertexneighbors: delete %d of %d vertex neighbors for v%d.  Removes to-be-deleted, visible facets\n",
        ...
        FORALLnew_facets {
            if (qh->first_newfacet && newfacet->id >= qh->first_newfacet) {
                FOREACHvertex_(newfacet->vertices) qh_setappend(qh, &vertex->neighbors, newfacet);
            } else { /* called after qh_merge_pinchedvertices.  In 7-D, many more neighbors than new
                        facets.  qh_setin is expensive */
                FOREACHvertex_(newfacet->vertices) qh_setunique(qh, &vertex->neighbors, newfacet);
            }
        }
        trace3((qh, qh->ferr, 3058,
                "qh_update_vertexneighbors: delete interior vertices for qh.visible_list (f%d)\n",
            ...
            FOREACHvertex_(visible->vertices) {
                if (!vertex->newfacet && !vertex->deleted) {
                    ...
                    if (neighbor)
                        ...
                        trace2((
                            qh, qh->ferr, 2041,
                            "qh_update_vertexneighbors: delete interior vertex p%d(v%d) of visible f%d\n",
        ...
        trace3((qh, qh->ferr, 3058,
                "qh_update_vertexneighbors: delete old vertices for qh.visible_list (f%d)\n",
                getid_(qh->visible_list)));
        FORALLvisible_facets {
            FOREACHvertex_(visible->vertices) {
                if (!vertex->newfacet && !vertex->deleted) {
                    ...
                    trace2((
                        qh, qh->ferr, 2042,
                        "qh_update_vertexneighbors: will delete interior vertex p%d(v%d) of visible f%d\n",
                        qh_pointid(qh, vertex->point), vertex->id, visible->id));
                }
            }
        }
...
void qh_update_vertexneighbors_cone(qhT *qh /* qh.newvertex_list, newfacet_list, visible_list */) {
    ...
    if (qh->VERTEXneighbors) {
        trace3((
            qh, qh->ferr, 3059,
            "qh_update_vertexneighbors_cone: update v.neighbors for qh.newvertex_list (v%d) and qh.newfacet_list (f%d)\n",
            getid_(qh->newvertex_list), getid_(qh->newfacet_list)));
        FORALLvertex_(qh->newvertex_list) {
            delcount = 0;
            FOREACHneighbor_(vertex) {
                if (neighbor->visible) { /* alternative design is a loop over visible facets, but
                                            needs qh_setdel() */
                    delcount++;
            ...
            if (delcount) {
                trace4(
                    (qh, qh->ferr, 4021,
                     "qh_update_vertexneighbors_cone: deleted %d visible vertexneighbors of v%d\n",
                     delcount, vertex->id));
            }
        }
        FORALLnew_facets {
        ...
        trace3((
            qh, qh->ferr, 3065,
            "qh_update_vertexneighbors_cone: delete interior vertices, if any, for qh.visible_list (f%d)\n",
            getid_(qh->visible_list)));
        FORALLvisible_facets {
            FOREACHvertex_(visible->vertices) {
                if (!vertex->newfacet && !vertex->deleted) {
                    FOREACHneighbor_(
                        vertex) { /* this can happen under merging, qh_checkfacet QH4025 */
                        if (!neighbor->visible) break;
                    }
                    if (neighbor)
                        qh_setdel(vertex->neighbors, visible);
                        ...
                        trace2((
                            qh, qh->ferr, 2102,
                            "qh_update_vertexneighbors_cone: will delete interior vertex p%d(v%d) of visible f%d\n",
                            qh_pointid(qh, vertex->point), vertex->id, visible->id));
                    }
                }
            }
        ...
        trace3(
            (qh, qh->ferr, 3066,
             "qh_update_vertexneighbors_cone: delete interior vertices for qh.visible_list (f%d)\n",
             getid_(qh->visible_list)));
        FORALLvisible_facets {
            FOREACHvertex_(visible->vertices) {
                if (!vertex->newfacet && !vertex->deleted) {
