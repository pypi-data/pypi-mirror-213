...
/*-<a                             href="qh-geom_r.htm#TOC"
  ...
  notes:
    qh.STOPcone and qh.STOPpoint assume that qh_IDunknown==-1 (other negative numbers indicate
  points) qh_IDunknown is also returned by getid_() for unknown facet, ridge, or vertex
*/
#define pointT coordT
typedef enum { qh_IDnone = -3, qh_IDinterior = -2, qh_IDunknown = -1 } qh_pointT;
...
typedef enum {
    qh_PRINTnone = 0,
    qh_PRINTarea,
    qh_PRINTaverage, /* 'Fa' 'FV' 'Fc' 'FC' */
    qh_PRINTcoplanars,
    qh_PRINTcentrums,
    qh_PRINTfacets,
    qh_PRINTfacets_xridge, /* 'f' 'FF' 'G' 'FI' 'Fi' 'Fn' */
    qh_PRINTgeom,
    qh_PRINTids,
    qh_PRINTinner,
    qh_PRINTneighbors,
    qh_PRINTnormals,
    qh_PRINTouter,
    qh_PRINTmaple, /* 'n' 'Fo' 'i' 'm' 'Fm' 'FM', 'o' */
    qh_PRINTincidences,
    qh_PRINTmathematica,
    qh_PRINTmerges,
    qh_PRINToff,
    qh_PRINToptions,
    qh_PRINTpointintersect, /* 'FO' 'Fp' 'FP' 'p' 'FQ' 'FS' */
    qh_PRINTpointnearest,
    qh_PRINTpoints,
    qh_PRINTqhull,
    qh_PRINTsize,
    qh_PRINTsummary,
    qh_PRINTtriangles, /* 'Fs' 'Ft' 'Fv' 'FN' 'Fx' */
    qh_PRINTvertices,
    qh_PRINTvneighbors,
    qh_PRINTextremes,
    qh_PRINTEND
...
#define qh_ERRtopology \
    7 /* topology error, maybe due to nearly adjacent vertices, calls qh_printhelp_topology */
#define qh_ERRwide \
    8 /* wide facet error, maybe due to nearly adjacent vertices, calls qh_printhelp_wide */
...
struct facetT {
    ...
    coordT maxoutside; /* max computed distance of point to facet
                       ...
                       Initial maxoutside is qh.DISTround, otherwise distance tests need to account
                       for DISTround */
#endif
    ...
    setT *outsideset;     /* set of points outside this facet
                             ...
                             if NARROWhull, includes coplanars (less than qh.MINoutside) for
                             partitioning*/
    ...
    unsigned int id; /* unique identifier from qh.facet_id, 1..qh.facet_id, 0 is sentinel, printed
                        as 'f%d' */
...
#define qh_MAXnummerge 511     /* 2^9-1 */
    ...
    flagT mergehorizon : 1;    /* True if will merge into horizon (its first neighbor w/
                                  f.coplanarhorizon). */
    flagT cycledone : 1;       /* True if mergecycle_all already done */
    flagT tested : 1;          /* True if facet convexity has been tested (false after merge */
    flagT keepcentrum : 1;     /* True if keep old centrum after a merge, or marks owner for
                                  ->tricoplanar     Set by qh_updatetested if more than qh_MAXnewcentrum
                                  extra vertices     Set by qh_mergefacet if |maxdist| > qh.WIDEfacet */
...
struct ridgeT {
    ...
    flagT simplicialbot : 1; /* True if bottom was simplicial (original vertices)
                              use qh_test_centrum_merge if top and bot, need to retest since centrum
                              may change */
};
...
struct vertexT {
    ...
    setT *neighbors;   /* neighboring facets of vertex, qh_vertexneighbors()
                          ...
                          updated by merges
                          qh_order_vertexneighbors by 2-d (orientation) 3-d (adjacency), n-d
                          (f.visitid,id) */
...
#define QHULL_LIB_CHECK                                                                        \
    qh_lib_check(QHULL_LIB_TYPE, sizeof(qhT), sizeof(vertexT), sizeof(ridgeT), sizeof(facetT), \
                 sizeof(setT), sizeof(qhmemT));
#define QHULL_LIB_CHECK_RBOX                                                                      \
    qh_lib_check(QHULL_LIB_TYPE, sizeof(qhT), sizeof(vertexT), sizeof(ridgeT), sizeof(facetT), 0, \
                 0);
...
struct qhT {
    ...
    */
    ...
    boolT
        ALLOWwide; /* true 'Q12' if allow wide facets and wide dupridges, c.f. qh_WIDEmaxoutside */
    boolT
        ANGLEmerge; /* true 'Q1' if sort potential merges by type/angle instead of type/distance  */
    ...
    boolT MERGEvertices;     /* true if merging redundant vertices, 'Q3' disables or qh.hull_dim >
                                qh_DIMmergeVertex */
    realT MINvisible;        /* 'Vn' min. distance for a facet to be visible */
    boolT NOnarrow;          /* true 'Q10' if no special processing for narrow distributions */
    boolT NOnearinside; /* true 'Q8' if ignore near-inside points when partitioning, qh_check_points
                           may fail */
    ...
    int ROTATErandom;   /* 'QRn' n<-1 random seed, n==-1 time is seed, n==0 random rotation by time,
                           n>0 rotate input */
    ...
    int TRACEpoint;    /* 'TPn' start tracing when point n is a vertex, use qh_IDunknown (-1) after
                          qh_buildhull and qh_postmerge */
    ...
    boolT GETarea; /* true 'Fa', 'FA', 'FS', 'PAn', 'PFn' if compute facet area/Voronoi volume in
                      io_r.c */
    ...
    boolT
        ZEROcentrum; /* true if 'C-0' or 'C-0 Qx' and not post-merging or 'A-n'.  Sets ZEROall_ok */
    ...
    */
    ...
    realT DISTround;      /* max round off error for distances, qh.SETroundoff ('En') overrides
                             qh_distround */
    ...
    */
    ...
    facetT *facet_next;         /* next facet for buildhull()
                                   ...
                                   NARROWhull: previous facets may have coplanar outside sets for
                                   qh_outcoplanar */
    ...
    facetT *visible_list;       /* list of visible facets preceding newfacet_list,
                                   ...
                                   qh_triangulate appends mirror facets to visible_list at end of
                                   facet_list       qh_postmerge sets visible_list to facet_list
    ...
    ridgeT *traceridge;          /*   set in newridge, undone in delridge, errexit, errexit2,
                                    makenew_nonsimplicial, mergecycle_ridges */
    ...
    */
    ...
    pointT *coplanar_apex;  /* last apex declared a coplanar point by qh_getpinchedmerges, prevents
                               infinite loop */
    ...
    boolT isRenameVertex; /* true during qh_merge_pinchedvertices, disables duplicate ridge vertices
                             in qh_checkfacet */
    ...
    boolT NEWtentative;          /* true while new facets are tentative due to !qh.IGNOREpinched or
                                    qh.ONLYgood          from qh_makecone to qh_attachnewfacets */
    boolT findbestnew;           /* true if partitioning calls qh_findbestnew */
    boolT findbest_notsharp;     /* true if new facets are at least 90 degrees */
    boolT NOerrexit;             /* true if qh.errexit is not available, cleared after setjmp.  See
                                    qh.ERREXITcalled */
    ...
    unsigned int repart_facetid; /* previous facetid to prevent recursive
                                    qh_partitioncoplanar+qh_partitionpoint */
...
#define FORALLpoint_(qh, points, num)                                                      \
    for (point = (points), pointtemp = (points) + qh->hull_dim * (num); point < pointtemp; \
         point += qh->hull_dim)
...
void qh_errprint(qhT *qh, const char *string, facetT *atfacet, facetT *otherfacet, ridgeT *atridge,
                 vertexT *atvertex);
int qh_new_qhull(qhT *qh, int dim, int numpoints, coordT *points, boolT ismalloc, char *qhull_cmd,
                 FILE *outfile, FILE *errfile);
...
facetT *qh_findbest(qhT *qh, pointT *point, facetT *startfacet, boolT bestoutside, boolT newfacets,
                    boolT noupper, realT *dist, boolT *isoutside, int *numpart);
