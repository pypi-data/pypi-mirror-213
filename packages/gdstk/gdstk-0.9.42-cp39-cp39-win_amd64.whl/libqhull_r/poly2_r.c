...
boolT qh_addfacetvertex(qhT *qh, facetT *facet, vertexT *newvertex) {
    ...
    if (isnew) qh_setaddnth(qh, &facet->vertices, vertex_i, newvertex);
    return isnew;
} /* addfacetvertex */
...
below %2.2g of the nearest %sfacet.\n",
    ...
    FOREACHfacet_i_(qh, facets) { /* for each point with facet assignment */
        ...
        numpart++;
        bestfacet =
            qh_findbesthorizon(qh, !qh_IScheckmax, point, facet, qh_NOupper, &dist, &numpart);
...
void qh_check_maxout(qhT *qh) {
    facetT *facet, *bestfacet, *neighbor, **neighborp, *facetlist, *maxbestfacet = NULL, *minfacet,
                                                                   *maxfacet, *maxpointfacet;
    ...
    if (qh->VERTEXneighbors &&
        (qh->PRINTsummary || qh->KEEPinside || qh->KEEPcoplanar || qh->TRACElevel ||
         qh->PRINTstatistics || qh->VERIFYoutput || qh->CHECKfrequently ||
        ...
        FORALLvertices {
            FOREACHneighbor_(vertex) {
                ...
                if (dist < mindist) {
                    if (qh->min_vertex / minvertex_base > qh_WIDEmaxoutside &&
                        (qh->PRINTprecision || !qh->ALLOWwide)) {
...
#ifndef qh_NOtrace
                if (-dist > qh->TRACEdist || dist > qh->TRACEdist || neighbor == qh->tracefacet ||
                    vertex == qh->tracevertex) {
                    nearest = qh_vertex_bestdist(qh, neighbor->vertices);
                    qh_fprintf(
                        qh, qh->ferr, 8093,
            ...
            if (bestfacet && dist >= maxoutside) {
                if (qh->ONLYgood && !bestfacet->good &&
                    !((bestfacet = qh_findgooddist(qh, point, bestfacet, &dist, &facetlist)) &&
                      dist > maxoutside)) {
                    notgood++;
                } else if (dist / maxoutside_base > qh_WIDEmaxoutside &&
                           (qh->PRINTprecision || !qh->ALLOWwide)) {
                    nearest = qh_vertex_bestdist(qh, bestfacet->vertices);
                    if (nearest <
                        fmax_(qh->ONEmerge, qh->max_outside) * qh_RATIOcoplanaroutside * 2) {
                        qh_fprintf(
                            qh, qh->ferr, 7087,
    ...
    if (!qh->APPROXhull && maxoutside > qh->DISTround) { /* initial value for f.maxoutside */
        FORALLfacets {
            if (maxoutside < facet->maxoutside) {
                ...
                } else if (maxoutside + qh->DISTround <
                           facet->maxoutside) { /* maxoutside is computed distance, e.g., rbox 100 s
                                                   D3 t1547136913 | qhull R1e-3 Tcv Qc */
                    qh_fprintf(
                        qh, qh->ferr, 7082,
    ...
    trace1((
        ...
        qh_pointid(qh, minvertex->point), minvertex->id, qh->min_vertex, minfacet->id,
        qh_pointid(qh, maxpoint), maxpointfacet->id, qh->max_outside, maxfacet->id, notgood));
    if (!qh->ALLOWwide) {
        if (maxoutside / maxoutside_base > qh_WIDEmaxoutside) {
            ...
            if (maxoutside >
                (qh->ONEmerge * qh_WIDEmaxoutside2)) { /* wide facets may have been deleted */
                qh_fprintf(
                    qh, qh->ferr, 6298,
            ...
            if (qh->min_vertex <
                ...
                qh_fprintf(
                    ...
                    minvertex->id, minfacet->id, mindist,
                    -qh->min_vertex / (qh->ONEmerge + qh->DISTround));
                qh_errexit(qh, qh_ERRwide, minfacet, NULL);
            }
        }
    }
...
#else /* qh_NOmerge */
void qh_check_maxout(qhT *qh) { QHULL_UNUSED(qh) }
#endif
...
/*-<a                             href="qh-poly_r.htm#TOC"
  ...
  qh_check_output(qh)
    ...
    Maybe called after Voronoi output.  If so, it recomputes centrums since they are Voronoi centers
  instead.
*/
...
/*-<a                             href="qh-poly_r.htm#TOC"
  ...
  notes:
    ...
    Does not test against -qh.centrum_radius since repeated computations may have different
  round-off errors (e.g., 'Rn')
...
void qh_checkconvex(qhT *qh, facetT *facetlist, int fault) {
    ...
    realT dist;
    pointT *centrum;
    boolT waserror = False, centrum_warning = False, tempcentrum = False,
          first_nonsimplicial = False, tested_simplicial, allsimplicial;
    ...
    FORALLfacet_(facetlist) {
        ...
        if (qh->MERGING && (!qh->ZEROcentrum || !facet->simplicial)) {
            ...
            FOREACHneighbor_i_(qh, facet) {
                if (neighbor->tricoplanar) continue;
                ...
                if (dist >= -qh->DISTround) {
                    ...
                    if (dist > qh->DISTround) {
                        ...
                        qh_fprintf(
                            ...
                            facet->id, neighbor->id, qh_pointid(qh, vertex->point), vertex->id,
                            dist, neighbor->id);
                        errfacet1 = facet;
                        errfacet2 = neighbor;
                        waserror = True;
                    } else if (qh->ZEROcentrum) {
                        if (dist > 0.0) { /* qh_checkzero checked convex (dist < (-
                                             2*qh->DISTround)), computation may differ e.g. 'Rn' */
                        ...
                        trace0((
                            ...
                            facet->id, neighbor->id, qh_pointid(qh, vertex->point), vertex->id,
                            dist, neighbor->id, qh->furthest_id));
                    }
                }
            }
        }
        if (!allsimplicial) {
            ...
            if (qh->CENTERtype == qh_AScentrum) {
                if (!facet->center) facet->center = qh_getcentrum(qh, facet);
                centrum = facet->center;
            } else {
                if (!centrum_warning &&
                    !facet->simplicial) { /* recomputed centrum correct for simplicial facets */
            ...
            if (tempcentrum) qh_memfree(qh, centrum, qh->normal_size);
        }
    }
    if (waserror && !qh->FORCEoutput) qh_errexit2(qh, qh_ERRprec, errfacet1, errfacet2);
} /* checkconvex */
...
void qh_checkfacet(qhT *qh, facetT *facet, boolT newmerge, boolT *waserrorp) {
    ...
    if (facet->simplicial) {
        ...
        if (!newmerge && (numvertices < qh->hull_dim || numneighbors < qh->hull_dim) &&
        ...
        /* in 3-d, can get a vertex twice in an edge list, e.g., RBOX 1000 s W1e-13 t995849315 D2 |
         * QHULL d Tc Tv TP624 TW1e-13 T4 */
        ...
        if (!facet->newfacet && !neighbor->newfacet) {
            if ((!ridge->tested) | ridge->nonconvex | ridge->mergevertex) {
                qh_fprintf(
                    ...
                    ridge->id, ridge->nonconvex, ridge->mergevertex, ridge->tested, facet->id,
                    neighbor->id);
                errridge = ridge;
                waserror = True;
            }
    ...
    if (!facet->simplicial) {
        ...
        FOREACHneighbor_(facet) {
            if (neighbor->simplicial && !facet->degenerate && !neighbor->degenerate) {
                ...
                if (skipA < 0 || skipB < 0 ||
                    !qh_setequal_skip(facet->vertices, skipA, neighbor->vertices, skipB)) {
                    qh_fprintf(
                        qh, qh->ferr, 6135,
    ...
    if (!newmerge && qh->CHECKduplicates && qh->hull_dim < 5 &&
        (qh->IStracing > 2 || qh->CHECKfrequently)) {
        FOREACHridge_i_(qh, facet->ridges) { /* expensive, if was merge and qh_maybe_duplicateridges
                                                hasn't been called yet */
            if (!ridge->mergevertex) {
                for (i = ridge_i + 1; i < ridge_n; i++) {
                    ridge2 = SETelemt_(facet->ridges, i, ridgeT);
                    if (SETelem_(ridge->vertices, last_v) ==
                        SETelem_(ridge2->vertices,
                                 last_v)) { /* SETfirst is likely to be the same */
...
/*-<a                             href="qh-poly_r.htm#TOC"
  ...
  notes:
    called by qh_buildtracing, qh_checkpolygon, qh_collectstatistics, qh_printfacetlist,
  qh_printsummary not called by qh_printlists
...
boolT qh_checklists(qhT *qh, facetT *facetlist) {
    ...
    boolT waserror = False, newseen = False, nextseen = False, newvertexseen = False,
          visibleseen = False;
    ...
    if (facetlist == qh->newfacet_list || facetlist == qh->visible_list) {
        ...
        trace2((qh, qh->ferr, 2110, "qh_checklists: check qh.%s_list f%d and qh.vertex_list v%d\n",
                (facetlist == qh->newfacet_list ? "newfacet" : "visible"), facetlist->id,
                getid_(vertexlist)));
    } else {
        vertexlist = qh->vertex_list;
        previousvertex = NULL;
        trace2((qh, qh->ferr, 2111, "qh_checklists: check %slist f%d and qh.vertex_list v%d\n",
                (facetlist == qh->facet_list ? "qh.facet_" : "facet"), getid_(facetlist),
                getid_(vertexlist)));
    }
    if (facetlist) {
        ...
        FORALLfacet_(facetlist) {
            if (facet->visitid >= qh->visit_id || facet->id >= qh->facet_id) {
                ...
                if (facet->visitid == qh->visit_id)
                    qh_fprintf(
                        ...
                        facet->id, facet->id, facet->next->id, getid_(previousfacet), facet->id,
                        getid_(previousfacet));
                else
                    qh_fprintf(
                        qh, qh->ferr, 6350,
                        ...
                        facet->id, qh->facet_id, facet->visitid, qh->visit_id,
                        getid_(previousfacet));
                if (previousfacet)
            ...
            if (facetlist == qh->facet_list) {
                ...
                if (facet == qh->newfacet_list) newseen = True;
                if (facet == qh->facet_next) nextseen = True;
            }
        }
        if (facetlist == qh->facet_list) {
    ...
    if (vertexlist) {
        ...
        FORALLvertex_(vertexlist) {
            if (vertex->visitid >= qh->vertex_visit || vertex->id >= qh->vertex_id) {
                ...
                if (vertex->visitid == qh->visit_id)
                    qh_fprintf(
                        ...
                        vertex->id, vertex->id, vertex->next->id, getid_(previousvertex),
                        vertex->id, getid_(previousvertex));
                else
                    qh_fprintf(
                        qh, qh->ferr, 6368,
                        ...
                        vertex->id, qh->vertex_id, vertex->visitid, qh->visit_id,
                        getid_(previousvertex));
                if (previousvertex)
            ...
            if (vertex == qh->newvertex_list) newvertexseen = True;
        }
...
/*-<a                             href="qh-poly_r.htm#TOC"
  ...
  notes:
    called by qh_addpoint, qh_all_vertexmerge, qh_check_output, qh_initialhull, qh_prepare_output,
  qh_triangulate call with qh.facet_list or qh.newfacet_list or another list checks num_facets and
  num_vertices if qh.facet_list
...
void qh_checkpolygon(qhT *qh, facetT *facetlist) {
    ...
    boolT waserror = False, newseen = False, newvertexseen = False, nextseen = False,
          visibleseen = False;
    boolT checkfacet;
    ...
    if (!qh_checklists(qh, facetlist)) {
        waserror = True;
        qh_fprintf(qh, qh->ferr, 6374,
                   "qhull internal error: qh_checklists failed in qh_checkpolygon\n");
        if (qh->num_facets < 4000) qh_printlists(qh);
    }
    if (facetlist != qh->facet_list || qh->ONLYgood) nextseen = True; /* allow f.outsideset */
    FORALLfacet_(facetlist) {
    ...
    if (facetlist == qh->facet_list) {
        ...
        if (qh->VERTEXneighbors) {
            ...
            FORALLfacet_(facetlist) {
                if (!facet->visible) totfacetvertices += qh_setsize(qh, facet->vertices);
            }
        ...
        if (qh->hull_dim == 3 && numvertices + numfacets - numridges / 2 != 2) {
            ...
            /* occurs if lots of merging and a vertex ends up twice in an edge list.  e.g., RBOX
             * 1000 s W1e-13 t995849315 D2 | QHULL d Tc Tv */
        }
    }
    if (waserror) qh_errexit2(qh, qh_ERRqhull, errorfacet, errorfacet2);
} /* checkpolygon */
...
void qh_checkvertex(qhT *qh, vertexT *vertex, boolT allchecks, boolT *waserrorp) {
    ...
    if (waserror) {
        qh_errprint(qh, "ERRONEOUS", NULL, NULL, NULL, vertex);
        if (errfacet) qh_errexit(qh, qh_ERRqhull, errfacet, NULL);
        *waserrorp = True;
    }
} /* checkvertex */
...
void qh_createsimplex(qhT *qh, setT *vertices /* qh.facet_list */) {
    ...
    FOREACHvertex_i_(qh, vertices) {
        ...
        if (toporient) newfacet->toporient = True;
...
void qh_delridge(qhT *qh, ridgeT *ridge) {
    if (ridge == qh->traceridge) qh->traceridge = NULL;
...
facetT *qh_findbestfacet(qhT *qh, pointT *point, boolT bestoutside, realT *bestdist,
    ...
    bestfacet = qh_findbest(qh, point, qh->facet_list, bestoutside, !qh_ISnewfacets,
                            bestoutside /* qh_NOupper */, bestdist, isoutside, &totpart);
    if (*bestdist < -qh->DISTround) {
        ...
        if ((isoutside && *isoutside && bestoutside) ||
            ...
            bestfacet = qh_findbest(qh, point, bestfacet, bestoutside, False, bestoutside, bestdist,
                                    isoutside, &totpart);
            totpart += numpart;
        }
    }
...
facetT *qh_findbestlower(qhT *qh, facetT *upperfacet, pointT *point, realT *bestdistp,
    ...
    FOREACHneighbor_(upperfacet) {
        if (neighbor->upperdelaunay || neighbor->flipped) continue;
        (*numpart)++;
    ...
    *bestdistp = bestdist;
    trace3((qh, qh->ferr, 3015, "qh_findbestlower: f%d dist %2.2g for f%d p%d\n", bestfacet->id,
            bestdist, upperfacet->id, qh_pointid(qh, point)));
    return bestfacet;
} /* findbestlower */
...
facetT *qh_findfacet_all(qhT *qh, pointT *point, boolT noupper, realT *bestdist, boolT *isoutside,
    ...
    *isoutside = False;
    FORALLfacets {
        if (facet->flipped || !facet->normal || facet->visible) continue;
        if (noupper && facet->upperdelaunay) continue;
        totpart++;
        qh_distplane(qh, point, facet, &dist);
        if (dist > *bestdist) {
            *bestdist = dist;
...
/*-<a                             href="qh-poly_r.htm#TOC"
  ...
  qh_findgood(qh, facetlist, goodhorizon )
    ...
    goodhorizon is count of good, horizon facets from qh_find_horizon, otherwise 0 from
  qh_findgood_all if not qh.MERGING and qh.GOODvertex>0 facet includes point as vertex if !match,
  returns goodhorizon if qh.GOODpoint facet is visible or coplanar (>0) or not visible (<0) if
  qh.GOODthreshold facet->normal matches threshold if !goodhorizon and !match, selects facet with
  closest angle to thresholds sets GOODclosest
...
void qh_findgood_all(qhT *qh, facetT *facetlist) {
    ...
    if (!qh->GOODvertex && !qh->GOODthreshold && !qh->GOODpoint && !qh->SPLITthresholds) return;
    if (!qh->ONLYgood) qh_findgood(qh, qh->facet_list, 0);
    FORALLfacet_(facetlist) {
        if (facet->good) numgood++;
    }
    if (qh->GOODvertex < 0 || (qh->GOODvertex > 0 && qh->MERGING)) {
        FORALLfacet_(facetlist) {
            if (facet->good &&
                ((qh->GOODvertex > 0) ^
                 !!qh_isvertex(qh->GOODvertexp, facet->vertices))) { /* convert to bool */
                if (!--numgood) {
                    if (qh->ONLYgood) {
                        qh_fprintf(
                            qh, qh->ferr, 7064,
...
void qh_initbuild(qhT *qh) {
    ...
    if (qh->GOODvertex > 0)
        ...
        qh->GOODvertexp = qh_point(qh, -qh->GOODvertex - 1);
    if ((qh->GOODpoint && (qh->GOODpointp < qh->first_point /* also catches !GOODpointp */
                           || qh->GOODpointp > qh_point(qh, qh->num_points - 1))) ||
        (qh->GOODvertex && (qh->GOODvertexp < qh->first_point /* also catches !GOODvertexp */
    ...
    if (qh->SCALElast)
        qh_scalelast(qh, qh->first_point, qh->num_points, qh->hull_dim, qh->MINlastcoord,
                     qh->MAXlastcoord, qh->MAXabs_coord);
    qh_detroundoff(qh);
    if (qh->DELAUNAY && qh->upper_threshold[qh->hull_dim - 1] > REALmax / 2 &&
        qh->lower_threshold[qh->hull_dim - 1] < -REALmax / 2) {
        for (i = qh_PRINTEND; i--;) {
            if (qh->PRINTout[i] == qh_PRINTgeom && qh->DROPdim < 0 && !qh->GOODthreshold &&
                !qh->SPLITthresholds)
    ...
    trace4((qh, qh->ferr, 4091,
            "qh_initbuild: create sentinels for qh.facet_tail and qh.vertex_tail\n"));
    ...
    if (qh->PRINToptions1st || qh->TRACElevel || qh->IStracing) {
        if (qh->TRACElevel || qh->IStracing)
            qh_fprintf(
                ...
                qh->TRACElevel, qh->IStracing, qh->TRACEpoint, qh->TRACEmerge, qh->TRACEdist,
                qh->tracefacet_id, qh->traceridge_id, qh->tracevertex_id, qh->TRACElastrun,
                qh->rbox_command, qh->qhull_command);
    ...
    if (qh->ONLYgood) {
        ...
        if (qh->GOODvertex > 0 && !qh->MERGING /* matches qh_partitionall */
            ...
            facet = qh_findbestnew(qh, qh->GOODvertexp, qh->facet_list, &dist, !qh_ALL, &isoutside,
                                   &numpart);
...
void qh_initialhull(qhT *qh, setT *vertices) {
    ...
    if (dist > qh->DISTround) { /* clearly flipped */
        trace1((
            ...
            dist, firstfacet->id));
        FORALLfacets facet->toporient ^= (unsigned char)True;
        qh_setfacetplane(qh, firstfacet);
    }
    FORALLfacets {
        if (facet != firstfacet) qh_setfacetplane(qh, facet); /* qh_joggle_restart if flipped */
    }
    FORALLfacets {
        if (facet->flipped) {
            trace1((
        ...
        if (dist > qh->DISTround) { /* clearly flipped, due to axis-parallel facet or coplanar
                                       firstfacet */
            trace1((
                qh, qh->ferr, 1031,
...
vertexT *qh_makenewfacets(qhT *qh, pointT *point /* qh.visible_list */) {
    ...
    FORALLvisible_facets {
        ...
        if (visible->simplicial) newfacet = qh_makenew_simplicial(qh, visible, apex, &numnew);
        if (!qh->NEWtentative) {
    ...
    if (!qh->NEWtentative) qh->NEWfacets = True;
    trace1((qh, qh->ferr, 1032,
            "qh_makenewfacets: created %d new facets f%d..f%d from point p%d to horizon\n", numnew,
            qh->first_newfacet, qh->facet_id - 1, qh_pointid(qh, point)));
    if (qh->IStracing >= 4) qh_printfacetlist(qh, qh->newfacet_list, NULL, qh_ALL);
    return apex;
} /* makenewfacets */
...
/*-<a                             href="qh-poly_r.htm#TOC"
  ...
  notes:
    ...
    both MRGdupridge and MRGflipped are required merges -- rbox 100 C1,2e-13 D4 t1 | qhull d Qbb
      can merge flipped f11842 skip 3 into f11862 skip 2 and vice versa (forced by
  goodmatch/goodmatch2) blocks -- cannot merge f11862 skip 2 and f11863 skip2 (the widest merge)
    ...
    working/unused.h: [jan'19] Dropped qh_matchdupridge_coplanarhorizon, it was the same or slightly
  worse.  Complex addition, rarely occurs
  ...
  design:
    ...
    repeat twice -- once to make best matches, once to match the rest
      ...
      if pass 1,
        ...
        set newfacet, facet, matchfacet's hyperplane (removes from mergecycle of coplanarhorizon
  facets)
    ...
    for (makematch = 0; makematch < 2;
        ...
        for (newfacet = atfacet, newskip = atskip; newfacet;
            ...
            for (scan = hash; (facet = SETelemt_(qh->hash_table, scan, facetT));
                ...
                zinc_(Zhashtests);
                if (qh_matchvertices(qh, 1, newfacet->vertices, newskip, facet->vertices, &skip,
                                     &same)) {
                    ...
                    if (SETelemt_(facet->neighbors, skip, facetT) != qh_DUPLICATEridge) {
                        if (!makematch) { /* occurs if many merges, e.g., rbox 100 W0 C2,1e-13 D6
                                             t1546872462 | qhull C0 Qt Tcv */
                            qh_fprintf(
                                qh, qh->ferr, 6155,
                        ...
                        nextfacet = facet;
                        nextskip = skip;
                    } else if (SETelemt_(newfacet->neighbors, newskip, facetT) ==
                               qh_DUPLICATEridge) {
                        if (makematch) {
                            if (newfacet->tricoplanar) {
                                ...
                                *hashcount -= 2; /* removed two unmatched facets */
                                ...
                                SETelem_(goodmatch2->neighbors, goodskip2) =
                                    qh_MERGEridge; /* undo selection of goodmatch */
                                SETelem_(facet->neighbors, skip) = newfacet;
                                SETelem_(newfacet->neighbors, newskip) = facet;
                                *hashcount -= 2; /* removed two unmatched facets */
                                trace2((
                                    ...
                                    goodmatch->id, goodskip, goodmatch2->id, goodskip2,
                                    newfacet->id, newskip, facet->id, skip, gooddist));
                                goodmatch2 = NULL;
                            } else {
                                SETelem_(facet->neighbors, skip) = newfacet;
                                SETelem_(newfacet->neighbors, newskip) =
                                    qh_MERGEridge; /* resolved by qh_mark_dupridges */
                                *hashcount -= 2;   /* removed two unmatched facets */
                                trace3((
                                    qh, qh->ferr, 3073,
                            ...
                            if (!facet->normal)
                                qh_setfacetplane(
                                    qh, facet); /* qh_mergecycle will ignore 'mergehorizon' facets
                                                   with normals, too many cases otherwise */
                            if (!newfacet->normal) qh_setfacetplane(qh, newfacet);
                            dupdist = qh_getdistance(qh, facet, newfacet, &low,
                                                     &high); /* ignore low/high */
                            ...
                            if (isduplicate) {
                            ...
                            } else if (dupdist < maxgood &&
                                       (!newfacet->flipped ||
                                        facet->flipped)) { /* disallow not-flipped->flipped */
                                if (!goodmatch || (!goodmatch->flipped && dupdist < gooddist)) {
                                    goodmatch = facet;
                                    goodskip = skip;
                                    goodmatch2 = newfacet;
                            ...
                            } else if (dupdist2 < maxgood &&
                                       (!facet->flipped ||
                                        newfacet->flipped)) { /* disallow not-flipped->flipped */
                                if (!goodmatch || (!goodmatch->flipped && dupdist2 < gooddist)) {
                                    goodmatch = newfacet;
                                    goodskip = newskip;
                                    goodmatch2 = facet;
                                ...
                                if (dupdist > maxdist) { /* could keep !flipped->flipped, but
...
void qh_nearcoplanar(qhT *qh /* qh.facet_list */) {
    ...
    if (!qh->KEEPcoplanar && !qh->KEEPinside) {
        FORALLfacets {
            if (facet->coplanarset) qh_setfree(qh, &facet->coplanarset);
        }
    } else if (!qh->KEEPcoplanar || !qh->KEEPinside) {
        qh_outerinner(qh, NULL, NULL, &innerplane);
        if (qh->JOGGLEmax < REALmax / 2) innerplane -= qh->JOGGLEmax * sqrt((realT)qh->hull_dim);
        numpart = 0;
        FORALLfacets {
...
vertexT *qh_nearvertex(qhT *qh, facetT *facet, pointT *point, realT *bestdistp) {
    ...
    if (facet->tricoplanar) {
        ...
        FOREACHneighbor_(apex) {
            if (neighbor->center == center) {
                FOREACHvertex_(neighbor->vertices) qh_setappend(qh, &vertices, vertex);
            }
        }
    } else
    ...
    if (facet->tricoplanar) qh_settempfree(qh, &vertices);
    *bestdistp = sqrt(bestdist);
    if (!bestvertex) {
        qh_fprintf(qh, qh->ferr, 6261,
    ...
    trace3((qh, qh->ferr, 3019, "qh_nearvertex: v%d dist %2.2g for f%d p%d\n", bestvertex->id,
            *bestdistp, facet->id, qh_pointid(qh, point))); /* bestvertex!=0 by QH2161 */
    return bestvertex;
} /* nearvertex */
...
vertexT *qh_newvertex(qhT *qh, pointT *point) {
    ...
    trace4((qh, qh->ferr, 4060, "qh_newvertex: vertex p%d(v%d) created\n",
            qh_pointid(qh, vertex->point), vertex->id));
    return (vertex);
} /* newvertex */
...
pointT *qh_point(qhT *qh, int id) {
    ...
    id -= qh->num_points;
    if (id < qh_setsize(qh, qh->other_points)) return SETelemt_(qh->other_points, id, pointT);
    return NULL;
} /* point */
...
void qh_point_add(qhT *qh, setT *set, pointT *point, void *elem) {
    ...
    if ((id = qh_pointid(qh, point)) < 0)
        qh_fprintf(qh, qh->ferr, 7067,
                   "qhull internal warning (point_add): unknown point %p id %d\n", point, id);
    else if (id >= size) {
        qh_fprintf(qh, qh->ferr, 6160,
                   "qhull internal error (point_add): point p%d is out of bounds(%d)\n", id, size);
        qh_errexit(qh, qh_ERRqhull, NULL, NULL);
    } else
...
setT *qh_pointfacet(qhT *qh /* qh.facet_list */) {
    ...
    FORALLfacets {
        ...
        FOREACHpoint_(facet->coplanarset) qh_point_add(qh, facets, point, facet);
        FOREACHpoint_(facet->outsideset) qh_point_add(qh, facets, point, facet);
    }
    return facets;
} /* pointfacet */
...
setT *qh_pointvertex(qhT *qh /* qh.facet_list */) {
    ...
    FORALLvertices qh_point_add(qh, vertices, vertex->point, vertex);
    return vertices;
} /* pointvertex */
...
void qh_prependfacet(qhT *qh, facetT *facet, facetT **facetlist) {
    ...
    trace4((qh, qh->ferr, 4061, "qh_prependfacet: prepend f%d before f%d\n", facet->id,
            getid_(*facetlist)));
    if (!*facetlist) (*facetlist) = qh->facet_tail;
    list = *facetlist;
...
void qh_printhashtable(qhT *qh, FILE *fp) {
    ...
    FOREACHfacet_i_(qh, qh->hash_table) {
        if (facet) {
            FOREACHneighbor_i_(qh, facet) {
                if (!neighbor || neighbor == qh_MERGEridge || neighbor == qh_DUPLICATEridge) break;
            }
            if (neighbor_i == neighbor_n) continue;
            qh_fprintf(qh, fp, 9283, "hash %d f%d ", facet_i, facet->id);
            FOREACHvertex_(facet->vertices) qh_fprintf(qh, fp, 9284, "v%d ", vertex->id);
...
void qh_printlists(qhT *qh) {
    ...
    FORALLfacets {
        if (++count % 100 == 0) qh_fprintf(qh, qh->ferr, 8109, "\n     ");
    ...
    qh_fprintf(
        ...
        getid_(qh->visible_list), getid_(qh->newfacet_list), getid_(qh->facet_next),
        getid_(qh->newvertex_list));
    count = 0;
    FORALLvertices {
        if (++count % 100 == 0) qh_fprintf(qh, qh->ferr, 8112, "\n     ");
...
void qh_replacefacetvertex(qhT *qh, facetT *facet, vertexT *oldvertex, vertexT *newvertex) {
    ...
    if (old_i < new_i) new_i--;
    if ((old_i & 0x1) != (new_i & 0x1)) facet->toporient ^= 1;
...
void qh_resetlists(qhT *qh, boolT stats,
                   boolT resetVisible /* qh.newvertex_list newfacet_list visible_list */) {
    vertexT *vertex;
    ...
    trace2((
        ...
        getid_(qh->newvertex_list), getid_(qh->newfacet_list), getid_(qh->visible_list),
        getid_(qh->facet_list), getid_(qh->facet_next), getid_(qh->vertex_list), qh->NEWfacets,
        qh->NEWtentative, stats));
    if (stats) {
    ...
    FORALLvertex_(qh->newvertex_list) vertex->newfacet = False;
...
void qh_setvoronoi_all(qhT *qh) {
    ...
    FORALLfacets {
        if (!facet->normal || !facet->upperdelaunay || qh->UPPERdelaunay) {
            if (!facet->center) facet->center = qh_facetcenter(qh, facet->vertices);
        }
    }
} /* setvoronoi_all */
...
/*-<a                             href="qh-poly_r.htm#TOC"
  ...
  notes:
    ...
    call after qh_check_output since may switch to Voronoi centers, and qh_checkconvex skips
  f.tricoplanar facets Output may overwrite ->f.triowner with ->f.area while running,
  'triangulated_facet_list' is a list of one non-simplicial facet followed by its 'f.tricoplanar'
  triangulated facets See qh_buildcone
*/
void qh_triangulate(qhT *qh /* qh.facet_list */) {
    ...
    qh->newvertex_list = qh->vertex_tail;
    for (facet = qh->facet_list; facet && facet->next;
         facet = nextfacet) { /* non-simplicial facets moved to end */
        nextfacet = facet->next;
        if (facet->visible || facet->simplicial) continue;
        /* triangulate all non-simplicial facets, otherwise merging does not work, e.g., RBOX c
         * P-0.1 P+0.1 P+0.1 D3 | QHULL d Qt Tv */
    ...
    qh_resetlists(qh, False,
                  !qh_RESETvisible /* qh.newvertex_list, empty newfacet_list and visible_list */);
    ...
    if (qh->IStracing >= 4) qh_printlists(qh);
    trace2(
        (qh, qh->ferr, 2052,
    ...
    for (facet = triangulated_facet_list; facet && facet->next; facet = nextfacet) {
        ...
        if (facet->visible) {
            if (facet->tricoplanar) { /* a null or mirrored facet */
                ...
                if (visible && !owner) {
                    /*  RBOX 200 s D5 t1001471447 | QHULL Qt C-0.01 Qx Qc Tv Qt -- f4483 had 6
                     * vertices/neighbors and 8 ridges */
                    trace2((
                        qh, qh->ferr, 2053,
            ...
            if (owner)
                ...
                owner = facet;
                nextfacet =
                    visible->next; /* rescan tricoplanar facets with owner, visible!=0 by QH6162 */
    ...
    if (qh->CHECKfrequently) qh_checkpolygon(qh, qh->facet_list);
...
void qh_triangulate_facet(qhT *qh, facetT *facetA, vertexT **first_vertex) {
    ...
    if (qh->IStracing >= 4) qh_printfacet(qh, qh->ferr, facetA);
    FOREACHneighbor_(facetA) {
        neighbor->seen = False;
        neighbor->coplanarhorizon = False;
    }
    if (qh->CENTERtype == qh_ASvoronoi &&
        !facetA->center /* matches upperdelaunay in qh_setfacetplane() */
    ...
    if (!(*first_vertex)) (*first_vertex) = qh->newvertex_list;
    qh->newvertex_list = NULL;
    qh->visible_list = NULL;
...
void qh_triangulate_link(qhT *qh, facetT *oldfacetA, facetT *facetA, facetT *oldfacetB,
    ...
    if (qh_setin(facetA->neighbors, facetB)) {
        if (!qh_setin(facetB->neighbors, facetA))
            errmirror = True;
        else if (!facetA->redundant || !facetB->redundant ||
                 !qh_hasmerge(qh->degen_mergeset, MRGmirror, facetA, facetB))
...
/*-<a                             href="qh-poly_r.htm#TOC"
  ...
  design:
    since a null facet duplicates the first two vertices, the opposing neighbors absorb the null
  facet if they are already neighbors, the opposing neighbors become MRGmirror facets
*/
void qh_triangulate_mirror(qhT *qh, facetT *facetA, facetT *facetB) {
    facetT *neighbor, *neighborB;
    int neighbor_i, neighbor_n;

    trace3((qh, qh->ferr, 3022,
    ...
    FOREACHneighbor_i_(qh, facetA) {
        ...
        if (neighbor == facetB && neighborB == facetA)
            ...
            if (qh_hasmerge(qh->degen_mergeset, MRGmirror, neighbor, neighborB)) continue;
        }
...
/*-<a                             href="qh-poly_r.htm#TOC"
  ...
  design:
    since a null facet duplicates the first two vertices, the opposing neighbors absorb the null
  facet if they are already neighbors, the opposing neighbors will be merged (MRGmirror)
*/
...
#else  /* qh_NOmerge */
void qh_triangulate(qhT *qh) { QHULL_UNUSED(qh) }
#endif /* qh_NOmerge */
...
/*-<a                             href="qh-poly_r.htm#TOC"
  ...
  notes:
    called by qh_checkfacet, qh_vertexintersect, qh_rename_sharedvertex, qh_findbest_pinchedvertex,
  qh_neighbor_intersections if !qh.QHULLfinished, non-simplicial facets may have f.vertices with
  extraneous vertices cleaned by qh_remove_extravertices in qh_reduce_vertices
*/
