...
void qh_distplane(qhT *qh, pointT *point, facetT *facet, realT *dist) {
    ...
    switch (qh->hull_dim) {
        ...
        case 3:
            *dist =
                facet->offset + point[0] * normal[0] + point[1] * normal[1] + point[2] * normal[2];
            break;
        case 4:
            *dist = facet->offset + point[0] * normal[0] + point[1] * normal[1] +
        ...
        case 5:
            *dist = facet->offset + point[0] * normal[0] + point[1] * normal[1] +
                    point[2] * normal[2] + point[3] * normal[3] + point[4] * normal[4];
            break;
        case 6:
            *dist = facet->offset + point[0] * normal[0] + point[1] * normal[1] +
                    point[2] * normal[2] + point[3] * normal[3] + point[4] * normal[4] +
        ...
        case 7:
            *dist = facet->offset + point[0] * normal[0] + point[1] * normal[1] +
                    point[2] * normal[2] + point[3] * normal[3] + point[4] * normal[4] +
                    point[5] * normal[5] + point[6] * normal[6];
            break;
        case 8:
            *dist = facet->offset + point[0] * normal[0] + point[1] * normal[1] +
                    point[2] * normal[2] + point[3] * normal[3] + point[4] * normal[4] +
                    point[5] * normal[5] + point[6] * normal[6] + point[7] * normal[7];
            break;
        default:
            *dist = facet->offset;
            coordp = point;
    ...
    zzinc_(Zdistplane);
    if (!qh->RANDOMdist && qh->IStracing < 4) return;
    if (qh->RANDOMdist) {
        randr = qh_RANDOMint;
        *dist += (2.0 * randr / qh_RANDOMmax - 1.0) * qh->RANDOMfactor * qh->MAXabs_coord;
    }
#ifndef qh_NOtrace
    if (qh->IStracing >= 4) {
...
/*-<a                             href="qh-geom_r.htm#TOC"
  ...
  qh_findbest(qh, point, startfacet, bestoutside, qh_ISnewfacets, qh_NOupper, dist, isoutside,
  numpart ) find facet that is furthest below a point for upperDelaunay facets returns facet only if
  !qh_NOupper and clearly above
  ...
  design:
    ...
    test horizon facets
*/
facetT *qh_findbest(qhT *qh, pointT *point, facetT *startfacet, boolT bestoutside,
                    boolT isnewfacets, boolT noupper, realT *dist, boolT *isoutside, int *numpart) {
...
#ifndef qh_NOtrace
    if (qh->IStracing >= 4 ||
        (qh->TRACElevel && qh->TRACEpoint >= 0 && qh->TRACEpoint == qh_pointid(qh, point))) {
        if (qh->TRACElevel > qh->IStracing) qh->IStracing = qh->TRACElevel;
        qh_fprintf(
            qh, qh->ferr, 8004,
    ...
    if (!startfacet->flipped) { /* test startfacet before testing its neighbors */
        *numpart = 1;
        ...
        if (!bestoutside && *dist >= qh->MINoutside && (!startfacet->upperdelaunay || !noupper)) {
    ...
    while (facet) {
        ...
        FOREACHneighbor_(facet) {
            if (!neighbor->newfacet && isnewfacets) continue;
            if (neighbor->visitid == visitid) continue;
    ...
    if (isnewfacets) {
        if (!bestfacet) { /* startfacet is upperdelaunay (or flipped) w/o !flipped newfacet
                             neighbors */
            bestdist = -REALmax / 2;
            bestfacet = qh_findbestnew(qh, point, qh->newfacet_list, &bestdist, bestoutside,
                                       isoutside, &numpartnew);
            ...
            if (qh_sharpnewfacets(qh)) {
                ...
                zinc_(Zfindnewsharp);
                bestfacet = qh_findbestnew(qh, point, bestfacet, &bestdist, bestoutside, isoutside,
                                           &numpartnew);
                testhorizon = False; /* qh_findbestnew calls qh_findbesthorizon */
                qh->findbestnew = True;
            } else
    ...
    if (!bestfacet)
        bestfacet =
            qh_findbestlower(qh, lastfacet, point, &bestdist,
                             numpart); /* lastfacet is non-NULL because startfacet is non-NULL */
    if (testhorizon)                   /* qh_findbestnew not called */
        bestfacet = qh_findbesthorizon(qh, !qh_IScheckmax, point, bestfacet, noupper, &bestdist,
                                       &numpartnew);
    *dist = bestdist;
...
/*-<a                             href="qh-geom_r.htm#TOC"
  ...
  qh_findbesthorizon(qh, qh_IScheckmax, point, startfacet, qh_NOupper, &bestdist, &numpart )
    ...
    qh.coplanarfacetset used to maintain current search set, reset whenever best facet is
  substantially better returns(ischeckmax): best facet updates f.maxoutside for neighbors of
  searched facets (if qh_MAXoutside) returns(!ischeckmax): best facet that is not upperdelaunay or
  newfacet (qh.first_newfacet) allows upperdelaunay that is clearly outside returns: bestdist is
  distance to bestfacet numpart -- updates number of distance tests
  ...
  design:
    for each horizon facet of coplanar best facets
      ...
      unless upperdelaunay or clearly outside
         update best facet
*/
facetT *qh_findbesthorizon(qhT *qh, boolT ischeckmax, pointT *point, facetT *startfacet,
                           boolT noupper, realT *bestdist, int *numpart) {
    ...
    searchdist =
        qh_SEARCHdist; /* an expression, a multiple of qh.max_outside and precision constants */
    ...
    while (True) {
        numfacet++;
        is_5x_minsearch = (ischeckmax && facet->nummerge > 10 &&
                           qh_setsize(qh, facet->neighbors) >
                               100); /* QH11033 FIX: qh_findbesthorizon: many tests for facets with
                                        many merges and neighbors.  Can hide coplanar facets, e.g.,
                                        'rbox 1000 s Z1 G1e-13' with 4400+ neighbors */
        trace4((
            qh, qh->ferr, 4002,
            "qh_findbesthorizon: test neighbors of f%d bestdist %2.2g f%d ischeckmax? %d noupper? %d minsearch %2.2g is_5x? %d searchdist %2.2g\n",
            facet->id, *bestdist, getid_(bestfacet), ischeckmax, noupper, minsearch,
            is_5x_minsearch, searchdist));
        FOREACHneighbor_(facet) {
            if (neighbor->visitid == visitid) continue;
            neighbor->visitid = visitid;
            if (!neighbor
                     ->flipped) { /* neighbors of flipped facets always searched via nextfacet */
                qh_distplane(
                    qh, point, neighbor,
                    &dist); /* duplicate qh_distpane for new facets, they may be coplanar */
                (*numpart)++;
                if (dist > *bestdist) {
                    if (!neighbor->upperdelaunay || ischeckmax ||
                        (!noupper && dist >= qh->MINoutside)) {
                        if (!ischeckmax) {
                            minsearch = dist - searchdist;
                            if (dist > *bestdist + searchdist) {
                                zinc_(Zfindjump); /* everything in qh.coplanarfacetset at least
                                                     searchdist below */
                                coplanarfacetset_size = 0;
                            }
                        }
                    ...
                    continue; /* skip this neighbor, do not set nextfacet.  If ischeckmax, dist
                                 can't be positive */
#if qh_MAXoutside
                if (ischeckmax && dist > neighbor->maxoutside) neighbor->maxoutside = dist;
#endif
            ...
            if (nextfacet) {
                ...
                if (!coplanarfacetset_size++) {
                    ...
                    qh_setappend(qh, &qh->coplanarfacetset,
                                 nextfacet); /* Was needed for RBOX 1000 s W1e-13 P0 t996547055 |
                                QHULL d Qbb Qc Tv and RBOX 1000 s Z1 G1e-13 t996564279 | qhull Tv */
            }
            nextfacet = neighbor;
        } /* end of EACHneighbor */
        facet = nextfacet;
    ...
    if (!ischeckmax) {
        ...
        if (newbest) zinc_(Znewbesthorizon);
    }
    trace4((
        qh, qh->ferr, 4003,
        "qh_findbesthorizon: p%d, newbest? %d, bestfacet f%d, bestdist %2.2g, numfacet %d, coplanarfacets %d, numdist %d\n",
        qh_pointid(qh, point), newbest, getid_(bestfacet), *bestdist, numfacet, numcoplanar,
        *numpart - numpartinit));
    return bestfacet;
} /* findbesthorizon */
...
/*-<a                             href="qh-geom_r.htm#TOC"
  ...
  design:
    ...
    test horizon facets
*/
facetT *qh_findbestnew(qhT *qh, pointT *point, facetT *startfacet, realT *dist, boolT bestoutside,
    ...
    if (!startfacet || !startfacet->next) {
        if (qh->MERGING) {
            qh_fprintf(
                qh, qh->ferr, 6001,
                "qhull topology error (qh_findbestnew): merging has formed and deleted a cone of new facets.  Can not continue.\n");
            qh_errexit(qh, qh_ERRtopology, NULL, NULL);
        } else {
            qh_fprintf(qh, qh->ferr, 6002,
    ...
    *numpart = 0;
    ...
    if (qh->IStracing >= 4 ||
        (qh->TRACElevel && qh->TRACEpoint >= 0 && qh->TRACEpoint == qh_pointid(qh, point))) {
        if (qh->TRACElevel > qh->IStracing) qh->IStracing = qh->TRACElevel;
        qh_fprintf(qh, qh->ferr, 8008,
                   "qh_findbestnew: point p%d facet f%d. Stop? %d if dist > %2.2g,",
                   qh_pointid(qh, point), startfacet->id, isdistoutside, distoutside);
        qh_fprintf(qh, qh->ferr, 8009, " Last qh_addpoint p%d, qh.visit_id %d, vertex_visit %d,",
                   qh->furthest_id, visitid, qh->vertex_visit);
            ...
            if (!facet->flipped) {
                ...
                if (*dist > bestdist) {
                    if (!facet->upperdelaunay || *dist >= qh->MINoutside) {
                        bestfacet = facet;
                        if (isdistoutside && *dist >= distoutside) goto LABELreturn_bestnew;
                        bestdist = *dist;
                    }
                }
            } /* end of !flipped */
    ...
    if (testhorizon ||
        !bestfacet) /* testhorizon is always True.  Keep the same code as qh_findbest */
        bestfacet =
            qh_findbesthorizon(qh, !qh_IScheckmax, point, bestfacet ? bestfacet : startfacet,
...
*/
void qh_backnormal(qhT *qh, realT **rows, int numrow, int numcol, boolT sign, coordT *normal,
                   boolT *nearzero) {
    int i, j;
...
#ifndef qh_NOtrace
                if (qh->IStracing >= 4) {
                    qh_fprintf(qh, qh->ferr, 8011,
                               "qh_gausselim: 0 pivot at column %d. (%2.2g < %2.2g)\n", k,
                               pivot_abs, qh->DISTround);
...
pointT *qh_getcentrum(qhT *qh, facetT *facet) {
    ...
    trace4((qh, qh->ferr, 4007, "qh_getcentrum: for f%d, %d vertices dist= %2.2g\n", facet->id,
            qh_setsize(qh, facet->vertices), dist));
    return centrum;
} /* getcentrum */
...
*/
void qh_normalize2(qhT *qh, coordT *normal, int dim, boolT toporient, realT *minnorm,
                   boolT *ismin) {
    int k;
    ...
    if (dim == 2)
        ...
        norm =
            (*normal) * (*normal) + (*norm1) * (*norm1) + (*norm2) * (*norm2) + (*norm3) * (*norm3);
        for (k = dim - 4, colp = normal + 4; k--; colp++) norm += (*colp) * (*colp);
        norm = sqrt(norm);
    }
    if (minnorm) {
        if (norm < *minnorm)
    ...
    if (norm > qh->MINdenom) {
        ...
        *norm1 /= norm;
        if (dim == 2)
            ...
            *norm2 /= norm;
            *norm3 /= norm;
            for (k = dim - 4, colp = normal + 4; k--;) *colp++ /= norm;
        }
    } else if (norm == 0.0) {
        temp = sqrt(1.0 / dim);
        ...
        for (k = dim, colp = normal; k--; colp++) { /* k used below */
            ...
            if (!zerodiv)
                ...
                *maxp = temp;
                ...
                /* qh_joggle_restart ignored for Znearlysingular, normal part of
...
void qh_setfacetplane(qhT *qh, facetT *facet) {
    ...
    zzinc_(Zsetplane);
    if (!facet->normal) qh_memalloc_(qh, normsize, freelistp, facet->normal, coordT);
#ifndef qh_NOtrace
    ...
    if (qh->hull_dim <= 4) {
        ...
        qh_sethyperplane_det(qh, qh->hull_dim, qh->gm_row, point0, facet->toporient, facet->normal,
                             &facet->offset, &nearzero);
    }
    if (qh->hull_dim > 4 || nearzero) {
        ...
        if (nearzero) {
            if (qh_orientoutside(qh, facet)) {
                trace0((
                    qh, qh->ferr, 2,
                    "qh_setfacetplane: flipped orientation due to nearzero gauss and interior_point test.  During p%d\n",
                    qh->furthest_id));
    ...
    if (qh->DELAUNAY) {
        if (qh->UPPERdelaunay) { /* matches qh_triangulate_facet and qh.lower_threshold in
                                    qh_initbuild */
            ...
            if (vertex->point != point0) {
                ...
                if (istrace) {
                    qh_fprintf(
                        ...
                        qh_pointid(qh, vertex->point), vertex->id, dist, facet->id,
                        qh->furthest_id);
...
#ifndef qh_NOtrace
    if (qh->IStracing >= 4) {
        qh_fprintf(qh, qh->ferr, 8017, "qh_setfacetplane: f%d offset %2.2g normal: ", facet->id,
                   facet->offset);
        for (k = 0; k < qh->hull_dim; k++)
...
*/
void qh_sethyperplane_det(qhT *qh, int dim, coordT **rows, coordT *point0, boolT toporient,
                          coordT *normal, realT *offset, boolT *nearzero) {
    realT maxround, dist;
    int i;
    pointT *point;
    ...
    if (dim == 2) {
        ...
        *nearzero = False; /* since nearzero norm => incident points */
        ...
        qh_normalize2(qh, normal, dim, toporient, NULL, NULL);
        *offset = -(point0[0] * normal[0] + point0[1] * normal[1] + point0[2] * normal[2]);
        ...
        normal[0] = -det3_(dY(2, 0), dZ(2, 0), dW(2, 0), dY(1, 0), dZ(1, 0), dW(1, 0), dY(3, 0),
                           dZ(3, 0), dW(3, 0));
        normal[1] = det3_(dX(2, 0), dZ(2, 0), dW(2, 0), dX(1, 0), dZ(1, 0), dW(1, 0), dX(3, 0),
                          dZ(3, 0), dW(3, 0));
        normal[2] = -det3_(dX(2, 0), dY(2, 0), dW(2, 0), dX(1, 0), dY(1, 0), dW(1, 0), dX(3, 0),
                           dY(3, 0), dW(3, 0));
        normal[3] = det3_(dX(2, 0), dY(2, 0), dZ(2, 0), dX(1, 0), dY(1, 0), dZ(1, 0), dX(3, 0),
                          dY(3, 0), dZ(3, 0));
        qh_normalize2(qh, normal, dim, toporient, NULL, NULL);
        *offset = -(point0[0] * normal[0] + point0[1] * normal[1] + point0[2] * normal[2] +
                    point0[3] * normal[3]);
    ...
    if (*nearzero) {
        ...
        trace0((
            qh, qh->ferr, 3,
            "qh_sethyperplane_det: degenerate norm during p%d, use qh_sethyperplane_gauss instead.\n",
            qh->furthest_id));
    }
} /* sethyperplane_det */
...
/*-<a                             href="qh-geom_r.htm#TOC"
  ...
  design:
    ...
    compute offset
*/
void qh_sethyperplane_gauss(qhT *qh, int dim, coordT **rows, pointT *point0, boolT toporient,
                            coordT *normal, coordT *offset, boolT *nearzero) {
