...
void qh_qhull(qhT *qh) {
    ...
    if (!qh->STOPadd && !qh->STOPcone && !qh->STOPpoint) {
        if (qh->ZEROall_ok && !qh->TESTvneighbors && qh->MERGEexact) qh_checkzero(qh, qh_ALL);
        if (qh->ZEROall_ok && !qh->TESTvneighbors && !qh->WAScoplanar) {
            ...
            if (qh->MERGEexact || (qh->hull_dim > qh_DIMreduceBuild && qh->PREmerge))
                qh_postmerge(
                    qh, "First post-merge", qh->premerge_centrum, qh->premerge_cos,
            ...
            if (qh->POSTmerge)
                qh_postmerge(qh, "For post-merging", qh->postmerge_centrum, qh->postmerge_cos,
                             qh->TESTvneighbors);
            if (qh->visible_list == qh->facet_list) { /* qh_postmerge was called */
                ...
                qh_resetlists(
                    qh, False,
                    qh_RESETvisible /* qh.visible_list newvertex_list qh.newfacet_list */);
            }
        ...
        if (qh->KEEPnearinside && !qh->maxoutdone) qh_nearcoplanar(qh);
    }
...
boolT qh_addpoint(qhT *qh, pointT *furthest, facetT *facet, boolT checkdist) {
    ...
    qh->maxoutdone = False;
    if (qh_pointid(qh, furthest) == qh_IDunknown) qh_setappend(qh, &qh->other_points, furthest);
    if (!facet) {
        qh_fprintf(
            qh, qh->ferr, 6213,
    ...
    qh_detmaxoutside(qh);
    if (checkdist) {
        facet = qh_findbest(qh, furthest, facet, !qh_ALL, !qh_ISnewfacets, !qh_NOupper, &dist,
                            &isoutside, &numpart);
    ...
    if (!apex) {
        ...
        qh->retry_addpoint = 0;
        return True; /* ignore this furthest point, resolved a dupridge by making furthest a
                        coplanar point */
    }
    ...
    if (qh->PREmerge || qh->MERGEexact) {
        ...
        qh->findbestnew = True;
    if (qh->IStracing >= 4) qh_checkpolygon(qh, qh->visible_list);
    ...
    qh->NEWfacets = False;
    if (qh->IStracing >= 4) {
        if (qh->num_facets < 200) qh_printlists(qh);
    ...
    if (qh->STOPpoint > 0 && qh->furthest_id == qh->STOPpoint - 1 &&
        qh_setsize(qh, qh->vertex_mergeset) > 0)
        return False;
...
void qh_build_withrestart(qhT *qh) {
    ...
    while (True) {
        ...
        if (qh->JOGGLEmax < REALmax / 2) qh_joggleinput(qh);
        qh_initbuild(qh);
        qh_buildhull(qh);
...
*/
vertexT *qh_buildcone(qhT *qh, pointT *furthest, facetT *facet, int goodhorizon,
                      facetT **retryfacet) {
    vertexT *apex;
    realT newbalance;
    int numnew;
    ...
    *retryfacet = NULL;
    ...
    qh->NEWtentative =
        (qh->MERGEpinched || qh->ONLYgood); /* cleared by qh_attachnewfacets or qh_resetlists */
    apex = qh_makenewfacets(
        qh, furthest /* qh.newfacet_list visible_list, attaches new facets if !qh.NEWtentative */);
    numnew = (int)(qh->facet_id - qh->first_newfacet);
    newbalance =
        numnew - (realT)(qh->num_facets - qh->num_visible) * qh->hull_dim / qh->num_vertices;
    ...
    if (qh->ONLYgood) { /* qh.MERGEpinched is false by QH6362 */
        ...
        qh_fprintf(
            qh, qh->ferr, 6375,
            "qhull option error (qh_buildcone): option 'Q14' (qh.MERGEpinched) is not available due to qh_NOmerge\n");
        qh_errexit(qh, qh_ERRinput, NULL, NULL);
#endif
    } else {
        /* qh_makenewfacets attached new facets to the horizon */
        qh_matchnewfacets(
            qh); /* ignore returned value.  qh_forcedmerges will merge dupridges if any */
        qh_makenewplanes(qh /* qh.newfacet_list */);
        qh_update_vertexneighbors_cone(qh);
    }
    wadd_(Wnewbalance, newbalance);
    wadd_(Wnewbalance2, newbalance * newbalance);
    trace2((qh, qh->ferr, 2067,
            "qh_buildcone: created %d newfacets for p%d(v%d) new facet balance %2.2g\n", numnew,
            qh_pointid(qh, furthest), apex->id, newbalance));
    return apex;
} /* buildcone */
...
boolT qh_buildcone_mergepinched(qhT *qh, vertexT *apex, facetT *facet, facetT **retryfacet) {
    ...
    *retryfacet = NULL;
    ...
    if (maxdupdist >
        qh_RATIOtrypinched * qh->ONEmerge) { /* one or more dupridges with a wide merge */
        if (qh->IStracing >= 4 && qh->num_facets < 1000) qh_printlists(qh);
        qh_initmergesets(qh /* qh.facet_mergeset,degen_mergeset,vertex_mergeset */);
        if (qh_getpinchedmerges(qh, apex, maxdupdist,
                                &iscoplanar /* qh.newfacet_list, qh.vertex_mergeset */)) {
...
/*-<a                             href="qh-qhull_r.htm#TOC"
  ...
  notes:
    ...
    QH11030 FIX: Review effect of qh.GOODclosest on qh_buildcone_onlygood ('Qg').  qh_findgood
  preserves old value if didn't find a good facet.  See qh_findgood_all for disabling
...
boolT qh_buildcone_onlygood(qhT *qh, vertexT *apex, int goodhorizon) {
    ...
    if (qh_findgood(qh, qh->newfacet_list, goodhorizon) == 0) {
        if (!qh->GOODclosest) {
            ...
            qh_delvertex(qh, apex);
            qh_resetlists(qh, False /*no stats*/,
                          qh_RESETvisible /* qh.visible_list newvertex_list qh.newfacet_list */);
            zinc_(Znotgoodnew);
...
void qh_buildhull(qhT *qh) {
    ...
    FORALLvertices {
        ...
        if ((qh->STOPpoint > 0 && id == qh->STOPpoint - 1) ||
            ...
            trace1(
                (qh, qh->ferr, 1038, "qh_buildhull: stop point or cone P%d in initial hull\n", id));
            return;
        }
    }
...
At %02d:%02d:%02d & %2.5g CPU secs, qhull has created %d facets and merged %d.\n\
 ...
 outside points.  Next is point p%d(v%d), %2.2g above f%d.\n",
                   tp->tm_hour, tp->tm_min, tp->tm_sec, cpu, qh->facet_id - 1, total,
                   qh->num_facets, qh->num_vertices, qh->num_outside + 1, furthestid, qh->vertex_id,
                   dist, getid_(facet));
        ...
        qh_fprintf(
            ...
            furthestid, qh->vertex_id, dist, getid_(facet), qh->num_facets, zzval_(Ztotmerge),
            qh->num_outside + 1, cpu, qh->furthest_id, qh->vertex_id - 1, cpu - qh->lastcpu,
            qh->num_facets - qh->lastfacets, zzval_(Ztotmerge) - qh->lastmerges,
            zzval_(Zsetplane) - qh->lastplanes, zzval_(Zdistplane) - qh->lastdist,
            qh->retry_addpoint);
        qh->lastcpu = cpu;
    ...
    if (qh->vertex_visit > (unsigned int)INT_MAX) { /* 31 bits */
        ...
        qh->vertex_visit = 0;
        FORALLvertices vertex->visitid = 0;
    }
...
void qh_findhorizon(qhT *qh, pointT *point, facetT *facet, int *goodvisible, int *goodhorizon) {
    ...
    *goodvisible = *goodhorizon = 0;
    ...
    if (qh->IStracing >= 4) qh_errprint(qh, "visible", facet, NULL, NULL, NULL);
    qh->visit_id++;
    FORALLvisible_facets {
        ...
        FOREACHneighbor_(visible) {
            ...
            if (dist > qh->MINvisible) {
                ...
                if (qh->IStracing >= 4) qh_errprint(qh, "visible", neighbor, NULL, NULL, NULL);
            } else {
                ...
                if (qh->IStracing >= 4) qh_errprint(qh, "horizon", neighbor, NULL, NULL, NULL);
            }
        }
    }
    if (!numhorizon) {
...
pointT *qh_nextfurthest(qhT *qh, facetT **visible) {
    ...
    while ((facet = qh->facet_next) != qh->facet_tail) {
        ...
        if (qh->NARROWhull) {
            if (facet->notfurthest) qh_furthestout(qh, facet);
...
#endif
            if (dist <
                qh->MINoutside) { /* remainder of outside set is coplanar for qh_outcoplanar */
                qh->facet_next = facet->next;
                continue;
            }
        }
        ...
        if (qh->RANDOMoutside) {
            ...
            if (qh->NARROWhull) {
                FORALLfacets {
                    ...
                    if (facet->outsideset) outcoplanar += qh_setsize(qh, facet->outsideset);
                }
            }
            randr = qh_RANDOMint;
...
void qh_partitionall(qhT *qh, setT *vertices, pointT *points, int numpoints) {
    ...
    for (i = numpoints, point = points; i--; point += qh->hull_dim) *(pointp++) = point;
    qh_settruncate(qh, pointset, numpoints);
    FOREACHvertex_(vertices) {
        if ((id = qh_pointid(qh, vertex->point)) >= 0) SETelem_(pointset, id) = NULL;
    }
    id = qh_pointid(qh, qh->GOODpointp);
    if (id >= 0 && qh->STOPcone - 1 != id && -qh->STOPpoint - 1 != id)
        SETelem_(pointset, id) = NULL;
    if (qh->GOODvertexp && qh->ONLYgood && !qh->MERGING) { /* matches qhull()*/
        if ((id = qh_pointid(qh, qh->GOODvertexp)) >= 0) SETelem_(pointset, id) = NULL;
    }
    ...
    if (qh->BESToutside || qh->MERGING || qh->KEEPcoplanar || qh->KEEPinside ||
        ...
        qh->findbestnew = True;
        FOREACHpoint_i_(qh, pointset) {
            if (point) qh_partitionpoint(qh, point, qh->facet_list);
        }
        qh->findbestnew = False;
    }
    ...
    if (qh->IStracing >= 4) qh_printfacetlist(qh, qh->facet_list, NULL, True);
} /* partitionall */
...
void qh_partitioncoplanar(qhT *qh, pointT *point, facetT *facet, realT *dist, boolT allnew) {
    ...
    if (bestdist > qh->max_outside) {
        if (!dist && facet != bestfacet) { /* can't be recursive from qh_partitionpoint since facet
                                              != bestfacet */
            zinc_(Zpartangle);
        ...
        if (!repartition) {
            if (bestdist > qh->MAXoutside * qh_RATIOcoplanaroutside) {
                ...
                if (point == qh->coplanar_apex) {
                    /* otherwise may loop indefinitely, the point is well above a facet, yet near a
                     * vertex */
                    qh_fprintf(
                        qh, qh->ferr, 6425,
...
void qh_partitionpoint(qhT *qh, pointT *point, facetT *facet) {
    ...
    if (qh->findbestnew)
        bestfacet =
            qh_findbestnew(qh, point, facet, &bestdist, qh->BESToutside, &isoutside, &numpart);
    else
    ...
    if (isoutside) {
        if (!bestfacet->outsideset || !qh_setlast(bestfacet->outsideset)) { /* empty outside set */
            qh_setappend(qh, &(bestfacet->outsideset), point);
            if (!qh->NARROWhull || bestdist > qh->MINoutside) isnewoutside = True;
#if !qh_COMPUTEfurthest
        ...
        trace4((
            ...
            qh_pointid(qh, point), bestfacet->id, bestfacet->newfacet, isnewoutside));
               ...
               bestdist >= -qh->MAXcoplanar) { /* for 'd', bestdist skips upperDelaunay facets */
        if (qh->DELAUNAY) qh_joggle_restart(qh, "nearly incident point");
        ...
        zinc_(Zpartinside);
        trace4((qh, qh->ferr, 4067,
                "qh_partitionpoint: point p%d is inside all facets, closest to f%d dist %2.2g\n",
                qh_pointid(qh, point), bestfacet->id, bestdist));
        if (qh->KEEPinside) qh_partitioncoplanar(qh, point, bestfacet, &bestdist, qh->findbestnew);
    }
} /* partitionpoint */
    ...
    *numoutside = 0;
    FORALLvisible_facets {
        ...
        if (visible->outsideset) {
            ...
            *numoutside += size;
            qh->num_outside -= size;
            FOREACHpoint_(visible->outsideset) qh_partitionpoint(qh, point, newfacet);
        }
    ...
    if (delsize > 0) {
        ...
        FOREACHvertex_(qh->del_vertices) {
            if (vertex->point && !vertex->partitioned) {
                ...
                if (allpoints) /* not used */
                    /* [apr'2019] infinite loop if vertex recreates the same facets from the same
                       horizon e.g., qh_partitionpoint if qh.DELAUNAY with qh.MERGEindependent for
                       all mergetype, ../eg/qtest.sh t427764 '1000 s W1e-13 D3' 'd' */
                    qh_partitionpoint(qh, vertex->point, qh->newfacet_list);
                else
                    qh_partitioncoplanar(qh, vertex->point, qh->newfacet_list, NULL,
                                         qh_ALL); /* search all new facets */
...
void qh_printsummary(qhT *qh, FILE *fp) {
    ...
    double cpu;
    int size, id, nummerged, numpinched, numvertices, numcoplanars = 0, nonsimplicial = 0,
                                                      numdelaunay = 0;
    facetT *facet;
    const char *s;
    ...
    if (qh->ERREXITcalled)
        ...
        if (qh->GOODvertex || qh->GOODpoint || qh->SPLITthresholds) goodused = True;
    } else if (qh->DELAUNAY) {
        if (qh->GOODvertex || qh->GOODpoint || qh->GOODthreshold) goodused = True;
...
Convex hull of %d points in %d-d:\n\n",
        ...
        if (goodused) qh_fprintf(qh, fp, 9316, "  Number of 'good' facets: %d\n", qh->num_good);
        if (nonsimplicial)
  ...
  qh_fprintf(qh, fp, 9329, "  partition standard deviation: %2.2g\n", stddev);
    ...
    if (nummerged) {
        qh_fprintf(qh, fp, 9330, "  Number of distance tests for merging: %d\n",
                   zzval_(Zbestdist) + zzval_(Zcentrumtests) + zzval_(Zvertextests) +
