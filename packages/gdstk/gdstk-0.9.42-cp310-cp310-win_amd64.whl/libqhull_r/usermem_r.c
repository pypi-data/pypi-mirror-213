...
*/
void qh_exit(int exitcode) { exit(exitcode); } /* exit */
...
/*-<a                             href="qh-user_r.htm#TOC"
  ...
  notes:
    qh_fprintf_stderr() is called when qh.ferr is not defined, usually due to an initialization
  error if msgcode is a MSG_ERROR (6000), caller should set qh.last_errcode (like qh_fprintf) or
  variable 'last_errcode'
...
void qh_fprintf_stderr(int msgcode, const char *fmt, ...) {
    ...
    va_start(args, fmt);
    if (msgcode) fprintf(stderr, "QH%.4d ", msgcode);
...
*/
void qh_free(void *mem) { free(mem); } /* free */
...
/*-<a                             href="qh-user_r.htm#TOC"
    ...
    notes:
      same as malloc()
*/
void *qh_malloc(size_t size) { return malloc(size); } /* malloc */
