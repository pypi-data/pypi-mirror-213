...
/*-<a                             href  ="qh-globa_r.htm#TOC"
  ...
  notes:
    ...
    check download size
    recompile user_eg_r.c, rbox_r.c, libqhull_r.c, qconvex_r.c, qdelaun_r.c qvoronoi_r.c, qhalf_r.c,
  testqset_r.c
*/
...
void qh_appendprint(qhT *qh, qh_PRINT format) {
    ...
    for (i = 0; i < qh_PRINTEND; i++) {
        if (qh->PRINTout[i] == format && format != qh_PRINTqhull) break;
        if (!qh->PRINTout[i]) {
...
void qh_checkflags(qhT *qh, char *command, char *hiddenflags) {
    ...
    while (*s) {
        ...
        if (strstr(hiddenflags, chkkey)) {
            ...
            while (!chkerr && *s && !isspace(*s)) {
                ...
                if (isalpha(opt)) {
                    chkopt[2] = opt;
                    if (strstr(hiddenflags, chkopt)) chkerr = chkopt;
                    if (prevopt != ' ') {
                        chkopt2[2] = prevopt;
                        chkopt2[3] = opt;
                        if (strstr(hiddenflags, chkopt2)) chkerr = chkopt2;
                    }
    ...
    if (waserr) qh_errexit(qh, qh_ERRinput, NULL, NULL);
} /* checkflags */
...
void qh_clear_outputflags(qhT *qh) {
    ...
    for (k = qh->input_dim + 1;
         k--;) { /* duplicated in qh_initqhull_buffers and qh_clear_outputflags */
...
unsigned long qh_clock(qhT *qh) {
    ...
    if (!clktck) {
        if ((clktck = sysconf(_SC_CLK_TCK)) < 0) {
            qh_fprintf(
                qh, qh->ferr, 6030,
                "qhull internal error (qh_clock): sysconf() failed.  Use qh_CLOCKtype 1 in user_r.h\n");
            qh_errexit(qh, qh_ERRqhull, NULL, NULL);
        }
    }
    if (times(&time) == -1) {
        qh_fprintf(
            qh, qh->ferr, 6031,
            "qhull internal error (qh_clock): times() failed.  Use qh_CLOCKtype 1 in user_r.h\n");
        qh_errexit(qh, qh_ERRqhull, NULL, NULL);
    }
...
void qh_freebuffers(qhT *qh) {
    ...
    if (qh->line) /* allocated by qh_readinput, freed if no error */
        qh_free(qh->line);
    if (qh->half_space) qh_free(qh->half_space);
    if (qh->temp_malloc) qh_free(qh->temp_malloc);
...
void qh_freebuild(qhT *qh, boolT allmem) {
    ...
    if (qh->del_vertices) qh_settruncate(qh, qh->del_vertices, 0);
    if (allmem) {
        ...
        FORALLvertices qh_setfreelong(qh, &(vertex->neighbors));
    }
    ...
    if (allmem) {
        ...
        while ((facet = qh->facet_list)) {
            if (!facet->newfacet || !qh->NEWtentative ||
                qh_setsize(qh, facet->ridges) > 1) { /* skip tentative horizon ridges */
                trace4((qh, qh->ferr, 4095,
                        "qh_freebuild: delete the previously-seen ridges of f%d\n", facet->id));
        ...
        freeall = True;
        if (qh_setlarger_quick(qh, qh->hull_dim + 1, &newsize)) freeall = False;
        FORALLfacets {
...
void qh_init_B(qhT *qh, coordT *points, int numpoints, int dim, boolT ismalloc) {
    ...
    if (qh->qhmem.LASTsize == 0) qh_initqhull_mem(qh);
    /* mem_r.c and qset_r.c are initialized */
    qh_initqhull_buffers(qh);
    qh_initthresholds(qh, qh->qhull_command);
    if (qh->PROJECTinput || (qh->DELAUNAY && qh->PROJECTdelaunay)) qh_projectinput(qh);
    if (qh->SCALEinput) qh_scaleinput(qh);
...
void qh_initflags(qhT *qh, char *command) {
    ...
    if (command <= &qh->qhull_command[0] ||
        command > &qh->qhull_command[0] + sizeof(qh->qhull_command)) {
        if (command != &qh->qhull_command[0]) {
            *qh->qhull_command = '\0';
            strncat(qh->qhull_command, command,
                    sizeof(qh->qhull_command) - strlen(qh->qhull_command) - 1);
        }
        while (*s && !isspace(*s)) /* skip program name */
            s++;
    }
    while (*s) {
        ...
        switch (*s++) {
            ...
            case 'C':
                if (!isdigit(*s) && *s != '.' && *s != '-') {
                    qh_fprintf(qh, qh->ferr, 7003,
                               "qhull input warning: no centrum radius given for option 'Cn'\n");
                    lastwarning = s - 1;
                } else {
                    if (*s == '-') {
            ...
            case 'E':
                if (*s == '-') {
                    ...
                    qh_fprintf(qh, qh->ferr, 7005,
                               "qhull option warning: no maximum roundoff given for option 'En'\n");
                    lastwarning = s - 1;
                } else {
            ...
            case 'H':
                ...
                if (start < t) {
                    if (!(qh->feasible_string =
                              (char *)calloc((size_t)(t - start + 1), (size_t)1))) {
                        qh_fprintf(qh, qh->ferr, 6034,
            ...
            case 'R':
                if (!isdigit(*s)) {
                    qh_fprintf(
                        qh, qh->ferr, 7007,
                        "qhull option warning: missing random perturbation for option 'Rn'\n");
                    lastwarning = s - 1;
                } else {
            ...
            case 'W':
                if (*s == '-') {
                    ...
                    qh_fprintf(qh, qh->ferr, 7011,
                               "qhull option warning: missing outside width for option 'Wn'\n");
                    lastwarning = s - 1;
                } else {
                    ...
                    switch (*s++) {
                        ...
                        case 'D':
                            if (!isdigit(*s)) {
                                qh_fprintf(
                                    qh, qh->ferr, 7004,
                                    "qhull option warning: missing dimension for option 'GDn'\n");
                                lastwarning = s - 2;
                            } else {
                                if (qh->DROPdim >= 0) {
                                    qh_fprintf(
                    ...
                    switch (*s++) {
                        ...
                        case 'A':
                            if (!isdigit(*s)) {
                                qh_fprintf(
                                    qh, qh->ferr, 7006,
                                    "qhull option warning: missing facet count for keep area option 'PAn'\n");
                                lastwarning = s - 2;
                            } else {
                        ...
                        case 'F':
                            if (!isdigit(*s)) {
                                qh_fprintf(
                                    qh, qh->ferr, 7010,
                                    "qhull option warning: missing facet area for option 'PFn'\n");
                                lastwarning = s - 2;
                            } else {
                        ...
                        case 'M':
                            if (!isdigit(*s)) {
                                qh_fprintf(
                                    qh, qh->ferr, 7090,
                                    "qhull option warning: missing merge count for option 'PMn'\n");
                                lastwarning = s - 2;
                            } else {
                    ...
                    switch (*s++) {
                        ...
                        case 'T':
                            ...
                            if (!isdigit(*s)) {
                                qh_fprintf(
                                    qh, qh->ferr, 7091,
                                    "qhull option warning: missing number of test points for option 'QTn'\n");
                                lastwarning = s - 2;
                            } else {
...
#else
                                    /* ignore 'Q14' for q_benchmark testing of difficult cases for
                                     * Qhull */
                                    qh_fprintf(
                                        qh, qh->ferr, 7099,
                                        "qhull option warning: option 'Q14-merge-pinched' disabled due to qh_NOmerge\n");
#endif
                                    break;
                                case '7':
                        ...
                        switch (*s++) {
                            ...
                            case 'I':
                                ...
                                t = qh_skipfilename(qh, s);
                                {
                                    ...
                                    qh_copyfilename(qh, filename, (int)sizeof(filename), s,
                                                    (int)(t - s)); /* WARN64 */
                                    s = t;
                                    if (!freopen(filename, "r", stdin)) {
                                        qh_fprintf(
                                            qh, qh->ferr, 6041,
                                            "qhull option error: cannot open 'TI' file \"%s\"\n",
                                            filename);
                                        qh_errexit(qh, qh_ERRinput, NULL, NULL);
                                    } else {
                            ...
                            case 'M':
                                if (!isdigit(*s)) {
                                    qh_fprintf(
                                        qh, qh->ferr, 7030,
                                        "qhull option warning: missing merge id for trace option 'TMn'\n");
                                    lastwarning = s - 2;
                                } else {
                            ...
                            case 'R':
                                if (!isdigit(*s)) {
                                    qh_fprintf(
                                        qh, qh->ferr, 7031,
                                        "qhull option warning: missing rerun count for trace option 'TRn'\n");
                                    lastwarning = s - 2;
                                } else {
                            ...
                            case 'W':
                                if (!isdigit(*s)) {
                                    qh_fprintf(
                                        qh, qh->ferr, 7033,
                                        "qhull option warning: missing max width for trace option 'TWn'\n");
                                    lastwarning = s - 2;
                                } else {
...
void qh_initqhull_buffers(qhT *qh) {
    ...
    qh->gm_matrix =
        (coordT *)qh_memalloc(qh, (qh->hull_dim + 1) * qh->hull_dim * (int)sizeof(coordT));
...
void qh_initqhull_globals(qhT *qh, coordT *points, int numpoints, int dim, boolT ismalloc) {
    ...
    if (qh->TRIangulate && qh->JOGGLEmax < REALmax / 2 && !qh->PREmerge && !qh->POSTmerge &&
        qh->PRINTprecision)
        qh_fprintf(
            qh, qh->ferr, 7038,
    ...
    if (qh->MERGING) {
        qh_fprintf(
            qh, qh->ferr, 6045,
            "qhull option error: merging not installed (qh_NOmerge) for 'Qx', 'Cn' or 'An')\n");
        qh_errexit(qh, qh_ERRinput, NULL, NULL);
    }
#endif
    ...
    qh->KEEPnearinside =
        (qh->DOcheckmax && !(qh->KEEPinside && qh->KEEPcoplanar) && !qh->NOnearinside);
    if (qh->MERGING)
        qh->CENTERtype = qh_AScentrum;
    else if (qh->VORONOI)
        qh->CENTERtype = qh_ASvoronoi;
    if (qh->TESTvneighbors && !qh->MERGING) {
        qh_fprintf(qh, qh->ferr, 6049,
...
#ifdef qh_NOtrace
    if (qh->IStracing || qh->TRACEmerge || qh->TRACEpoint != qh_IDnone ||
        qh->TRACEdist < REALmax / 2) {
        qh_fprintf(
            qh, qh->ferr, 6051,
            "qhull option error: tracing is not installed (qh_NOtrace in user_r.h).  Trace options 'Tn', 'TMn', 'TPn' and 'TWn' mostly removed.  Continue with 'Qw' (allow warning)\n");
        if (!qh->ALLOWwarning) qh_errexit(qh, qh_ERRinput, NULL, NULL);
    }
#endif
    if (qh->RERUN > 1) {
        qh->TRACElastrun = qh->IStracing; /* qh_build_withrestart duplicates next conditional */
        if (qh->IStracing && qh->IStracing != -1) {
            qh_fprintf(qh, qh->ferr, 8162,
                       "qh_initqhull_globals: trace last of TR%d runs at level %d\n", qh->RERUN,
                       qh->IStracing);
            qh->IStracing = 0;
        }
...
void qh_initqhull_mem(qhT *qh) {
    ...
    numsizes = 8 + 10;
    qh_meminitbuffers(qh, qh->IStracing, qh_MEMalign, numsizes, qh_MEMbufsize, qh_MEMinitbuf);
...
void qh_initqhull_outputflags(qhT *qh) {
    ...
    if (printgeom) {
        ...
        if (qh->PRINTnoplanes && !(qh->PRINTcoplanar + qh->PRINTcentrums + qh->PRINTdots +
                                   qh->PRINTspheres + qh->DOintersections + qh->PRINTridges)) {
            qh_fprintf(qh, qh->ferr, 6058,
                       "qhull option error: no output specified for Geomview\n");
            qh_errexit(qh, qh_ERRinput, NULL, NULL);
        }
        if (qh->VORONOI && (qh->hull_dim > 3 || qh->DROPdim >= 0)) {
            qh_fprintf(qh, qh->ferr, 6059,
...
/*-<a                             href="qh-globa_r.htm#TOC"
  ...
  initialized see: qh_maxmin() determines the precision constants qh_freeqhull()
*/
void qh_initqhull_start2(qhT *qh, FILE *infile, FILE *outfile, FILE *errfile) {
    ...
    /* memset is the same in qh_freeqhull() and qh_initqhull_start2() */
    memset((char *)qh, 0,
           sizeof(qhT) - sizeof(qhmemT) - sizeof(qhstatT)); /* every field is 0, FALSE, NULL */
    qh->NOerrexit = True;
    qh->DROPdim = -1;
    qh->ferr = errfile;
    qh->fin = infile;
    ...
    qh->tracefacet_id = UINT_MAX;  /* recompile to trace a facet, set to UINT_MAX when done, see
                                      userprintf_r.c/qh_fprintf */
    qh->traceridge_id = UINT_MAX;  /* recompile to trace a ridge, set to UINT_MAX when done, see
                                      userprintf_r.c/qh_fprintf */
    qh->tracevertex_id = UINT_MAX; /* recompile to trace a vertex, set to UINT_MAX when done, see
                                      userprintf_r.c/qh_fprintf */
    seed = (int)time(&timedata);
    qh_RANDOMseed_(qh, seed);
    qh->run_id = qh_RANDOMint;
    if (!qh->run_id) qh->run_id++; /* guarantee non-zero */
            ...
            while (*s && !isspace(key = *s++)) {
                if (key == 'b' && *s == 'B') {
                    ...
                    if (!isdigit(*s)) {
                        qh_fprintf(
                            qh, qh->ferr, 7047,
                            "qhull option warning: no dimension given for Qhull option 'Q%c'\n",
                            key);
                        lastwarning = lastoption;
                        continue;
                    }
...
*/
void qh_lib_check(int qhullLibraryType, int qhTsize, int vertexTsize, int ridgeTsize,
                  int facetTsize, int setTsize, int qhmemTsize) {
...
void qh_option(qhT *qh, const char *option, int *i, realT *r) {
    ...
    if (i) sprintf(buf + strlen(buf), " %d", *i);
    if (r) sprintf(buf + strlen(buf), " %2.2g", *r);
    ...
    if (buflen > remainder) {
        trace1((qh, qh->ferr, 1058,
                "qh_option: option would overflow qh.qhull_options. Truncated '%s'\n", buf));
    }
