...
import pyaioredis
from pyaioredis.exceptions import ConnectionError
...
class TestPubSubAutoDecoding:
    ...
    async def test_pattern_message_handler(self, r: pyaioredis.Redis):
        p = r.pubsub(ignore_subscribe_messages=True)
    ...
    async def test_context_manager(self, r: pyaioredis.Redis):
...
class TestPubSubRedisDown:
    async def test_channel_subscribe(self, r):
        r = pyaioredis.Redis(host="localhost", port=6390)
        p = r.pubsub()
