...
import pyaioredis
from pyaioredis.client import Monitor
...
async def _get_info(redis_url):
    client = pyaioredis.Redis.from_url(redis_url)
...
def create_redis(request, event_loop):
    """Wrapper around pyaioredis.create_redis."""
    ...
    async def f(url: str = request.config.getoption("--redis-url"), **kwargs):
        ...
        pool = pyaioredis.ConnectionPool(parser_class=parser_class, **url_options)
        client: pyaioredis.Redis = pyaioredis.Redis(connection_pool=pool)
        if single:
        ...
        def teardown():
            async def ateardown():
                ...
                except pyaioredis.ConnectionError:
...
async def wait_for_command(client: pyaioredis.Redis, monitor: Monitor, command: str):
