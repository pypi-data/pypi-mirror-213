...
class OrderFilling(Repr, IntEnum):
    ...
    Attributes:
        FOK (int): This execution policy means that an order can be executed only in the specified volume. If the necessary amount of a
        ...
        IOC (int): An agreement to execute a deal at the maximum volume available in the market within the volume specified in the order. If the
        ...
        RETURN (int): This policy is used only for market (ORDER_TYPE_BUY and ORDER_TYPE_SELL), limit and stop limit orders (ORDER_TYPE_BUY_LIMIT,
...
class OrderTime(Repr, IntEnum):
    ...
    Attributes:
        ...
        SPECIFIED_DAY (int): The order is active until 23:59:59 of the specified day. If this time appears to be out of a trading session,
...
class SymbolSwapMode(Repr, IntEnum):
    ...
    Attributes:
        ...
        REOPEN_BID (int): Swaps are charged by reopening positions. At the end of a trading day the position is closed. Next day it is reopened
...
class SymbolOrderGTCMode(Repr, IntEnum):
    ...
    Attributes:
        SYMBOL_ORDERS_GTC (int): Pending orders and Stop Loss/Take Profit levels are valid for an unlimited period until theirConstants,
            Enumerations and explicit cancellation


        SYMBOL_ORDERS_DAILY (int): Orders are valid during one trading day. At the end of the day, all Stop Loss and Take Profit levels, as well as
            pending orders are deleted.


        SYMBOL_ORDERS_DAILY_EXCLUDING_STOPS (int): When a trade day changes, only pending orders are deleted, while Stop Loss and Take Profit
...
class SymbolOptionMode(Repr, IntEnum):
    ...
    Attributes:
        ...
        AMERICAN (int): American option may be exercised on any trading day or before expiry. The period within which a buyer can exercise
...
class ErrorCode(Repr, IntEnum):
    ...
    Attributes:
        OK (int): generic success


        FAIL (int): generic fail


        INVALID_PARAMS (int): invalid arguments/parameters


        NO_MEMORY (int): no memory condition


        NOT_FOUND (int): no history


        INVALID_VERSION (int): invalid version


        AUTH_FAILED (int): authorization failed


        UNSUPPORTED (int): unsupported method


        AUTO_TRADING_DISABLED (int): auto-trading disabled


        INTERNAL_FAIL (int): internal IPC general error


        INTERNAL_FAIL_SEND (int): internal IPC send failed


        INTERNAL_FAIL_RECEIVE (int): internal IPC recv failed


        INTERNAL_FAIL_INIT (int): internal IPC initialization fail


        INTERNAL_FAIL_CONNECT (int): internal IPC no ipc


        INTERNAL_FAIL_TIMEOUT (int): = internal timeout
    """
    __enum_name__ = "RES_E"
    OK = mt5.RES_S_OK
    FAIL = mt5.RES_E_FAIL
    INVALID_PARAMS = mt5.RES_E_INVALID_PARAMS
    NO_MEMORY = mt5.RES_E_NO_MEMORY
    NOT_FOUND = mt5.RES_E_NOT_FOUND
    INVALID_VERSION = mt5.RES_E_INVALID_VERSION
    AUTH_FAILED = mt5.RES_E_AUTH_FAILED
    UNSUPPORTED = mt5.RES_E_UNSUPPORTED
    AUTO_TRADING_DISABLED = mt5.RES_E_AUTO_TRADING_DISABLED
    INTERNAL_FAIL = mt5.RES_E_INTERNAL_FAIL
    INTERNAL_FAIL_SEND = mt5.RES_E_INTERNAL_FAIL_SEND
    INTERNAL_FAIL_RECEIVE = mt5.RES_E_INTERNAL_FAIL_RECEIVE
    INTERNAL_FAIL_INIT = mt5.RES_E_INTERNAL_FAIL_INIT
    INTERNAL_FAIL_CONNECT = mt5.RES_E_INTERNAL_FAIL_CONNECT
    INTERNAL_FAIL_TIMEOUT = mt5.RES_E_INTERNAL_FAIL_TIMEOUT
