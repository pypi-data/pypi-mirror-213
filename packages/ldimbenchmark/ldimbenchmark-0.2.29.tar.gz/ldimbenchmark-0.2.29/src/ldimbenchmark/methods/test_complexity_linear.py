...
class TestLinearComplexityLeakageDetectionMethod(LDIMMethodBase):
    """
    ...
    def prepare(self, train_data: BenchmarkData) -> None:
        number = int(len(train_data.demands[list(train_data.demands.keys())[0]]) / 280)
        # 16 Byte * 65536 = 1 MB
    ...
    def detect_offline(
        ...
        number = int(len(evaluation_data.demands[list(evaluation_data.demands.keys())[0]]) / 280)
        # 16 Byte * 65536 = 1 MB
        memory = [None] * number * 65536 * 2
        sleep(number / 100)
        return []
