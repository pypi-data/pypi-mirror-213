...
TEST_URL_PREFIX = 'http://10.129.31.21:8106/sa?project=zw_test_0608'
TEST_DEBUG_URL_PREFIX = 'http://10.129.31.21:8106/sa?project=zw_test_0608'
...
class NormalTest(unittest.TestCase):
    ...
    def testIDM3(self):
        # consumer = DefaultConsumer(TEST_URL_PREFIX)
        # consumer._do_request = self.mock_request
        # consumer = DebugConsumer(TEST_URL_PREFIX, False)

        consumer=AsyncBatchConsumer(TEST_URL_PREFIX, flush_max_time=3, flush_size=5)
        sa = SensorsAnalytics(consumer)

        sa.bind(SensorsAnalyticsIdentity(SensorsAnalyticsIdentity.MOBILE, "sv1"), SensorsAnalyticsIdentity(SensorsAnalyticsIdentity.EMAIL, "sv2"))
        ...
        sa.unbind(SensorsAnalyticsIdentity(SensorsAnalyticsIdentity.EMAIL, "sv2"))

        sa.track_by_id("hello", None, SensorsAnalyticsIdentity(SensorsAnalyticsIdentity.LOGIN_ID, "sv1"))

        sa.profile_set_by_id({"p1": "v1"}, SensorsAnalyticsIdentity(SensorsAnalyticsIdentity.EMAIL, "sv1"))
