...
# pid = os.getpid()
LEVEL_LOGS = (logging.INFO + logging.WARN)>>1
logging.addLevelName(LEVEL_LOGS, "logs")

logging.basicConfig(level=logging.INFO, format="[%(asctime)s][pid:%(process)d %(levelname)s] %(message)s", datefmt='%Y-%m-%d %H:%M:%S')
logger = logging.getLogger()

def set_log_level():
    logging.basicConfig(level=LEVEL_LOGS, format="[%(asctime)s][pid:%(process)d %(levelname)s] %(message)s", datefmt='%Y-%m-%d %H:%M:%S')

def dup(fname):

    set_log_level()

    format_str = logging.Formatter('[%(asctime)s][%(module)s:%(lineno)d:%(levelname)s] %(message)s')
    # 日志输出到控制台
    # stdout = logging.StreamHandler()
    # stdout.setFormatter(format_str)
    # logger.addHandler(stdout)

    # 日志输出到文件
    fout = logging.handlers.TimedRotatingFileHandler(filename=fname, when='D', backupCount=3, encoding='utf-8')
    fout.setFormatter(format_str)
    logger.addHandler(fout)

    logging.basicConfig(level=LEVEL_LOGS, filename=fname, filemode='a', datefmt='%Y-%m-%d %H:%M:%S')

import logging, logging.handlers
import syslog

def set_syslog(host="syslog.e4ting.cn", port=514):
    address=(host, port)
...
info  = logger.info
warn  = logger.warn
logs  = partial(logger.log, LEVEL_LOGS)
def log(*args, level='info'):
    # 0: ^10
    # self.mod       = function.__module__
    # self.func_name = "{}.{}".format(self.mod, str(function).split()[1])
    # logger = logging.getLogger("[{}](pid:{}) {}".format(datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'), pid, traceback.extract_stack()[-2][2]))
    # getattr(logger, level if hasattr(logger, level) else 'debug')(" ".join([str(_) for _ in args]))
