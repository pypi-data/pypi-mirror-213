...
def pip_depependency_pinned_alpha_version(
    pip_package_name, major_version, minor_version, micro_version
):
    """
    Generate a pip dependency string that is pinned to a range of pre-release alpha versions, for example:
    "databricks-feature-lookup~=0.3.1a,!=0.3.1,<0.3.2". This ensures only pre-released versions starting
    with 0.3.1 will be installed.
    Examples of satisfying versions: 0.3.1a1, 0.3.1a, 0.3.1a2
    Examples of denied versions: 0.2.1, 0.3.1, 0.3.2a, 0.4.1
    """
    conditions = [
        f"~={major_version}.{minor_version}.{micro_version}a",
        f"!={major_version}.{minor_version}.{micro_version}",
        f"<{major_version}.{minor_version}.{micro_version+1}",
    ]
    version_requirement = ",".join(conditions)
    return f"{pip_package_name}{version_requirement}"
