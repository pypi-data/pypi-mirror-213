...
URL = 'http://www.chuangfeigu.com:58888/froapi/aip/car_verify'
...
class CarClassify():
    ...
    def inference(self, file, top_num=5, baike_num=0):
        """车型识别接口

        Args:
            file (str): 汽车图像文件路径，只支持 jpg/png/bmp 格式
            top_num (bool, optional): 返回结果top n，默认5。
            baike_num (bool, optional): 返回百科信息的结果数，默认不返回
...
if __name__ == "__main__":
    ...
    car_cl = CarClassify(CUSTID, APIKEY)
    ret = car_cl.inference(IMAGE)
    print(ret)
