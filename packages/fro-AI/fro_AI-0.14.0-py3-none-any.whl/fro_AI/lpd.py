...
model_url = 'http://gz.chuangfeigu.com:8087/fro_AI/models/license_plate/license_plate_detection_lpd_yunet_2023mar-6d4978a7.onnx'
...
class LPD_YuNet:
    ...
    def infer(self, image):
        assert image.shape[0] == self.input_size[
            1], '{} (height of input image) != {} (preset height)'.format(
                image.shape[0], self.input_size[1])
        assert image.shape[1] == self.input_size[
            0], '{} (width of input image) != {} (preset width)'.format(
    ...
    def _postprocess(self, blob):
        ...
        # NMS
        keepIdx = cv.dnn.NMSBoxes(bboxes=dets[:, 0:4].tolist(),
                                  scores=dets[:, -1].tolist(),
                                  score_threshold=self.confidence_threshold,
                                  nms_threshold=self.nms_threshold,
                                  top_k=self.top_k)  # box_num x class_num
        if len(keepIdx) > 0:
            dets = dets[keepIdx]
    ...
    def __init__(self, model_path=None):
        if model_path is None:
            ...
            model_path = get_file(get_file_name(model_url),
                                  ...
                                  cache_subdir='models/license_plate')
        self.model = LPD_YuNet(modelPath=model_path)
        self.last_shape = None

    def detect(self, image):
        if image.shape != self.last_shape:
            self.last_shape = image.shape
            self.model.setInputSize((image.shape[1], image.shape[0]))
        return self.model.infer(image)

    def visualize(self, image, dets, line_color=(0, 255, 0)):
        output = image.copy()
