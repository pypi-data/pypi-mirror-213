...
class DBWorker( object ):
    """
     ...
     Args:
         data (list or dict): The data to be converted into a DataFrame.
         index (list, optional): The index labels for the DataFrame. Defaults to None.
         delitem (list, optional): The list of columns to be deleted from the DataFrame. Defaults to None.
    ...
    def to_df_many(self, cols, match=None, projection=None):
        """
         ...
         """
        dfs = [self._to_df_large( c, match=match, projection=projection ) for c in cols]
        df = pd.concat( dfs )
        df.reset_index( inplace=True, drop=True )
        return df
