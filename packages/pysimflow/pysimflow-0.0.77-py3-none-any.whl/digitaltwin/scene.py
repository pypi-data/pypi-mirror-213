...
class Scene:
  ...
  def pan(self,x,y):
    ...
    farPlane = 10000
    rayForwardToTarget = camTarget - camPos
    invLen = farPlane / dist
    rayForward = invLen * rayForwardToTarget
    oneOverWidth = float(1) / float(width)
    oneOverHeight = float(1) / float(height)
    
    f = 10000 / dist

    horizon = np.array(horizon) / f
    vertical = np.array(vertical) / f

    dHor = horizon * oneOverWidth
    dVer = vertical * oneOverHeight
    
    length = np.array([-float(x) * dHor,float(y) * dVer,0])
    pos = camTarget + Rotation.from_euler('xyz',[0,0,yaw],True).apply(length)
