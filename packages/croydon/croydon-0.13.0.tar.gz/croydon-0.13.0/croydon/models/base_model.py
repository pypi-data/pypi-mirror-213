...
from .fields import ObjectIdField, ReferenceOnDestroy
    ...
    async def _check_refs_on_destroy(self):
        if self._references:
            for ref in self._references:
                ref_field = getattr(ref.ref_class, ref.ref_field)
                if ref_field.on_destroy == ReferenceOnDestroy.RAISE:
                    ref_count = await ref.ref_class.find({ref.ref_field: self.id}).count()
                    if ref_count > 0:
                        raise ObjectHasReferences(f"{self.__class__.__name__} has existing")
                elif ref_field.on_destroy == ReferenceOnDestroy.CASCADE:
