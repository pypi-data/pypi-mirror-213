...
def generate_select_sql(catalog_entry, columns, limit=None):
    ...
    select_sql = f"SELECT {limit_clause}{','.join(escaped_columns)} FROM {escaped_db}.{escaped_table}"
...
def should_sync_column(md_map, field_name):
    ...
    return singer.should_sync_field(field_metadata.get('inclusion'), field_metadata.get('selected'), True)
